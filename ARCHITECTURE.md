...
- **👥 유즈맵 소셜 — 친구는 한 목록, 파티는 게시판이 앞에 선다**(2026-08-18).
  ...
  - 파티 칸 8개(2열×4행)는 소셜 구역 안에 **스크롤 없이** 들어와야 한다 — 칸 높이 30px · 격자 간격 4px · 아바타 20px · 닉 10px / 역할 8.5px. 계산은 `구역 안높이 170 − 머리줄 31 = 139 ≥ 4×30 + 3×4 = 132`. 도크 `min-height` 도 186px 로 못 박아 짧은 화면에서 다시 스크롤이 생기지 않게 했다. 스모크가 `scrollHeight <= clientHeight` 로 검사한다.
