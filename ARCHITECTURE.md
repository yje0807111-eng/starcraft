...
- **🎯 레벨 포인트(`LP_STATS` / `unit.pts` / `lpMul`)** — 레벨업으로 얻어 **선형 배수**(`1 + n×LP_STEP`, 1p=+5%)로 붙는다. 총량 = `(레벨-1)×LP_PER_LEVEL`, **환생하면 레벨과 함께 되감긴다**. 세기 조절은 `LP_STEP` 하나. ⛔ 복리로 만들지 말 것 — 그건 환생 포인트(`rpMul`)의 몫이고, 둘 다 복리면 후반이 폭주한다. ⛔ 항목마다 `step` 을 다르게 두지 말 것(어디 찍는 게 이득인지가 숨은 지식이 된다).
  - 설계 의도: **사냥터는 주 무대가 아니다.** 유즈맵이 중심이고 여기는 눌러 놓고 떠나는 곳이라, 자주 눌러야 하는 것은 사냥터 업그레이드 하나로 두고 레벨 포인트는 낮은 배수로 조용히 쌓이게 한다.
  - 반영처는 넷: `csAxis`(CS 축) · `hbAllyMul`(동료·펫·터렛) · `hbBunkerAtkMul`(벙커) · 치명타 피해. ⚠ 아군 축은 CS 축 검사로 안 잡히므로 스모크가 `hbAllyMul`/`hbBunkerAtkMul` 을 **따로 실측**한다.
