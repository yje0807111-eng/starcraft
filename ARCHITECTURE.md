...
- **🤝 전직은 폐지됐고, 옛 상위 직업 12종이 그대로 '동료'가 됐다**(2026-08-12 설계 전환). `PROF_JOBS`는 **뿌리 3종만** 남았고(캐릭터는 만든 그대로 끝까지 간다), `profClassChange`·`profClassCost`·`hbGrowJobs`·`PROF_JOB_PARENT`는 **삭제됐다** — 되살리지 말 것.
  ...
  - **🎰 뽑기 `HB_MATE_GACHA`** — 누적 뽑기 횟수(`hunt.mateN`)로 단계가 오르고, 단계마다 등급 확률표가 있다. 설계상 **1단계는 상위 등급이 정확히 0%**(일반 90 / 레어 9 / 에픽 1)에서 시작한다. 표를 고칠 때 지킬 세 가지: ① 각 단계 확률 합 = 정확히 1 ② 단계가 오르면 **일반을 뺀 어떤 등급도 확률이 줄면 안 된다** ③ 최종 단계에서는 모든 등급이 0%보다 커야 한다(안 그러면 그 등급 동료는 영원히 못 얻는다). 셋 다 스모크가 검사한다. ⚠ 한 판은 **'뽑기 전' 단계 확률**로 굴린다(`hbGachaProbs(mateN-1)`) — 안 그러면 그 판이 자기 결과로 확률을 바꾼다.
