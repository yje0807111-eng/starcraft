...
### 🐺🗿 페럴·콜로서스 — 관리자에만 있다 (2026-08-20)
5종족 설계(`RACES.md`) 중 **페럴(수인)·콜로서스(거신)** 을 코드로 확정했다. 지금 사는 곳은 **관리자 페이지뿐**이다.
...
- ⛔ **오토배틀(`STK_*`)에는 넣지 않았다.** 관리자에서 확정한 뒤 옮기는 순서다 — `STK_RACES`/`STK_BUILDINGS`/`STK_UNITS`/`STK_RACE_ORDER`/`STK_RACE_STAT`에 두 종족 키가 **없는 것이 정상**이다.
...
- 아직 없는 것: `deploy`(전개)·`minRange`(최소 사거리)·광폭화 **로직**. `U` 에 필드만 있다 — **페럴 > 콜로서스 상성의 핵심**이라 이게 없으면 오각형이 성립하지 않는다.
