...
| `장비 페이퍼돌` | `_profPaperdoll`(초상+신체 위치 슬롯, 좌표는 `PROF_SLOT_POS`) · `_profGearPick`(슬롯별 선택) · `_gearPick` 상태 |
...
- **장비 화면은 페이퍼돌이다**: 캐릭터 그림 위/옆에 슬롯을 놓고(`_profPaperdoll`) 슬롯을 누르면 그 슬롯 아이템만 보여주는 화면(`_profGearPick`, 상태 `_gearPick`)으로 바뀐다. 그림은 **현재 직업의 유닛 초상**(`PROF_JOBS[jobId].unit` → `unitPortraitHTML`)이라 전직하면 그림도 바뀐다. 슬롯 좌표는 `PROF_SLOT_POS` 한 곳 — **지금 초상은 흉상**이라 그 기준으로 잡혀 있고, 전신 그림으로 바꾸면 이 표만 갈면 된다.
