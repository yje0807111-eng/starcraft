...
## 10. 함정 목록 (실제로 밟았던 것)
- **`M3D.sync`는 모든 모델 풀을 숨기지 않는다.** `shopModels`·`buildModels`·보스는 숨기지만 **뽑기 비콘(`beaconInsts`)·미건설 고스트(`ghostModels`)·배치 고스트(`buildGhostModels`)는 그대로 둔다.** 공용 캔버스를 빌려 쓰는 화면(던전)에서 이걸 안 숨기면 상점/건설에서 만든 **'미사일 포탑' 고스트**가 그 위에 은은하게 남는다 → `M3D.hideIdlePools()`를 빌릴 때 한 번 호출한다(상점·건설 탭은 매 프레임 다시 켜므로 부작용 없음).
