...
- **유즈맵 화면은 상자 하나다**(2026-08-18). `.msPanel` 안에 **정렬 띠(`#msSortTabs`) → 목록(`#msList`) → 소셜 도크(`#msSocialDock`, 채팅·친구·파티)** 가 차례로 들어간다. ⚠ 상자 안에 상자를 또 그리지 않는다 — 도크 안 `.msSocial` 은 자기 테두리·면·바깥여백을 벗고(마을 시트의 `.inTown` 과 같은 처리), 목록과의 경계는 도크의 `border-top` 1px 하나로만 말한다. 스모크가 셋이 같은 상자 안인지, 소셜이 자기 테두리를 되찾지 않았는지 검사한다.
