...
|---|---|---|
| 고원 도달 | **60/60** | **6/6** |
| 오르막이 유일한 길(막으면 못 올라감) | **60/60** | — |
| 통로 폭(오르막 5칸 중 위까지 뚫린 칸) | **94.8%** | — |
| 오르막을 밟은 판 | — | 5/6 |
| 등반까지 걸린 시간 | — | 20~23초 |
- 🅰🅱 옛 값(4칸)과 **차이가 없다**(둘 다 고원 6/6 · 오르막 6/6 · 막힌 칸 프레임 623 ↔ 359).
  1칸으로 줄여도 통로가 안 좁아진다 — 오르막 **줄**이 짧아질 뿐 **폭**(5칸)은 그대로이기 때문이다.
- ⚠ 「오르막을 밟은 판 5/6」의 1판은 **표본 누락으로 보인다** — 1칸이라 지나가는 프레임이 적어
  0.05초 표집이 건너뛴다. 그 판도 고원에는 올랐다(`hi:true`).
