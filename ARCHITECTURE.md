...
#### 🗺 단계·라운드 배지 `#campBar` (2026-08-25 · 3단계)

맵 위·재화 바 아래의 얇은 표시. **마크업은 `sc-ums-web.html`, 값은 `css/30-home.css`,
채우기는 `campBarRender()`** — 셋이 각자 한 벌씩이다. ⛔ JS 에서 마크업을 만들지 말 것.

| | 0단계(캠프) | 1~10단계(던전) |
|---|---|---|
| 클래스 | `.safe` — 중립 | `.dng` — 밑변 광원 `--neon`(위험) |
| 내용 | 「🏕 캠프 — 안전」 | 「⚔ 던전 N · 라운드 R/50 · 적 n」 + 진행 바 |

⚠ **매 프레임 불린다 — 바뀐 것만 쓴다**(`_campBarS` 캐시). 무조건 갈면 리플로가 초당 30번 난다.
화면을 떠날 때 `campBarReset()` 으로 캐시를 비운다(옛 값 잔상 방지).

⛔ **z-index 를 낮게 잡지 말 것.** `#vBuild` 안 형제들이 **6~8**(`cstMain` 6 · `cstFog`/`techMap3d` 7 ·
`cstLabels`/`cstPrev` 8)이라 그보다 낮으면 **맵 밑에 깔려 화면에 아예 안 나온다** — 실제로 3 으로
뒀다가 그랬다. 지금 **9**. 스모크가 형제들의 최대 z 와 비교해 지킨다.

⛔ **위치를 상수로 잡지 말 것.** `top:calc(var(--topPad) + var(--curH) + 8px)` — 재화 바 높이가
바뀌면 배지가 그 뒤로 깔린다(이것도 실제로 그랬다).

⛔ **진행 바에 시안(`--acc-sel`)을 쓰지 말 것** — 시안은 「지금 선택된 것」 전용이다(`DESIGN.md` §2).
진행은 파랑(`--info`) — 생산 진행 바(`.cgBar`)와 같은 규칙.

진입·완주·탈락 알림은 `campSay()` → `toast()`/`playSfx()`(등록된 단일 소스).
