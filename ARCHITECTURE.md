...
- 🚪 **오르막(램프)** — 절벽이 길을 막으므로 **유일한 통로**다(⛔ 빼지 말 것). 그림도 「비탈」로 읽혀야 한다:
  ⛔ 절벽만 안 그리는 식으로 두지 말 것 — 끊긴 자리가 **구멍**으로 보인다(2026-09-11 실측).
  읽히게 하는 것은 셋이다 — ① 위가 밝고 아래로 **사라지는 면** ② **어두운 계단 홈**(밝은 선을 그으면
  사다리로 보인다) ③ **절벽 면과 같은 색의 옆벽**(파고들었다는 증거). 고원 칠은 먼저 `clearRect` 로 걷는다.
  📐 폭 **5칸**(3칸은 사다리로 보인다) · 길이 **4칸**(2~3칸은 「끊긴 자리」) · 둘 사이 **12칸 이상**
  (붙으면 「넓은 오르막 하나」가 된다) — 전부 실측으로 잡았고 스모크가 잰다.
