...
> 🗄 **`js/99-attic.js` = 다락(휴지통).** 지금 화면에서 도달할 수 없는 코드를 여기로 모은다 —
> 살아 있는 코드와 섞여 있으면 어느 쪽이 지금 쓰는 것인지 알 수 없어서다(실제로 옛 화면이 여러 번 되살아났다).
> **무엇이 왜 여기 있는지는 `ATTIC.md`.** ⛔ 지우지 않는다(유보는 삭제가 아니다 · GAME_DIRECTION §5).
> ⚠ 다락은 **classic script 중 맨 마지막**이다 — 그 뒤에 무엇을 두지 말 것. 스모크 「다락」이 잡는다.
> 🔒 **CSS 다락 `css/99-attic.css`**(2026-09-05) — 어디에도 안 쓰는 클래스의 규칙. ⛔ `<link>` 하지 않는다(CSS 는 로드되면 곧 살아 있는 것).
> **래칫 `scripts/dead-audit.mjs`** 가 `npm test` 앞에서 죽은 함수·CSS 총량을 잰다 — `test/dead-known.json` 밖의 새 이름이면 실패. 이사 도구는 `attic-move.mjs`(JS) · `attic-css.mjs`(CSS). 왜·어떻게는 `ATTIC.md` §5.
...
| `js/04-profile.js` | 361 | 개인 프로필 — 레벨 곡선 · 유즈맵 보상(앵커는 **캠프 시급**) · 젬 지갑. 🗄 장비·펫·마을 환생은 다락 |
| `js/05-home.js` | 166 | HOME 껍데기 — 재화 바 갱신 · 더보기 ☰. 🗄 사냥터 업그레이드 카드·던전 고르기는 다락 |
| `js/06-daily.js` | 285 | 일일 퀘스트 · 출석 |
| `js/07-home-upgrade.js` | 220 | 하단 네비(`NAV_TREE`) · 유즈맵 소셜 도크 · 펫 장착 칸. 🗄 사냥터 UI는 다락 |
| `js/08-ui-parts.js` | 351 | 옛 `08-hunt.js` 에서 **살아남은 부품**만 — 던전 이름표(`HB_DUNGEONS`/`hbDun`) · `edgePush` · 3D 예열/부팅 · 로그아웃 확인 · `resIco` · 캠프 상점 · `segNavHTML`. 🗄 사냥터·토벌·마을·정비 본체는 다락(2026-09-10) |
| `js/09-usemap-base.js` | 547 | 유즈맵 바탕 — 코인 공학소(`metaBonus`) · 상시 공용 보스 · 보스방 파견/이펙트 · 유닛 판매·조합 |
...
| `js/20-camp-research.js` | | 🔬 캠프 연구 구역 — 19 의 값·상태를 읽는다 |
| `js/21-camp-battle.js` | | ⚔ 캠프 전투 — `campStepUnits` 한 함수가 표적·이동·사격을 다 한다 |
| `js/22-camp-rune.js` | | 💠 **룬** — 젬으로 사서 칸에 끼우는 성장 축(2026-09-02) |
| `js/23-camp-dungeon.js` | | 🏰 **던전 = 적 기지** — 배치표 · 릴레이 압박 · 구간 관문 · 전리품 · 안개(2026-09-09) |
| `js/24-terrain.js` | | 🗺 **지형층** — 벽 · 언덕(고원) · 램프. 네 조각 오토타일 · 안개와 같은 격자(2026-09-10) |
| `js/90-m3d.module.js` | 1472 | **유일한 module 스크립트** — three.js 3D → `window.M3D` |

### 🗺 지형층 — 벽·언덕·램프 (`js/24-terrain.js` · 2026-09-10)

**왜 층을 나눴나.** 그전에는 「벽」을 *바닥이 통째로 들어간 완성 타일*로 만들려 했다.
그러면 바닥 종류마다 벽 세트를 다시 뽑아야 하고(바닥 3 × 벽 12 = 36장), 언덕이 들어오면
(바닥 × 벽 × 높이)로 곱해진다. 그래서 셋으로 갈랐다:

| 층 | 무엇 | 어디 |
|---|---|---|
| ① 바닥 | 통 텍스처(반복) | `.bmapFloor` (16-build 이 만든다) |
| ② **지형** | 벽 · 절벽 · 램프 — **배경이 투명** | `.bmapTerr` (이 파일 · z-index 1) |
| ③ 그 위 | 건물 · 유닛 | `#techMap3d` (z 7) |

**격자는 안개와 같은 것이다**(`FOG_COLS` × 범위 비례 rows · `_fogTile` 규약).
⛔ 지형용 격자를 따로 만들지 말 것 — 시야 차단이 `f.height` 를 읽는데 두 격자가 어긋나면
「보이는 절벽」과 「가리는 절벽」이 한 칸씩 밀린다.

**그림은 네 조각 오토타일이다.** 한 칸을 1/4씩 넷으로 나누고, 각 조각은 이웃 셋(가로·세로·대각)만
보고 다섯 모양 중 하나를 고른다(`campTerrShape`): `out` / `ev` / `eh` / `fill` / `in`.
그래서 **그림 12칸**(3×3 블록 + 안쪽모서리 1칸)이면 256가지 배치가 전부 나온다.
⛔ 8방향 비트마스크(47조각)로 되돌리지 말 것 · ⛔ 회전으로 조각을 아끼지 말 것(빛이 같이 돈다).

**지형은 안 변한다 → 한 번 굽고(`campTerrBake`) 매 프레임엔 한 장만 그린다**(`campTerrDraw`).
⛔ 조각을 매 프레임 그리지 말 것: 칸 하나에 4번 × 48×80칸 = 15,000번이다.
그리는 자리 계산은 **안개와 똑같다**(`_techW2S(0,wy0) → (1,wy1)` 사각으로 확대).

⚠ **캔버스는 매 프레임 사라진다** — `renderBuildTab` 이 `#cstMain` 을 innerHTML 로 갈아 끼운다.
...
가장자리와 절벽 앞면만 또렷하게 그리고 윗면은 살짝 밝히기만 한다 — RTS 의 절벽이 원래 그렇게
읽힌다. ⛔ 바닥이 타일로 바뀌기 전에는 불투명으로 되돌리지 말 것.

**생성기는 씨앗을 받는다**(`campTerrGen(dg, seed)` · `campFoeRng` 재사용).
⛔ `Math.random` 금지 — 저장·복원하면 지형이 바뀌고 스모크가 못 잰다.
- 🏔 고원은 **지도 위 끝부터** `TERR_PLATEAU_Y1` 까지다. ⛔ 북쪽 경계를 만들지 말 것(가로줄 절벽이 「벽」으로 보인다).
- ⛰ 아래 가장자리는 **계단**이다(한 칸씩만 오르내린다). ⛔ 열마다 따로 굴리지 말 것 — 1칸짜리 이·저가 번갈아 나와 **빗살**이 된다(실측).
- 🚪 **오르막(램프)** — 절벽이 길을 막으므로 **유일한 통로**다(⛔ 빼지 말 것).
  📐 폭 **5칸**(3칸은 사다리로 보인다) · 길이 **1칸**(2026-09-11 사용자 확정: 「4칸에 걸쳐서 올라가는게
  아니고 1칸만에 바로」) · 둘 사이 **12칸 이상**(붙으면 「넓은 오르막 하나」가 된다) · 입구 줄은
  **평평하다**(`base` — 가장자리가 계단이라 그냥 파면 들쭉날쭉해 옆벽이 어긋난다). 스모크가 넷 다 잰다.

  **그림은 「절벽에 난 틈」이다** — 오르막 칸의 **아래 모서리**(= 절벽 앞면이 시작될 자리)에서 그리기
  시작해 낮은 땅으로 `apron + 0.9칸` 내려간다.
  ⛔⛔ 절벽 선 **위**(고원 안)에서 시작하지 말 것 — 고원과 색이 다른 것은 무엇이든 **「고원에 올려놓은
  상자」**로 읽힌다. 2026-09-11 에 이걸로 세 번 헛돌았다(밝은 미끄럼틀 → 사다리 난간 → 흰 판).
  읽히게 하는 것은 셋이다 — ① **단 셋**(위가 밝고 아래로 어두워진다 · 「걸어 내려간다」를 말하는 유일한
  신호) ② 단마다 밑에 **어두운 홈**(밝은 선을 그으면 사다리 가로대가 된다) ③ **쐐기 옆벽**(꼭대기는 폭 0,
  아래로 벌어진다 — 비탈이 위에서는 고원 높이라 벽이 없다. ⛔ 폭이 일정한 띠로 그리지 말 것: 평행한 두
  밝은 선이 되어 사다리 난간으로 보인다).
  ⚠ 입구에서는 고원의 **밝은 테를 걷어 내고**(`eUp`) 그 자리를 고원 윗면 값으로 메운다 — 안 걷으면 밝은
  선이 입구를 가로질러 「구멍에 뚜껑을 덮은」 꼴이 된다.
  🎬 고르는 도구는 **`scripts/camp-ramp-pick.mjs`**(안 여섯을 한 장에). ⚠ 비교판은 게임과 **같은 겹치기**
  라야 한다 — 바닥과 지형을 한 캔버스에 그리면 `clearRect` 가 바닥까지 지워 비탈이 **검은 구멍**으로
  보인다(1차 비교판이 통째로 거짓이었다).
- 🧱 벽은 **0.05 ~ 0.16 띠**에만. ⛔ 내 격자(0.18↓)·적 기지 줄(`CAMP_FOE_ROW` 맨 아래 +0.02)과 겹치지 말 것(실측: 병영 자리에 벽이 났다).
- 🚧 **길이 막히면 안 된다**(`campTerrConnected` · flood fill). 막혔으면 마지막 덩어리부터 걷어 낸다 —
  릴레이 던전은 적이 나한테 와야 성립한다.

#### 🧱 바닥 타일 (5단계 · 2026-09-11)

**그림 한 장 → 반복 텍스처.** 확대하면 뭉개지고(원본의 일부만 쓴다) 3D 건물과 축척이 안 맞던 것을
타일로 바꿨다. 표는 `CAMP_TILE`(js/19-camp.js) 하나다 — 던전 번호 → `{ 파일, px }`.

- ⭐ **배경 변수는 그대로 쓴다**(`--campBg`) — 까는 방식만 `#phone.tileFloor` 클래스로 가른다.
  ⛔ 두 번째 배경 변수를 만들지 말 것(어느 쪽이 진짜인지 알 수 없어진다).
- 📐 크기는 **px** 다. `.bmapFloor` 가 뷰 변환(scale)을 받으므로 px 로 두면 확대·축소를 저절로 따라간다.
  ⛔ **격자 한 칸(≈8px)에 맞추지 말 것** — 1024 짜리 그림이 뭉개져 흙은 갈색 단색, 금속은 회색 덩어리가 된다
  (2026-09-10 실측). 실측으로 고른 값은 **180px**(맵 폭 380 → 두 장쯤 · 비교 `docs/mock/camp-floor-pick0.png`).
- 🌑 **톤 낮추기(`.60`)를 걸지 않는다.** 그 값은 밝은 베이지 그림을 뒤로 물리려던 것이다 —
  이미 어두운 타일에 걸면 검은 여백이 된다(0.78 어둡다 · 0.92 어둡다 · **1.0 여기** · 1.15 뜬다).
  ⚠ 밝기를 바꾸고 싶으면 **CSS 한 줄**을 고친다(타일을 다시 뽑지 말 것).
- ⚠ 바닥이 평평해지므로 **지형층의 고원 색이 진해진다**(`TERR_HI_TOP.tile`) — 옛 그림에는 고원이 이미
  **그려져** 있어 연한 값(.16)이면 됐지만, 타일에서는 이 층이 혼자 「높은 데」를 말해야 한다.
  ⛔ 옛 그림을 쓰는 던전에서 진한 값으로 되돌리지 말 것(그림 위에 회청색 판이 얹힌다).
- 🩹 **들일 때 이음매를 본다**(`scripts/tile-import.mjs --flat`): 세로 방향 **전체 밝기 기울기**가 있으면
  깔았을 때 가로 줄무늬가 된다. 금속 갑판이 위 55 ↔ 아래 47 이었다 → 기울기만 나눠 51/51(이음매 6.65 → 4.3).
  ⛔ 줄별로 완전히 평탄화하지 말 것 — 무늬의 진짜 명암까지 지워져 밋밋해진다.

🎬 `node scripts/camp-floor-pick.mjs [던전번호]` — 반복 크기·밝기를 한 장에 붙여 비교한다.

#### 👁 시야 차단 (2단계 · 2026-09-10)

**막는 일은 이미 엔진이 하고 있었다.** `_fogReveal` 의 `if(!air && fogHeightAt(...)>vh) continue;`
한 줄이 「저지 → 고지」를 자른다 — 지금까지 그 배열이 **비어 있었을 뿐**이다(캠프는
`techFogInit(..., {flat:true})` 로 관리자 테스트 언덕조차 꺼 두었다). 그래서 2단계에 새로 만든 것은
**배열을 채우는 함수 둘**뿐이다. ⛔ 차단 규칙을 새로 짜지 말 것.

| 무엇 | 어디 |
|---|---|
| 엔진이 부르는 훅 | `techFogInit` 끝(10-engine) → `campTerrHeightFill(f)` |
| 실제로 채우는 곳 | `campTerrPushHeight(f)` — `campTerrSync` 가 지형을 새로 만들 때도 부른다 |

⚠ **훅을 「campTerrSync 만 부르고 끝」으로 줄이지 말 것.** 지형 서명이 그대로면 sync 는 아무것도 안 하는데
**안개는 방금 새로 파여 높이가 0**이다. 그리고 `techFogInit` 은 끝에서 **바로** `techFogCompute()` 를 돌리므로,
그때 0이면 **첫 시야가 절벽을 넘어** 퍼지고 그 한 번으로 고원 위 건물이 `b.vis` 로 켜진다
(「가 보지도 않았는데 보인다」). 그래서 훅은 **늘 다시 밀어 넣는다**.

🚪 **램프는 「낮은 쪽」으로 친다.** 비탈이니 아래에서 보이는 것이 맞고, 그래야 올라갈 길이 보인다.
⛔ 램프를 고지로 치지 말 것 — 통로가 통째로 어둠에 잠겨 「어디로 올라가나」가 사라진다.
⚠ 그래서 램프 위의 유닛은 아직 고원을 못 본다. 다 올라 **고원 칸을 밟는 순간** 열린다(능선을 넘는 느낌).

🧹 **집으로 돌아오면 안개의 고저도 걷는다**(`_terrPushed` 가 참일 때만).
`campFogSync` 는 캠프에서 `f.on=false` 로 끄기만 하고 배열은 그대로 두므로, 안 걷으면 **절벽이 데이터에 남아**
다음에 안개를 켜는 순간 없는 벽이 시야를 막는다(2026-09-10 스모크가 잡았다).
⚠ **우리가 심은 것일 때만** 지운다 — 안 그러면 관리자 탭의 `TECH_HILL` 까지 지운다.

#### 🚶 길막기 — 전장 흐름장 (3단계 · 2026-09-11)

**⛔ 두 번 실패했다. 되돌리기 전에 여기를 읽을 것.**

| 시도 | 무엇을 했나 | 어떻게 깨졌나 |
|---|---|---|
| 2026-09-05 | 건물을 원형 장애물로 만들어 **국소 회피**에 맡겼다 | 아군이 갇혔다(의무병이 아군에게서 1152 → 1387) |
| 2026-09-10 | 지형을 **`_techNavRects`** 에 얹어 기지의 A\*(`_techFindPath`)를 빌렸다 | **또 갇혔다.** A/B 6판: 지형 끔 6/6 전진 · 켬 **4/6**(2판은 반대로 가서 굳었다) |

두 번째의 원인은 한 줄이다 — `_techFindPath` 는 **기지 격자 전용**이라 노드를 고를 때 `p[1]>0.12` 로 거른다
(`js/16-build.js`). 그래서 **격자 위 지형의 꼭짓점이 전부 버려진다**: 장애물은 보되 **돌아갈 모서리가 안 생긴다.**
⛔ 그 방식으로 되돌리지 말 것. 16-build.js 도 못 고친다(관리자 탭·오토배틀 공유).

**⭐ 지금 방식 — 전장은 제 길찾기를 갖는다.** 목적지 칸마다 **BFS 거리 지도**(흐름장)를 한 장 만들어 캐시하고,
유닛은 제 칸에서 **내리막**만 따라간다.

| 함수 | 하는 일 |
|---|---|
| `campTerrMask()` | 막힌 칸 표 — **지형 + 레인 밖**(아래 ⚠) |
| `campTerrFlow(goal)` | 목적지 칸 → BFS 거리 지도(LRU `TERR_FLOW_KEEP`) |
| `campTerrWay(gA,gB)` | 직선이 안 막히면 **null** · 막히면 흐름을 따라 **직선이 통하는 가장 먼 칸** |
| `campMove`(21-camp-battle) | `campTerrWay` 로 **목표만 바꾼다** — 건물 회피(`_techFindPath`)는 그대로 |

- ⭐ 지형은 흐름장, 건물은 A\* — **두 길찾기가 각자 제 일만 한다.** ⛔ 하나로 합치려 하지 말 것.
- ⚠ **밀어내기는 안 한다.** 억지로 밀려 들어간 유닛도 흐름을 타고 스스로 나온다(2026-09-05 의 교훈).
  그래서 「막힌 칸을 몇 프레임 밟았나」는 0 이 아니라 **0~1** 이 정상이다(우회 끄면 3~8 · 실측 8판).
- ⚠ **레인 밖을 반드시 막을 것.** 지형 격자는 gx 0~1 을 갖지만 **전장은 gx 0.06~0.94 만** 간다
  (`campG2W` 가 `CAMP_LANE_W` 로 펴고 물리가 0~world 로 자른다). 안 막으면 유닛이 gx 0.06 에 서서
  **gx 0.01 로 가라는 지시를 30초 동안** 받는다(실측 8판 중 1판이 영영 안 움직였다).
- ⚠ **연결성 검사도 레인 안에서** 잰다(`campTerrLaneRows`) — 격자 맨 위·아래 줄은 전장이 못 가는 자리다.
  ⛔ 「gy 가 든 행」으로 잡지 말 것: 마스크는 **칸 중심**으로 판단해 한 행 어긋나면 목표 행이 통째로 막혀
  **연결성 검사가 늘 실패한다**(실측 300/300 · 그러면 생성기가 벽을 전부 걷어 낸다).
- ⚠ **한 칸도 안 보여도 포기하지 않는다.** 벽에 바싹 붙으면 바로 옆 칸 중심까지도 선이 모서리를 스쳐
  「안 보인다」가 된다. 거기서 null 을 주면 직선으로 벽을 밀고 이동 물리의 진행도 창(`_pgHold`)이 걸려 **선다**.
- 🧹 `campTerrGen` 은 **비우고 시작한다** — 같은 지형에 두 번 부르면 램프·벽이 **쌓인다**(실측 12 → 22 → 33).

📊 실측(2026-09-11 · 8판): 벽 너머 도달 **8/8** · 고원(램프) 도달 **8/8** · 램프를 탄 판 4/8(우회 끄면 0/8) ·
던전 1 클리어 96~99초(기준선 93초).

📊 **등반 실측**(2026-09-11 · 오르막을 4칸 → 1칸으로 줄인 뒤 · `scripts/camp-terr-climb.mjs`)

**① 정적 3000판 — 격자는 설계대로다.**
고원 도달 **3000/3000** · 오르막을 막으면 **3000/3000 못 올라감**(= 마스크 위에서 오르막이 유일한 길) ·
오르막 5칸 중 위까지 뚫린 칸 **94.8%**(1칸으로 줄여도 통로가 안 좁아진다 — **줄**이 짧아질 뿐 **폭**은 그대로다).

**② 살아 있는 판 300판 — 유닛은 올라간다. 그런데 오르막으로만 올라가지는 않는다.**
| | 오르막 **1칸**(지금) | 오르막 4칸(옛) | 우회 끔(대조군) |
|---|---|---|---|
| 고원 도달 | **300/300** | 300/300 | 300/300 |
| 고원 밑변을 **오르막으로** 넘은 횟수 | 464 | 548 | 235 |
| 고원 밑변을 **절벽을 뚫고** 넘은 횟수 | 515 (**52.6%**) | 453 (45.3%) | 1609 (87.3%) |
| 막힌 칸 프레임(중앙) | 24 | 27 | 161 |
| 등반 시간(중앙) | 21.2초 | 21.4초 | 24.5초 |

- ⚠⚠ **「오르막이 유일한 통로」는 격자 위에서만 참이고 전장에서는 거짓이다.** 고원 밑변을 넘는 일의
  **절반쯤이 절벽을 그대로 통과한다.** 옆 칸 표집 오차가 아니다 — 오르막에서 **6칸 이상 떨어진 자리**에서
  넘은 것이 177회다(거리 분포 1칸 129 · 2칸 71 · 3칸 54 · 4칸 41 · 5칸 43 · **6+칸 177**).
  원인은 **밀어내기를 안 하는 설계**다(위 ⚠ 항목). `campTerrWay` 는 **목표만** 바꾸고 몸을 막지 않으므로,
  목표가 고원 위에 있으면 유닛은 절벽을 향해 직진하다 그냥 넘어간다.
- ⭐ 그래도 **흐름장은 제 일을 한다** — 끄면 절벽 통과가 45~53% → **87.3%**, 막힌 칸 프레임 중앙이 24 → **161**
  (6.7배)로 뛴다. 「우회가 아무 일도 안 한다」가 아니라 **「막지는 않고 유도만 한다」**가 맞는 표현이다.
- 📉 1칸이 4칸보다 절벽 통과가 **7%p 높다**(52.6% ↔ 45.3% · n≈1000 이라 오차 ±2%p). 비탈에 머무는
  프레임이 줄어서다. **고원 도달은 둘 다 300/300 이라 진행에는 영향이 없다.**
- 🔍 **새는 데를 갈라 셌다**(`camp-terr-climb.mjs` 넷째 인자 · 200판 587회):
  **우회점을 줬는데도 넘음 54.0%**(몸을 안 막는 설계 그 자체) · **직선이 「안 막혔다」고 봄 36.6%**
  (절벽 테두리는 **한 칸 두께**인데 `campTerrSegBlocked` 가 선 위에 점을 찍어 보므로 대각선이 그 한 칸을
  **점 사이로 건너뛴다**) · 막혔는데 우회점 없음 9.4% · `campMove` 를 안 탐 **0%**.
  넘는 순간 **목표가 고원 위인 것이 82%** — 적 본진·테크가 고원에 있어 그쪽으로 직진하다 밟고 오른다.
  ⛔ **「건물 A\* 길(`_cpWp`)을 들고 있어서」는 원인이 아니다**(0/422). 거기서 멈추지 말 것.
- ✅ **①은 고쳤다**(2026-09-11 · 직선 판정을 **격자 행진**으로) — 점 찍기는 한 칸 두께를 못 잡는다.
  ⛔ 점 찍기로 되돌리지 말 것 · ⛔ **「점을 더 촘촘히」로 고치려 하지 말 것**: 대각선은 칸 모서리를
  스치므로 **어떤 간격에도 빈틈이 남는다**. 스모크가 아주 촘촘한 점 찍기를 **기준자**로 삼아 견준다
  (주입 시험: 옛 방식으로 되돌리면 3/500 을 놓쳐 실패한다).

  | 같은 자(300씨앗 · 200판) | 점 찍기(옛) | **격자 행진(지금)** |
  |---|---|---|
  | 절벽 통과 | 352회 (51.5%) | **254회 (39.1%)** |
  | 오르막으로 넘음 | 332회 | **396회** |
  | 그중 「직선이 안 막혔다고 봄」 | 238 (38.8%) | **122 (30.5%)** |
  | 오르막을 밟은 판 | 184/200 | **198/200** |
  | 막힌 칸 프레임(중앙) | 31 | **24** |
  | 고원 도달 · 등반 시간(중앙) | 200/200 · 21.4초 | 200/200 · 21.4초 |

- ⛔⛔ **「절벽 위에서 속도를 깎는다」는 해 봤고 안 된다**(2026-09-11 · 사용자 요청으로 시도 → 되돌림).
  ⛔ 다시 시도하지 말 것. 왜 안 되는지가 셋이다:
  1. **속도는 방향을 안 바꾼다.** 절벽으로 향하는 이유는 웨이포인트를 무시하는 조타·밀림이고,
     그 힘들은 **이동 비용을 안 본다**(흐름장은 막힌 칸을 아예 「못 간다」로 보므로 비교할 비용이 없다).
     느려지면 **늦게 넘을 뿐 덜 넘지 않는다**.
  2. **깎을 시간이 없다.** 유닛이 막힌 칸을 밟는 것은 판당 중앙 **17 유닛프레임**(전체의 0.5%)뿐이다 —
     테두리가 한 칸이라 1~2 프레임에 통과한다.
  3. **띠를 넓히면 가야 할 길이 같이 느려진다.** 오르막 입구는 절벽 테두리에 둘러싸여 있다.

  | 300판 · 같은 자 | 끔 | 배수 0.20 | 배수 0.20 + 띠 1칸 | + 띠 2칸 |
  |---|---|---|---|---|
  | 절벽 통과 | 36.9% | 34.6% | 32.5% | **40.3%** |
  | 등반 시간(중앙) | 21.2초 | 21.6초 | 24.1초 | **28.0초** |
  | 고원 도달 | 300/300 | 300/300 | 300/300 | 300/300 |
  ⚠ 36.9 ↔ 34.6 은 **잡음이다**(n≈950 · 오차 ±1.5%p · 1.5σ). 세기 다섯을 훑어도 순서가 안 나왔다
  (1.0/0.5/0.35/0.2/0.1 → 29.0/31.1/35.6/30.0/26.5%). **얻는 것 없이 느려지기만 해서 코드를 되돌렸다.**
- 🔍 **넘긴 것이 누구냐 — 갈라 쟀다**(2026-09-11 · `camp-terr-climb.mjs` 다섯째 인자 · 300판 567회).
  유닛을 움직이는 것은 둘뿐이다(`campMove` · 프레임 끝의 `strikeSeparate`). 프레임마다
  p0 → p1(campMove 뒤) → p2(밀어내기 뒤) 를 잡아 어느 구간에서 넘었는지 셌다.

  | | 몫 |
  |---|---|
  | ① `campMove`(이동 명령)가 넘겼다 | **86.4%** |
  | ② `strikeSeparate`(겹침 밀어내기) | 12.5% |
  | ③ 기타 | 1.1% |
  | ①을 다시 가르면 — 우회점을 아예 안 줬다 | 48.4% (그중 목표가 고원 위 59%) |
  | ①을 다시 가르면 — **우회점도 선도 멀쩡한데 넘었다** | **51.6%** |
  | 그때 명령 방향 ↔ 실제 이동 각도 | 평균 **38.6°** (45° 초과 83/253) |
  | `best<0` 안전망이 막힌 우회점을 준 것 | **0%** |

- ⭐⭐ **결론: 남은 원인은 판정이 아니라 「조타 이탈」이다.** 지형층은 제 일을 하고 있다 —
  우회점을 주고, 그 선도 뚫려 있다. 그런데 `stepUnitMove` 의 회피·조향이 **명령 방향에서 평균 38.6°**
  틀어 놓고, 그 어긋남이 한 칸짜리 테두리를 넘기기에 충분하다.
  ⛔ **선 판정을 더 손보는 것으로는 못 줄인다**(안전망 몫이 0% 다 — 거기엔 고칠 것이 없다).
  ⛔ **밀어내기·속도 벌칙도 아니다**(위 두 항목 참고 — 둘 다 해 봤거나 원인이 아니다).
- ✅ **「우회점을 가깝게」는 먹혔다**(2026-09-11 · `TERR_WAY_NEAR` = **1칸**). 옛 끈 당기기는
  「보이는 **가장 먼** 칸」을 줬는데, 목표가 멀수록 조타가 벌어질 여지가 커진다.

  | 800판 · 같은 자 | 상한 없음(옛) | 6칸 | 3칸 | 2칸 | **1칸(지금)** |
  |---|---|---|---|---|---|
  | 절벽 통과 | 33.0% | 32.4% | 28.1% | 25.9% | **23.1%** |
  | 등반 시간(중앙) | 21.2초 | 21.2초 | 21.1초 | 21.1초 | **21.2초** |
  | 고원 도달 | 800/800 | 800/800 | 800/800 | 800/800 | **800/800** |

  🎬 **움직임도 좋아졌다**(`camp-trace.mjs` · RUNS=5): 방향 뒤집힘 9.4 → **6.6회/유닛** ·
  사거리 안 비율 14.1% → **18.9%** · 준 피해 758 → 771. ⚠ 「가까우면 계단식이 되어 느려진다」를
  걱정했는데 **반대였다** — 궤적이 오히려 매끄럽다(`docs/mock/trace-near1/`).
  ⛔ 상한을 6칸 이상으로 되돌리지 말 것(6칸은 옛 동작과 차이가 없다 — 효과는 **아주 조일 때만** 난다).
  ⚠ 스모크는 **6칸**을 글자 그대로 잰다(막힌 칸에 밀린 유닛은 주변 3칸에서 다시 잡으므로 5칸대가
  정상이다 · 옛 동작은 28칸까지 갔다 — 주입 시험에서 69/131 이 걸렸다).

- 🧾 **남은 절벽 통과 23%** 는 여전히 조타 이탈이고, 더 손대려면 `stepUnitMove` 의 조향에 들어가야 한다
  (⛔ 18-strike 는 고치지 않는다). 지금은 여기서 멈춘다.
  ⚠ 남은 「안 막혔다고 봄 30.5%」는 **판정이 틀려서가 아닐 수 있다** — 이동 목표는 저지인데
  유닛끼리 밀어내는 힘에 떠밀려 넘는 경우가 섞인다(넘는 순간 목표가 저지인 것이 129/400).
- ⚠⚠ **전장 폭은 `CAMPB.world` 다.** 이걸 틀리면 `campW2G` 가 gx 2505 를 뱉고 **모든 수치가 조용히
  거짓이 된다** — 첫 실행에서 「고원 도달 1/6 · 오르막 0/6」이라는 가짜 실패를 봤다.
- ⚠ **표본을 작게 잡지 말 것.** 6판일 때는 「오르막을 밟은 판 5/6」만 보여 표집 누락으로 넘겼는데,
  300판으로 늘리고 **넘은 자리의 열**을 재고서야 절벽 통과가 드러났다.

🎬 **손댔으면 `node scripts/camp-terr-shot.mjs`(그림) · `node scripts/camp-terr-climb.mjs`(등반) 로 볼 것** — 칸이 맞물리나 / 절벽이 높이로 읽히나가
전부라 숫자로는 안 보인다.

### 💠 룬 — 젬으로 사서 끼우는 성장 축 (`js/22-camp-rune.js` · 2026-09-02)

**연구·환생 트리와 다른 점은 「칸이 한정」이라는 것 하나다.** 사 모아도 동시에 켜지는 것은
정해진 수뿐이라, 성장이 아니라 **선택**이 축이다. 값·방향은 `GEM.md` §8 이 단일 소스.

| | |
|---|---|
| 칸을 여는 것 | 🏆 **통산 최고 레벨**(`campRuneBestLv()` → `campBestLevel()` · 2026-09-11) — 표는 `RUNE_SLOT_LV` |
| 칸이 사는 곳 | `C.rune`(=`p.camp.rune`) — ⛔ 환생이 지우지 않는다(젬으로 산 것이다) |
| 사는 재화 | **젬만**(`campRuneBuy`) |
| 갈래 | 일반(하·중·상 세 등급) · **유니크**(등급 하나 · 칸 3개 · 다른 룬에 없는 능력) |
| 효과 입구 | **`campRuneEff(key)` 하나** — ⛔ **합이다**(GEM.md §5-2) |
| 화면 | `#campRune` — 환생 화면과 **같은 규격**(`#phone` 직속 · z-index 120 · `bottom:var(--navH)`) |
| 입구 | **`campRuneEnter(sec)` 하나** — ⛔ `campRuneOpen` 을 직접 부르면 네비 하위가 안 나온다 |

⭐ **종류가 칸보다 많다** — 일반 9종/5칸 · 유니크 4종/3칸. ⛔ 종류 = 칸 이면 전부 끼워져
고를 것이 없어진다(그러면 이것은 그냥 「연구 2」다). 값 표는 `GEM.md` §8-3.

🔌 **효과가 닿는 곳은 룬마다 한 자리뿐이다** — 표는 `GEM.md` §8-5. ⛔ 한 효과를 두 곳에
걸면 화면의 표기(+20%)가 곧바로 거짓말이 된다.
특히 **가속의 룬은 `campFrame` 의 `dt` 한 줄**(`campDtMul()`)이 전부다 — 일꾼·건설·전투·
정제소가 모두 그 dt 를 타므로, 수입 공식에 또 곱하면 두 겹이 된다.
⚠ **신속의 룬만 게이트가 필요하다**(`campRuneMulIn`) — `16-build.js` 는 관리자 건설 탭·
오토배틀과 **공유**라, 캠프 밖에서도 걸리면 관리자 탭 일꾼까지 빨라진다.

🏆 **왜 레벨인가**(2026-09-11): 라운드는 없어졌고, 통산 관문은 **던전 셋 = 18 이 끝**이라
한 바퀴만 돌면 27칸이 전부 열려 버렸다 — 룬은 긴 축인데 자가 짧았다. 레벨은 환생할수록
더 높이 오르므로 회차를 거듭할수록 열린다.
⚠ 다만 레벨 자체(`C.lv`)는 **회차마다 1 로 되감긴다**. 💠 룬은 젬으로 산 물건이라 한 번 열린
칸이 닫히면 결제가 사라지는 것이다 → 자는 **되감기지 않는 `C.lvBest`** 다(`campBestLevel`).
⛔ `campLevel()`(지금 레벨)으로 칸을 열지 말 것. ⛔ `campRunReset` 이 `lvBest` 를 지우지 말 것
(그리고 `campRebirth`·`campTutoReset` 의 keep 목록에도 있어야 한다 — `campWipeBoard` 가
저장을 다시 읽으므로). ⚠ 문턱 값(`RUNE_SLOT_LV`)은 **아직 안 쟀다** — 실측은 「환생 없이
던전 3 완주 = Lv.18」 하나뿐이고, 거기 기대어 「첫 완주 = 27칸 중 18칸」으로 놓았다.

⛔ **전제 넷 중 하나라도 무너지면 젬이 곧 지수 축이 된다.** ① 칸은 **통산 최고 레벨**이 연다(젬이 아니다)
② 칸이 한정이다 ③ 효과는 합이다 ④ 산 것은 환생해도 남는다. 스모크 넷이 이걸 잠근다.
...
#### 📈 레벨 — 판 안의 성장 축 (2026-09-11 · 옛 1차 환생을 대체)

| | 어디 |
|---|---|
| 상태 | `C.lv`(레벨) · `C.xp`(그 레벨의 누적) · `C.lvPts`(**안 쓴** 포인트) · 🏆 `C.lvBest`(**통산 최고**) — 넷 다 `js/19-camp.js` |
| 곡선 | `campXpNeed(lv)` = `CAMP_XP_A × CAMP_XP_R^(lv-1)` (등비) · 한 마리 값 `campKillXp()` = `CAMP_XP_KILL × campFoeDiff(지금)` |
| 쌓기 | `campAddXp(n)` — 레벨업까지 한 번에 하고 오른 수만큼 `C.lvPts` 에 얹는다 |
| **지급 지점** | **`js/21-camp-battle.js` 의 죽은 유닛 정리 한 곳**(`campStepUnits` 끝 · `side === 'ai'` 일 때만) |
| 쓰는 곳 | 성장 트리 — `campRtPts()` 가 `campLvPtsLeft()` 를 읽는다(⛔ `C.rbPts` 는 옛 환생 지갑이라 아무도 입금하지 않는다) |
| 쓰는 곳 ② | 💠 **룬 칸 해금** — `campRuneBestLv()` 가 `campBestLevel()`(= `C.lvBest`)을 읽는다 |
| 되감기 | `campRunReset` 이 `lv`·`xp`·`lvPts`·`rbTree` 를 함께 지운다 — **환생의 keep 목록에 트리가 없다** |
| ⛔ 되감지 않는 것 | 🏆 `C.lvBest` **하나**(`campBestLevel`) — 💠 룬 칸을 여는 자다. 되감으면 **젬으로 산 룬이 들어갈 자리가 닫힌다**. `campRunReset` 이 안 지우고, `campRebirth`·`campTutoReset` 의 keep 목록에도 들어 있다(`campWipeBoard` 가 저장을 다시 읽으므로 둘 다 필요하다) |
| 화면 | 재화 바 맨 왼쪽(`curPaintLv` · `js/12-appshell.js`) |

⭐ **왜 지급 지점이 한 곳인가** — 사격·광역·스킬·지뢰·핵·건물 사격이 **전부 그 정리를 지난다.**
킬 지점마다 붙이면 반드시 하나를 빠뜨린다(실제로 스킬 구역 둘은 `me.kills` 도 안 세고 있었다).

⚠ **`campRunReset` 을 살아 있는 캠프에 대고 부르지 말 것**(테스트 포함) — 판까지 비운다(광맥·건물·엔트리).
스모크에서 그렇게 쟀다가 뒤따르는 스텝 다섯이 「광맥이 안 깔림」으로 무너졌다(2026-09-11).
검사에는 **가짜 상태 객체**를 넘긴다.

#### 🔁 환생 — 하나뿐이고 자가 「도달 깊이」다 (2026-09-11)

| | 어디 |
|---|---|
| 관문 | `campCanRebirth()` = `campRebDepth() >= campRebNeed()` — **마지막 던전 클리어**(= 무한층 진입) |
| 깊이 | `campRebDepth()` = 통산 관문 수(던전 하나 = `CAMP_DG_STEPS`) · `C.dgDone` 도 함께 본다 |
| 포인트 | `campRebPtGain()` = 깊이 × `CAMP_REB_PT_GATE` × 팩 |
| 배수 | `campRebMulGain()` = `CAMP_REB_MUL_STEP`(1) × 팩 → `C.rebMul` 에 **더한다** · `campRebMul()` = 1 + 그 값 |
| 되감김 | `campRunReset` — 진행 · 레벨 · 성장 트리 · 캠프 경제. 남는 것은 배수 · `C.rbPts` · 💠 룬 · `C.best` |

⛔ **「번 돈」(`campWealth`)을 관문·포인트의 자로 되돌리지 말 것** — 돈→포인트→배수→돈 되먹임이 그거였다.
`campWealth` 자체는 남아 있지만 이제 **벤치의 자**일 뿐이다(`scripts/camp-bench.mjs` · `test/dead-known.json`).
⛔ 배수를 난이도 로그로 되돌리지 말 것 — `CAMP_REB_K`·`CAMP_RP_BASE`·`CAMP_RP_DG` 는 없앴다.
🚧 **환생 포인트를 쓰는 곳이 아직 없다** — 화면(`.crFx`)이 「준비 중」이라고 말한다.
♾ **무한층이 그 구간을 만들었다**(2026-09-11) — 아래 절.

#### ♾ 무한층 — 던전 셋 위의 등반 (2026-09-11 · 단계 3)

| | 어디 |
|---|---|
| 층 번호 | `campInfN(dg)` = `dg − CAMP_DG_MAX`(던전이면 0) · 빌려 쓰는 던전 `campInfBase(dg)` — 둘 다 `js/23-camp-dungeon.js` |
| 표 | `campInfDef(dg)` — **`CAMP_DG` 를 그대로 빌린다**(종족 순환) · 이름만 「무한 n층」 · ⛔ 무한층 전용 건물 표를 만들지 말 것 |
| 난이도 | `campFoeDiff` 가 그대로 이어진다 — 한 층 = 던전 하나(관문 여섯의 곱 ×7.17) · ⛔ 무한층 전용 배율을 두지 말 것 |
| 보상 | `campMineDef(dg)`(19-camp) — `CAMP_MINE` 밖이면 마지막 두 칸의 비(×5.83)로 외삽 · **난이도(×7.17)보다 느리다 = 그게 벽이다** |
| 한 층을 깨면 | 🧗 **곧바로 다음 층**(`campInfClear` → `campEnterDungeon(dg+1)`) — ⛔ `campFail()` 을 부르지 말 것(등반이 끊긴다) |
| 기록 | `C.infBest`(`campInfBest()`) — **되감기지 않는다**(환생·튜토리얼 keep 목록) |
| 보상 지갑 | 🪙 `PLAYER_META.coins`(🗺 유즈맵 강화가 쓰는 그 지갑) — ⛔ 새 재화를 만들지 말 것 |
| 잠금 | `campDgOpen(dg>CAMP_DG_MAX)` = `C.dgDone[CAMP_DG_MAX]` — **환생 관문과 같은 문** |
| 화면 | 던전 선택의 **줄 하나**(`.cdRow.inf` · 늘 무한 1층부터) · 칩은 `campDgName` 으로 「무한 7층 · 2/6」 |

⭐ **왜 캠프로 안 돌아오나** — 던전 1~3 은 커리큘럼이라 집에 들러 전리품을 쓰고 다시 온다.
무한층은 **등반**이라 「어디까지 버티나」가 전부다. 그리고 그래야 환생의 물음
「지금 끊을까, 한 층 더 갈까」가 생긴다(환생 포인트 = 도달 깊이 = `campRebDepth`).

⚠ **값 둘은 안 쟀다**: `CAMP_INF_COIN`(층당 코인 12) · 보상 외삽의 비(×5.83).

#### 🔁 환생 강화 — 환생 포인트로 사는 것 (2026-09-11)

「세지는 것」은 안 판다(그건 레벨·성장 트리의 몫) — **빨라지고 편해지는 것**만이다.

| | 어디 |
|---|---|
| 표 | `CAMP_REB_UPG`(19-camp.js) · 갈래는 `CAMP_REB_UPG_GRP`(스킵 · 자동화 · 상한 해제) |
| 상태 | `C.rbUpg = {키:레벨}` — **영구다**(`campRunReset` 이 안 지우고, 환생·튜토리얼 keep 목록에 있다) |
| 값 | `campRebUpgCost(k, lv)` = `c0 × cr^레벨` · 지갑은 `C.rbPts` |
| 화면 | `#rebUpgScreen` · `campRebEnter('boost')` · 네비 「환생 › 환생 강화」 |
| 소비처 | `campRebSkipApply`(던전 개방) · `campWorkerMax`(일꾼 상한) · `campUnitRate`(재구매 배수) · `campAutoTapTick`(자동 채굴) · `campAwayCapS`(자리 비움 상한) · `campAutoUpgTick`(자동 강화) · 던전 완주 분기(자동 원정 · 19-camp `campCombatStep`) |
| 켜고 끄는 줄 | `onoff:true`(자동 강화 · 자동 원정) — `max:1` 이고 줄의 힌트가 값이 아니라 **「켜짐」**이다 |

⭐ **화면도 줄 목록도 유즈맵 강화 것을 그대로 쓴다** — CSS 는 `#mapUpgScreen` 선택자에 얹었고,
줄은 `ptRowsHTML(pred, src)`(09-usemap-base.js) 한 함수가 만든다. 여기서 주는 것은 **출처**(`_rebUpgSrc`)뿐이다.
⛔ 두 번째 줄 렌더러·전용 CSS 블록을 만들지 말 것.

🚨 **`dgStart.max` 는 `CAMP_DG_MAX − 1` 이 상한이다.** 마지막 던전까지 열어 주면 환생 직후부터
관문(`campRebDepth ≥ campRebNeed`)이 참이라 **누를 때마다 배수가 +1 씩 붙는 무한 환생**이 된다.
같은 이유로 `campRunReset` 이 `C.dgDone` 을 **반드시** 비운다(스모크가 둘 다 잰다).

⛔ **성장 트리와 같은 효과를 팔지 말 것**(인구 상한·시작 미네랄·시작 일꾼은 트리에 있다) —
한 효과가 두 곳에 있으면 어느 표기도 진실이 아니다. 스모크가 키 겹침을 잰다.

🚫 **§0-A 의 항목 중 일부러 안 넣은 것 셋**(2026-09-11 · 「나중에」가 아니라 **넣으면 안 되는 것**이다):

| 안 넣은 것 | 왜 |
|---|---|
| **보급소 상한** | 엔진이 인구를 `TECH_SUP_MAX`(200 · 16-build.js)에서 자른다 — 보급소를 24채보다 더 지어도 인구가 안 는다. 그 벽을 넘는 길은 **트리의 「인구 상한」**(`campSupAdd` 가 그 위에 더한다) 하나뿐이라, 줄을 세우면 같은 효과를 두 곳에서 파는 것이 된다 |
| **오프라인 수익률** | 트리의 「방치 수급」(`idle` → `campRtMul('idle')`)이 이미 그 배수다. ⭐ 대신 **상한**(`awayCap` → `campAwayCapS`)을 판다 — 배수가 아니라 「몇 시간까지 쌓이나」라 축이 안 겹친다 |
| **시작 자원 · 시작 일꾼** | 트리에 있다 |

⏳ **남은 하나는 「시작 구성 바꾸기」**(병영 대신 기갑공장부터)인데 **닿는 데가 없다** —
캠프의 시작 건물을 정하는 자리가 `campWipeBoard` 한 줄이라 「무엇부터」라는 선택지 자체가 없다.
⛔ 그 줄을 만들기 전에 표에 올리지 말 것(포인트를 받으면서 말을 안 하면 거짓 판매다).
⚠ **아직 넷뿐이다** — §0-A 가 말한 나머지(시작 구성·자동 건설/연구/던전·오프라인 수익률)는 표에 줄을 더한다.
⚠ 값(`max`·`c0`·`cr`)은 **안 쟀다**.

#### 🌳 성장 트리 — 차수가 **1** 이다 (2026-09-11 · 옛 환생 트리)

레벨 포인트로 산다. 한 회차 예산이 **17점**이라(위 실측) 옛 규격(161노드 · 20티어 · 최고 5.5조)이
통째로 안 맞았다. **계열 32개는 그대로 두고 차수만 없앴다** — 노드 43개 · 트리 전체 83점 · 예산의 20%.

| | 값 |
|---|---|
| 차수 | `CAMP_RT_MAX_DEF = 1` — 계열의 `mx` 도 이 상한에 눌린다(`campRtMax`) |
| 계열 값 | `campRtCost` = **등급값 하나**(흔함 1 · 보통 2 · 귀함 4) · ⛔ 티어 지수·손값(`cs`)은 안 쓴다 |
| 마디 값 | 가운데 1 · 갈래 2 · 묶음 2 |
| 효과 | 사다리의 **[1] 칸만** 쓰인다(`CAMP_RT_LADDER`·`CAMP_RT_CUT`·`CAMP_RT_DISC`·`CAMP_RT_SUP`) |

⛔ 차수를 5 로 되돌리지 말 것 — 트리 전체 비용이 예산의 수십억 배로 돌아간다.
⚠ `campTreeState` 는 **차수 상한 밖이면 null** 이다(안 넣으면 없는 2차가 「살 수 있다」로 뜬다).
⚠ 효과값 넷은 **안 쟀다**(BALANCE §5-11) — 옛 5차를 한 칸으로 누른 출발점이다.

#### 🧭 자원 줄의 배치 — 본부는 위, 가스는 광맥 양 끝 (2026-09-02)

세로로 **본부(0.59) → 광맥·가스(0.67)** 순이다. 가스는 본부 양옆이었는데 광맥 줄로 내려왔다.

⚠ **가스 자리는 광맥 줄의 「시각 끝」에서 잰다**(`campMineHalfW()`), 격자 중심에서 몇 칸이 아니다.
칸 수·간격·크기를 바꿔도 저절로 따라온다. ⛔ 옛 `CAMP_GAS_GAP`(중심 기준) 으로 되돌리지 말 것.

⚠ **「일직선」은 상자가 아니라 발치로 맞춘다.** 광맥 스프라이트는 상자를 0.30칸 위로 밀어 발이
`m.y` 에 닿게 하고(`16-build.js` `_dy`), 가스는 `align-items:flex-end` 라 **부지 밑변이 곧 발치**다.
그래서 가스 부지 밑변을 `광맥 y + 0.45칸`(`CAMP_GAS_FOOT`)에 맞춘다.

⛔ **부지 행(`r0`)은 정수여야 한다** — 정제소 배치 검사가 `s.r0===TECH_GAS.r0` 로 딱 비교하고
고스트의 `r0` 는 격자에 스냅된 정수다. 그런데 광맥 발치는 마침 두 행의 **가운데**에 떨어진다.
그래서 가까운 행에 부지를 놓고, 남는 반 칸은 **CSS `--gzDY` 가 그림만 내려서** 메운다
(제 높이 기준 % 라 줌을 따라간다 — ⛔ px 로 박지 말 것). 실측 발치차 6px → 0.1px.

⚠ 가스와 광맥은 **살짝 겹쳐야** 붙어 보인다(실측 좌우 −7.5px). 그림이 부지 안쪽으로 물러나 있어서다
(`object-fit:contain` + `scale(.95)`), 부지끼리 딱 붙이면 그림 사이에 틈이 남는다 — `CAMP_GAS_PAD` 가 음수인 이유.

⚠ **가스를 옮기면 카메라가 따라 움직인다** — `campCalcViewBot()` 이 가장 아래 요소(광맥·가스)에서
화면의 아래 끝을 잰다. 한 행 옮겼더니 화면 전체가 10px 내려갔다. 배치를 바꿨으면 하단 시트에
물리지 않는지 다시 볼 것.

#### ⛏ 광맥 8덩이 × 일꾼 5명 = 일꾼 상한 40 (2026-09-02)

**세 숫자가 한 줄에 묶여 있다.** 하나만 바꾸면 일꾼이 남거나 광맥 자리가 논다.

| 숫자 | 어디 | 값 |
|---|---|---|
| 광맥 덩이 수 | `CAMP_MINE_COLS` (`19-camp.js`) | 8 |
| 덩이당 붙는 일꾼 | `CAMP_MINE_CAP` (`19-camp.js`) → `res.cap` | 5 |
| 일꾼 상한 | `CAMP_WORKER_MAX` (`19-camp.js`) | 40 |

⭐ 스모크가 `8 × 5 === 40` 을 직접 검사한다 — 셋 중 하나를 고치면 거기서 걸린다.

⚠ **cap 은 2026-09-02 부터 경성이다**(사용자 확정). `_techMinerFull`(`16-build.js`)이 cap 명에서
막는다. 옛 규칙은 **안 막고** 초과분마다 왕복 +5% 였다(`_techMinerSlow` — 지금은 초과가 안 생겨 항상 1.0).
⛔ 연성으로 되돌리면 「미네랄 한 개당 최대 5마리」가 뜻을 잃는다.
⚠ `res.cap` 표식이 **없으면 1** 이라 관리자 건설 탭·오토배틀은 기존 단일 락 그대로다(`inf` 와 같은 수법).

⛔ **환생 연구 `wkCap`(일꾼 상한 +5 → +250)을 배선할 때 광맥 수를 같이 늘려야 한다.**
안 그러면 늘어난 일꾼이 붙을 자리가 없어 광맥 옆에 서서 논다(막힌 일꾼은 크래시가 아니라
채취 지점 옆에 겹쳐 대기한다). 지금은 `workerCap` 을 읽는 코드가 없어 아직 문제가 아니다.

⚠ **광맥 줄의 가로 중심은 `campMineCol() + (COLS-1)/2` 다** — 덩이가 칸을 채우는 게 아니라
칸에 **점으로** 앉기 때문이다(반 칸 차이). 가스 좌우 대칭이 여기에 걸려 있다.
⛔ 반올림하지 말 것 — 짝수 칸에서 반 칸 밀린다(옛 식이 그랬고, 홀수 7칸에서 반올림이 우연히 가려 줬다).

#### 🏢 적이 칠 건물 — 부서지면 **그 자리에서** 다음 것으로 (2026-08-31)

⛔ 프레임 **처음에 한 번만** 골랐다. 그 건물이 프레임 중간에 부서지면 뒤에 오는 적들이
**이미 죽은 건물**을 계속 때려 그만큼의 피해가 버려졌다(실측: 같은 프레임에 2발 → 1발).
⭐ `nextBld()` 가 「살아 있나」를 확인하고, 죽었으면 `campFrontBld()` 로 다시 고른다.
⚠ 재는 것은 **발사 수**다 — 죽은 건물의 체력은 어차피 0 으로 잘려 눈에 안 보인다.
⚠ 유닛마다 최근접을 고르게 하지 말 것 — 무리가 갈라져 건물 여럿을 동시에 갉는다.
규칙은 「앞에서부터 차례로」(`campFrontBld` = y 가 가장 작은 것)다.

#### 🧹 캠프를 나갈 때 패치를 **전부** 되돌린다 (2026-08-31)

캠프는 진입할 때 공용 함수 10개를 감싼다(가격·격자·줌·생산·배치·표적·연구 …). `campHideView` 가
전부 되돌리는데 **`campUnpatchResearch` 하나만 배선이 빠져 있었다** — `techPanelRender` ·
`renderCampIdleSheet` 래퍼가 영영 남았다.
⚠ 지금은 래퍼가 `campIsOn()` 으로 스스로 빠져서 무해했지만, 그 가드를 지우는 순간
**관리자 건설 탭이 캠프 연구 카드로 오염된다**(TECH_TREE·`techPanelRender` 는 공유다).
⭐ 스모크가 `campHideView` 소스를 훑어 「정의돼 있는데 안 돌려주는 패치」를 잡는다.

#### 🛟 프레임 예외 복구 — 한 번의 예외로 화면이 굳지 않는다 (2026-08-31)

⛔ `campFrame` 은 재예약(`requestAnimationFrame`)이 `try` **밖 아래**에 있었다. 안에서 예외가
한 번 나면 그 줄에 도달을 못 해 **캠프가 그대로 굳는다.** 그런데 250ms 타이머는 계속 돌아
재화 바 숫자만 올라가서 **살아 있는 것처럼 보였다** — 멈춘 것보다 나쁜 상태다.

⚠ **예외를 삼키지 않는다.** `setTimeout(()=>{throw err})` 로 다시 던져 `window.onerror` ·
스모크 `pageerror` 가 그대로 본다. ⛔ 매 프레임 던지면 콘솔이 넘치므로 **연속 실패의 첫 번째만**
알린다. 연속 `CAMP_ERR_GIVEUP`(120 · 약 4초) 회면 루프를 접고 안내를 띄운다.
⚠ 스모크는 `window.__campErrQuiet` 로 그 되던짐만 막는다(`_campTapForce` 와 같은 시험용 문).
⚠ 유즈맵 메인 `loop()`(`js/14-input-fx.js`)도 **같은 구조**다 — 캠프만의 문제가 아니었다.
거기는 아직 안 고쳤다.

#### 🌱 캠프 밖에서도 돈다 — 화면을 떠나도 멈추지 않는다 (2026-09-03)

방치형이므로 **유즈맵 선택·상점·정비에 있는 동안에도 일꾼이 계속 왕복하고 재화 바가 오른다.**
예전엔 `campExit` 이 타이머·프레임을 끄고 나간 시각만 남겨, 돌아올 때 한 번에 정산했다.

| | 예전 | 지금 |
|---|---|---|
| 화면을 떠날 때 | 타이머·프레임 정지 + `leftAt` 기록 | **아무것도 안 멈춘다** · `leftAt` 안 찍는다 |
| 밖에 있는 동안 | 수입 0 (돌아올 때 정산) | **실시간으로 번다** |
| 진짜로 멈출 때 | — | 앱 숨김(`campOnHide`) · 유즈맵 게임/오토배틀(250ms 타이머가 찍는다) |

⛔ **`leftAt` 을 화면 전환에서 찍지 말 것** — 밖에서 번 것 위에 자리 비움 몫이 얹혀 **이중 지급**된다.

##### ⏱ 고정 걸음 — 밖에서도 같은 크기로 여러 번 걷는다
`campFrame` 은 밖에서 **250ms 간격**으로 돈다(`CAMP_BG_FRAME_MS` · 안 보이는 것을 60fps 로 그릴 이유가 없다).
그런데 `dt` 를 `Math.min(0.05, 경과)` 로 자르고 있어서 **250ms 중 50ms 만 흐르고 200ms 는 사라졌다.**

⭐ 그래서 `CAMP_STEP_DT`(= 캠프 화면의 걸음, 30ms) 로 **여러 번 걷는다.** 자투리는 `_campAcc` 로 다음 프레임에 넘긴다.
⛔ 「큰 `dt` 한 번」으로 대신하지 말 것 — 도착·인도 판정이 「남은 거리 < 속도×dt」 라서
**큰 걸음일수록 왕복이 빨리 끝난다**(실측: dt 0.03 → 0.05 만으로 분당 960 → 1320).

##### 💠 가속의 룬도 걸음 **수**로 건다
`campDtMul()`(가속의 룬)을 `dt` 에 곱하면 걸음이 커져 **배수보다 더 벌게 된다**(위 이유와 같다).
그래서 누적(`_campAcc`)에 곱해 **걸음 수를 늘린다.** 따라잡기 상한도 배수만큼 함께 늘린다 —
안 늘리면 룬을 켠 채 캠프 밖에 있을 때 상한에 걸려 손해다.

📐 실측(2026-09-03): ×2 → 1.92~2.00배 · ×3 → 2.92배 (캠프 안·밖 모두)

##### 📐 좌표계는 세션 것이다 — `campPatchWorld()`
캠프는 격자를 촘촘하게(`CAMP_COLS`) 깔고 셀 높이를 맵의 종횡비(`_btRect`)로 정한다.
⛔ 이 둘은 `campUnpatchZoom` 과 **함께 풀면 안 된다.** 캠프 밖에서는 맵이 없어 `_btRect` 가 0×0 이 되고
열 수도 되돌아가 **셀이 2배로 부푼다** → 본진 발판이 커져 왕복이 짧아지고 **밖에서 더 많이 번다.**

📐 실측(2026-09-03 · 일꾼 4명 · 분당 미네랄):

| | 캠프 | 유즈맵 선택 | 상점 |
|---|---|---|---|
| 고치기 전 | 928 | **1376 (148%)** | 1376 |
| 고친 뒤 | 960 | 992 (103%) | 992 |

⚠ 이동량으로 갈랐다 — 밖에서는 왕복이 거의 없고(초당 0.009) 채취만 83% 였다. 캠프는 0.048 · 61%.
⭐ 그래서 `campPatchWorld()` 는 **한 번 걸면 풀지 않는다.** 대신 `_campSim`(campFrame 의 시뮬레이션 구간)
이 꺼져 있으면 **원본을 그대로 부른다** — ⛔ `campEcoOn()` 만으로 갈라서는 안 된다.
그러면 캠프 세션이 살아 있는 채로 관리자 건설 탭·오토배틀에 들어갔을 때 **그 화면이 캠프 격자로 그려진다.**

⚠ 스모크 「캠프: 화면을 떠나도 멈추지 않는다」가 넷을 다 잡는다(정지 / `leftAt` / 좌표계 / 남의 화면).
...
「때릴 수 있나」(`campCanHitFoes`)는 의무병을 뺐다. 그래서 **전투 유닛이 다 눕고 의무병만 서 있으면
본부가 멀쩡한데도 즉시 탈락**했다 — 문서에 못 박은 「패배 = 본부 파괴 하나뿐」을 정면으로 어긴다.
재현: 본부 750/750 · 적 1 · 의무병 1 → **한 프레임 만에 던전 0**. 게다가 뜨는 말이
「✈ 공중을 칠 수 없어 탈락」이라 원인을 가렸다.

⭐ 판정을 **`campArmedUnits()` 한 곳**으로 모았다 — 살아 있고 · 공격력이 있고 · 때릴 수 있는
레이어가 하나라도 있는 아군. `campCanHitFoes` 와 승패 가드가 같은 자를 본다.
⚠ 스모크 계약을 넣을 때 **가드를 새 함수 이름으로 걸지 말 것** — 옛 코드에 그 함수가 없어
검사가 통째로 건너뛰어진다(그렇게 한 번 헛통과했다). 양쪽에 다 있는 것으로 건다.

#### 🎬 전투 **움직임**을 보는 자[尺] — `scripts/camp-trace.mjs` (2026-08-31)

`camp-bench.mjs` 는 「몇 분에 어디까지」를 잰다. **움직임은 안 보여 준다** — 그래서 이 프로젝트는
전투 이동을 숫자로만 좇다가 네 번 헛짚고 전부 되돌렸다(닿으면 정지 / 뒤로 안 가기 / 목표 안쪽으로 /
전선 넓게). `camp-trace.mjs` 는 실제 `campCombatStep` 을 돌리면서 **매 프레임 전 유닛 좌표**를
받아 적고 ① 궤적 그림 ② 확대(가장 많이 뒤집힌 유닛) ③ 스냅샷 격자 ④ 수치를 함께 낸다.

```
CHROME_PATH=/opt/pw-browsers/chromium node scripts/camp-trace.mjs [초] [던전] [부대]
  ENGAGE=0  campEngageStep 을 끄고 잰다(대조군 — 「캠프가 손을 떼면」)
  RES=n     계열 업그레이드 레벨(기본 45)   RUNS=n  반복(기본 3 · 중앙값)
```

⚠ **`RES` 를 0 으로 두고 「전투가 안 된다」고 읽지 말 것.** 맨몸 마린은 체력 5 · 공격 1 인데
던전 1 라운드 1 의 적은 체력 **392** 다(실측). 이동을 어떻게 고쳐도 못 이긴다 — 연구로 키우라는
설계이지 움직임의 문제가 아니다. 30분 벤치의 실제 판은 계열 업그레이드 **91레벨**이었다.
⚠ 판마다 크게 흔들린다(같은 코드가 사거리 안 13~39%). **한 번 재고 결론 내지 말 것** — 중앙값으로 본다.

#### 🏰 던전 = 적 기지 — 릴레이와 구간 (`js/23-camp-dungeon.js` · 2026-09-09)

배치는 **표 하나**(`CAMP_DG`)에서 온다. 나중에 「남의 캠프를 친다」(비동기 습격)를 넣을 때
그 사람의 배치를 같은 모양으로 넘기면 그대로 돈다 — 그래서 입력이 표다.

건물 12채 = 진행 6 + 문지기 탑 3 + 보급고 3. 진행 건물은 **차례(`step` 1~6)** 를, 모든 건물은
**구간(`zone` 1~3)** 을 갖는다.

| 규칙 | 함수 | 요지 |
|---|---|---|
| 🔗 릴레이 | `campFoeActive()` | 살아 있는 진행 건물 중 **step 이 가장 작은 한 채**만 적을 뽑는다 — **그 건물 자리에서** |
| 압박 곡선 | `campFoeSpawnTick(dt)` | 단계 표(`CAMP_FOE_RELAY_S`/`_N`)로 주기는 짧아지고 마리는 는다. ⛔ 「살아 있는 건물 수로 나눈다」 폐기 |
| 커리큘럼 | `CAMP_DG[].bld[].foe` | 건물마다 뽑는 유닛이 다르다 → `campFoeId(prefer)`. 걸러서 비면 옛 티어 표로 되돌아간다 |
| 🚪 관문 | `campFoeZoneOpen(z)` | 그 구간의 **문지기 탑이 살아 있으면 그 구간의 진행 건물을 못 때린다**(`campFoeCanTarget`) |
| 🔌 D3 전력 | `campFoeTowerLive(z)` | 파일런은 **제 구간의** 탑만 먹인다. ⛔ 기지 전체로 보면 파일런 하나에 문 셋이 열린다 |
| ⚡ 보급고 | `campDepotMul()` | 깨면 30초 자원 ×2. ⛔ `campCommonMul` 에 넣지 말 것(피버와 같은 이유 — 대표 숫자가 튄다) |
| 🎯 표적 | `campFoeFront()` | 플레이어 지정 → 활성 건물(구간 열림) → 그 구간 문지기 → 폴백(앞줄). **보급고는 자동으로 안 간다** |

⚠ **스폰 큐(`CAMPB._wq`)는 숫자이거나 `{n,x,y,ids}`** 다 — 릴레이·반격은 자리를 들고 온다.
⛔ 객체만 받도록 좁히지 말 것: 옛 라운드 큐(`campSpawnFoes`)가 아직 숫자를 넣는다.

⚠ **이 절의 수치는 아직 안 쟀다.** 단계별 주기·마리·탑 체력·보급 지속 전부 출발점이다.
특히 「밀수록 어려워진다」는 벽이 되기 쉬우므로, **전리품이 성장을 따라오는지**를 먼저 재야 한다.

#### 🪧 진격 규칙 — **아군은 자리를 지킨다 · 미는 것은 손이다** (2026-09-10)

자는 **하나**다: 아군은 **자리(`_post`)에서 `campEngageOut`(1200) 안의 것만** 친다 —
적 유닛이든 **적 건물이든**. 그 밖으로는 저절로 안 나간다.

⛔ **옛 규칙을 되돌리지 말 것.** 2026-09-09~10 사이에는 「표적 건물이 있으면 거기로 간다」였고,
그 목표(`campBldGoal`)만 자리 제한을 안 탔다. 그래서 **던전에 들어가는 순간 전 병력이 맵 끝까지
자동 돌격**했다 — 언제 밀지가 플레이어의 판단이어야 하는데 그게 통째로 사라진다.

🖐 **진격은 드래그 명령**(`campMoveSel` → `u._order` → 도착하면 그 자리가 새 `_post`)이 한다.
새 자리에서 같은 규칙이 다시 돈다 → **자리 안의 것을 다 깨면 멈추고, 플레이어가 또 민다.**
실측: 던전 1 을 드래그 **12~17회 · 170~240초**에 깬다.

| | 아군(me) | 적(ai) |
|---|---|---|
| 적 유닛 | 자리에서 1200 안까지 마중 나간다 | 아군을 찾아간다 |
| 적 건물 | **자리에서 1200 안만** | 내 기지까지 **끝까지 간다**(패배 규칙의 짝) |

#### ⏳ 사격 순서와 리듬 — **적이 먼저 · 틈은 스폰이 만든다** (2026-09-10)

⚔ **적이 먼저다.** 사거리에 적이 있으면 적을 친다 — 아군이 맞는 시간을 줄이는 것이 먼저다.
⛔ 「건물에 닿은 유닛은 건물부터」로 되돌리지 말 것. 그건 적이 **틈 없이** 흘러나오던 시절의
우회였다(2026-09-09~10 · 그때는 적이 늘 사거리에 있어 본진이 60초에 이론 화력의 2% 밖에 안 깎였다).

⏳ **틈은 스폰이 만든다** — 한 번에 많이(8~18마리) · 주기는 길게(16~11초).

> **몰려온다 → 다 잡는다 → 그 틈에 벽을 친다.**

무리를 정리하고 남는 시간이 곧 공성 시간이다. 그래서 그 관문의 물음이
**「우리 화력이 무리를 제때 잡는가」**가 된다 — 못 잡으면 적이 쌓여 틈이 사라지고, 그때가 물러날 때다.

📊 실측(던전 1 · 드래그로 밀며): 적이 하나도 없는 시간의 비율
...
⭐ **화력이 셀수록 틈이 커진다** — 성장이 곧 공성 시간이라는 것이 이 리듬의 요점이다.

⚠ 상한(`CAMP_FOE_LIVE_MAX`)은 **무리의 두 배쯤** 둔다. 그리고 자리를 볼 때 **무리 전체가 들어가나**를
본다 — 「한 마리라도 들어가나」로 재면 상한을 무리−1 만큼 넘긴다(실측 24 > 18).
...
**옛 구조가 아팠던 이유 — 미는 주체가 셋이었다.** 오토배틀(`strikeStepUnits`)을 돌린 **뒤에**
그 결과를 되돌리고 다시 미는 방식이었다:

1. 오토배틀이 표적에게 직진시킨다
2. `campEngageStep` 이 `u.x = u._sx` 로 그것을 **무르고** 제 목표로 다시 민다
3. `campLeash` 가 600 경계에서 **위치를 직접 잘라** 끌어당긴다

셋이 매 프레임 싸우니 유닛이 덜덜 떨었다. ⛔ **상수로 네 번 고쳐 보고 네 번 다 되돌렸다**
(닿으면 정지 / 뒤로 안 가기 / 목표 안쪽으로 / 전선 넓게).

**지금 구조** — `campStepUnits(dt)` 하나가 한 프레임을 통째로 맡는다:

```
패스 ①  표적 선정만 (죽음·대기·치유·벙커 처리 포함, 이동 없음)
        → 표적별로 묶어 자리 번호를 준다(uid 순 고정)
패스 ②  campGoalFor 로 목표를 한 번에 정하고 → 사거리 안이면 사격 · 밖이면 이동 한 번
```

⭐ **자리 제한을 「목표를 정할 때」 건다**(`campGoalFor` 끝). 그 뒤로 아무도 위치를 덮어쓰지
않으므로 되돌리기도 목줄도 필요 없다.

**🖐 내 명령은 명령이다 — SC 「이동」(2026-09-05 사용자 확정 · A안).** 던전에서 유닛을 골라 빈 바닥을
누르면(`campPtrUp` ④ → `campMoveSel`) 유닛에 `_order` 가 붙는다. `_order` 가 살아 있는 동안 패스 ①은
그 유닛의 **표적을 잡지 않고**, 패스 ②는 벙커·치유·복귀보다 먼저 **명령 지점으로만** 민다.
도착(`campArriveR` · 마린 ≈ 11 · 또는 `CAMP_ORDER_STALL` 동안 더 못 가까워짐)하면 그 자리가 새 `_post` 가
되고 AI 가 다시 맡는다. `_idleT` 를 채워 두어 복귀 대기(0.8초)를 안 탄다.

| 왜 바꿨나 — 같은 명령을 기지와 던전에서 잰 값(`scripts/camp-move-trace.mjs`) | 기지 | 던전(전) | 던전(후) |
|---|---|---|---|
| 명령 뒤 출발 | 0.03초 | **0.8초 굳음** | 첫 프레임 |
| 싸우는 중 뒤로 빼기 | 즉시 | **적이 죽을 때까지 무시**(거리 1012 → 1074) | 즉시 |
| 멈추는 자리 | 자리 | **45 앞**(`CAMP_POST_R` 은 AI 복귀용) | ≈22 앞(`CAMP_ORDER_ARRIVE` 24 · 공용 이동 물리가 관성으로 거기 선다) |
| 탭 히트 반경 | 0.055 | 0.022 | 0.055(`CAMP_PICK_R`) |

⛔ 옛 방식(자리만 옮기고 복귀에 맡기기)으로 되돌리지 말 것 — 스모크 「이동 명령은 바로 출발하고 · 적을 무시하고 · 찍은 자리에 선다」가 잰다.
⚠ 어택땅(가는 길에 사거리 안 적만 쏘기)은 **안 넣었다** — 필요해지면 `_order.atk` 플래그 하나로 패스 ①에서 사거리 안 표적만 허용하면 된다.

| 실측(30초 · 11기 · 연구 45 · 3판 중앙값) | 옛 구조 | **지금** |
|---|---|---|
| 방향 뒤집힘 / 유닛 | 96.4회 | **6.5회** |
| 실효(낼 수 있던 화력 중 실제) | 0.49 | **0.55** |
| 아군이 쏜 횟수 | 127회 | **161회** |
| 목줄이 위치를 자른 횟수 | 3760회(손 뗐을 때) | **0회** |
| 30분 벤치 도달(각 3~4판) | R10·R8·R9·R8 | R13·R9·R8 |

⛔ **`js/18-strike.js` 를 고치지 않는다** — 유즈맵 오토배틀과 공유한다. 거기서 가져오는 것은
**부품**뿐이다(`strikeHit` 피해·실드·상성 · `strikeMoveToward` 한 걸음 · `strikeReach` ·
`strikeNear`/`strikeGridBuild` 격자 · `strikeSeparate` 겹침 회피 · `strikeSkillTick` 스킬 29종 ·
`strikeHealStep` 치유). 통짜인 것은 `strikeStepUnits` 하나뿐이고 이 파일이 그것만 다시 썼다 —
그래서 상성·데미지 타입·스킬 측정(`RACES.md`)이 그대로 살아 있다.

⛔ **되살리지 말 것 넷** — `campPostSnap` · `campPostStep` · `campEngageStep` · `campLeash` 는
`js/19-camp.js` 에 남아 있지만 **배선이 끊겼다**(유보 규칙으로 남긴 것이다). 되살리면 미는 주체가
다시 둘이 된다. 스모크 「캠프: 미는 주체가 하나다」가 `campCombatStep` 소스를 훑어 막는다.
같은 이유로 `campBldSnap`/`campBldAmp`(건물 피해 배율 우회)도 끊겼다 — 이제 사격이 직접 곱한다.

##### 💉 의무병도 대열로 돌아온다 (2026-08-31 사용자 발견)

⛔ `campStepUnits` 패스 ① 에서 의무병이 `continue` 로 빠져 **복귀 분기를 영영 안 탔다.**
오토배틀의 치유 경로(`strikeHealStep`)에는 **「자기 자리」라는 개념이 없다** —
다친 아군 → 없으면 가장 가까운 아군 → 110 안이면 정지. 그래서 전투가 끝나도 낙오한 아군 옆에
붙어 선 채로 남았다.

📊 실측(60초 · 적이 하나도 없는 「조용한 프레임」 242개):

| | 전 | 후 |
|---|---|---|
| 전원이 자리에 있는 프레임 | **0개 (0%)** | **99개 (41%)** |
| 딱 한 기만 낙오 | 121회 — **전부 의무병** | 24회 (기관총병 · 걸어오는 중) |
| 의무병이 자리에서 벗어난 거리(평균) | 603 | **466** |

⭐ 우선순위 셋을 패스 ② 에서 순서대로 태운다: **① 다친 아군 치유 → ② 싸우는 본대 따라가기
→ ③ 자기 자리.** ②가 없으면 「다친 사람은 없지만 전투 중」에 의무병이 집으로 가 버려 치유가 못 붙는다.
⚠ 따라갈 때의 도착 판정은 `CAMP_HEAL_FOLLOW`(90) — 의무병은 사거리가 0 이라 `campArriveR` 를
쓰면 6px 이 나와 제자리에서 떤다.
⚠ `_campHealNeed` 는 `strikeHealStep` ①의 판정식을 **복사한 것**이다(저 함수가 값을 안 돌려준다).
⛔ 한쪽만 고치면 「따라가긴 하는데 치유는 안 하는」 상태가 된다.

##### 👀 인식 거리 900/1500 → 450/750 — 「나오자마자 돌격」을 고친다 (2026-08-31)

⛔ **인식이 「나갈 수 있는 거리」보다 크면 안 된다.** 옛 값의 실효는 `900×1.4 = 1260`(혼자) ·
`1500×1.4 = 2100`(전파받음)으로 이동 제한(1200)보다 컸다 — 그래서 **적이 화면에 나타나는 순간
전군이 표적을 잡고 제한 끝까지 우르르 올라갔다.** 「지키다가 맞으러 나간다」가 아니었다.

| 실측(40초 · 11기) | 전 | 후 |
|---|---|---|
| 적을 알아채는 거리 | 1765 | **851** |
| 자리에서 벗어난 거리 (평균 / 최대) | 639 / 1362 | **388 / 906** |

⭐ **전파(`campAlertTick`)는 그대로 둔다** — 「같이 싸우는 느낌」이 거기서 나온다. 다만 전파받은
인식(`CAMP_ACQ_ALERT`)도 함께 조여야 뜻이 있다. 안 그러면 한 명이 보는 순간 전원이 멀리 본다.
⚠ 이제 **인식이 실질적인 제한**이고 `CAMP_ENG_OUT` 은 최후의 안전선 역할만 한다(실측 최대 이탈 906).
⚠ `scripts/camp-trace.mjs` 의 `ACQ=n` 으로 상한을 갈아 끼워 잰다. ⛔ 프레임 **앞에서** 조이면 소용없다 —
`campAlertTick` 이 매 틱 `u.acq` 를 다시 쓴다(상한 350 인데 알아챈 거리가 1844 로 그대로였다).

##### 🧱 건물은 그려진 자리에 선다 — 좌표계를 하나로 (2026-09-05 사용자 신고)

⛔ **옛날엔 좌표계가 둘이었다.** `campBuildStructs` 가 건물을 전장에 올릴 때:

| | 매핑 | 결과 |
|---|---|---|
| 방어 건물(벙커·포탑) | `campG2W` — 유닛과 같은 것 | 그림과 일치 |
| **나머지 건물** | `sx/sy` — 격자를 전장 0.15~0.85 × **0.62~0.92** 로 압축 | **그림과 어긋남** |

실측(2026-09-05 · 격자 좌표를 전장에 올렸다 되돌린 값):

| 그려진 격자 | 되돌린 자리 | 어긋남 |
|---|---|---|
| (0.10, 0.20) | (0.220, 0.478) | **0.303** |
| (0.50, 0.40) | (0.500, 0.526) | 0.126 |
| (0.50, 0.60) | (0.500, 0.575) | 0.025 |
| 본부 (0.50, 0.622) | (0.500, 0.580) | 0.042 |

화면의 **12~30%** 다. 그래서 ① 적이 건물을 치러 엉뚱한 데로 갔고 ② 본부의 회피 원(유일한
장애물)이 그림에서 떠 있어 **빈 땅에서 유닛이 돌아갔다.** 지금은 **전부 `campG2W`** 다.

⚠ **맞바꾼 것** — 옛 규칙 「일반 건물은 뒤(62~92%)에 둔다, 앞에 나가면 곧바로 부서진다」는
사라졌다. 그 규칙과 「그림과 맞는다」는 **양립할 수 없다**(그림은 기지 맵이 격자로 그린다).
이제 격자 위쪽에 지으면 전선에 서고, 아래에 지으면 뒤에 선다 — 방어선을 플레이어가 고른다.

📌 **아직 다른 것 (해 보고 되돌렸다)** — 건물을 **단단한 장애물**로 만드는 것.
`strikeTempleRects` 를 캠프에서만 감싸 내 건물 전부를 원으로 돌려주게 해 봤다. 궤적은 좋아졌는데
(뒤집힘 16.7 → 13.8 · 사거리 안 22.2 → 23.6%) **아군이 갇혔다** — 의무병이 싸우는 아군에게
다가가긴커녕 멀어졌다(1152 → 1387 · 스모크가 잡았다). 원인은 반지름이 아니라 **구조**다:
기지는 `_techFindPath` 가 건물을 **우회하는 길**을 만들지만 **전장에는 경로탐색이 없다**
(국소 회피 + 원 밖으로 밀어내기뿐). 되살리려면 전장에 경로탐색/흐름장을 먼저 붙여야 한다.

📌 **이동 속도도 다르다 (안 건드렸다)** — 실측 기지 마린 **0.0650** 화면/초 vs 던전 **0.0435**
(기지의 67%). 기지는 `TECH_SPD_MUL`(0.6), 던전은 `MOVE_MUL`(3)로 나누는 **다른 식**이다.
맞추면 유닛이 적에게 1.5배 빨리 붙어 **전투 밸런스가 통째로 바뀐다** — 재보고 정할 것.

##### 🖐 던전 조작감 = 기지 조작감 — 드래그 문턱 (2026-09-05 사용자 신고)

⛔ **박스 문턱의 자[尺]가 달랐다.**

| | 식 | 390×767 맵에서 |
|---|---|---|
| 기지 원본 (`techPtrMove`) | `\|Δx\| + \|Δy\| > 6px` — **픽셀 · 두 축 합** | 대각 3+3 이면 선다 |
| 캠프 던전 (옛) | `\|Δx\|/폭 > 0.015 \|\| \|Δy\|/높이 > 0.015` — **비율 · 축별** | 가로 5.9px / **세로 11.5px** |

주석에는 「원본과 같은 값」이라 적혀 있었지만 **자가 달랐다** — 세로가 두 배 둔하고, 합이 아니라
축별이라 대각선은 더 둔했다. 그래서 「같은 레인저인데 던전에서만 드래그가 뻑뻑하다」가 됐다.
지금은 `CAMP_BOX_MIN_PX = 6` 으로 **원본과 같은 식**을 쓴다.

⚠ **스모크가 API 만 부르면 이 경로가 통째로 빈다.** 「캠프: 던전 드래그 문턱·⊘ 배선이 기지와 같다」는
진짜 `PointerEvent` 를 흘려보내 `techPtrDown → techPtrMove → campPtrUp` 을 그대로 탄다.

📌 **아직 다른 것 하나 (안 고쳤다)** — *지정한 상태에서 끌 때*의 뜻이 다르다.
기지는 유닛이 지정돼 있으면 드래그가 **이동 명령 추종**(`_btCmd` + `_techAssignMove`)이고 새 박스는
해제(⊘) 뒤에만 선다. 던전은 `_campSel` 이 `G.tech.selU` 를 비우므로 원본이 `selN===0` 으로 읽어
**늘 새 박스**가 선다. 바꾸려면 「던전에서 지정 중 드래그 = 이동」이라는 **동작 변경**이라 확인이 필요하다.

##### ⚔ 교전 중인 아군이 멀리서도 본대를 부른다 — `CAMP_ALERT_FIGHT_R` (2026-09-05)

DBG1 스냅샷(D1R33~R40)으로 잡은 정체: 앞으로 나간 유닛 하나(+의무병 둘)가 적 무리에 물리면 적은 그 유닛만 물고
안 내려오고, 그 유닛은 치유로 안 죽고, **본대 3~4기는 1,100px 뒤에서 눈(사거리+100 · 곁 전파 400)에 아무것도
안 들어와 놀았다.** 혼자 17마리를 깎으니 라운드가 몇 분, 치유가 피해를 이기면 영영이었다(20분 정체 2회).
`campAlertTick` 이 「표적이 사거리 안인 아군」(= 실제 교전 중)을 **1800** 까지 전파원으로 삼는다. 받는 쪽은 여전히
제 자리 제한(`campEngageOut`) 안에서만 간다 — 「나오자마자 우르르」(곁 400 규칙)와 다른 층이다.
실측: D1 완주 22분 → **16~20분** · 정체 0. 스모크 「교전 중인 아군이 멀리 있는 본대를 부른다」(옛 코드에서 실패 확인).

##### 🧱 경유점 도착 판정은 기지와 같은 0.05 — 길을 든 채 서는 교착 (2026-09-05)

`campMove` 의 경유점 판정을 0.018 로 두었더니 이동 물리가 목표 0.02~0.03 앞에서 「도착」으로 감속·정착해 멈추는데
판정은 「아직」이라 다음 점으로 안 넘겼다 — 적·아군이 `moving=false · hold=false · wp=3` 으로 20분을 섰다(D1R34).
기지 원본(`_dw<0.05`)과 같은 값으로 맞추고, 물리가 도착으로 멈추면 판정과 상관없이 다음 점으로 넘긴다.

##### 🕸 교착 안전망 넷 — 찍어서 잡았다 (2026-09-07)

`camp-bench.mjs` 가 한 라운드 4분을 넘기면 적·아군 전부를 한 번 찍는다(`🕸` 줄). 그것으로 잡은 넷과 고침 — 값은 전부
`js/21-camp-battle.js` 머리의 `CAMP_STUCK_*` · `CAMP_BLD_STUCK_T` · `CAMP_BLD_ALT_T` · `CAMP_LEASH_EXT*` 한 곳.
- **본부 사격 판정** — 본부의 밀어내는 원은 반폭 210(`strikeTempleHalf`)인데 「거리−46」으로 재서 사거리 63 이 영영 못 쐈다
  (아군 전멸 뒤 적 1 이 본부 앞 262 에서 10~30분). 본부만 `strikeTempleGap`(표면 거리)으로 잰다. 스모크 「적이 건물을 칠 때만 40배」①-b.
- **앞 건물 대체** — 적은 앞(y 최소) 건물만 치는데 거기까지 못 가면 30분을 선다. 5초 못 다가가면 그 유닛만 가장 가까운 건물을 12초 친다.
  ⛔ 처음부터 최근접으로 두지 말 것 — 무리가 갈라져 건물 여럿을 동시에 갉는다.
- **막힘 밀기(마지막 안전망)** — 목표가 도착 반경 밖인데 6초 동안 4px 도 못 가면 목표 쪽으로 28px 옮기고 길·진행도 창을 비운다.
  진행도 창(`_pgHold` 0.5초 대기 ↔ 0.22초 재시도)은 「영구 동결 없음」이 목적이지만 벽·원·서로 기다리기 앞에서는 영원히 돌았다.
  ⛔ 목표까지 순간이동시키지 말 것 — 28px 는 눈에 안 띄고, 원 안이면 도로 밀려나 다음엔 다른 쪽으로 간다.
- **목줄 늘리기** — 목줄(`CAMP_ENG_OUT` 1200)에 잘린 목표가 사거리 밖인 채 8초가 지날 때마다 **그 표적에 한해** +300(누적 · 표적이 바뀌면 원래대로).
  실측: 공중 적 셋이 앞에 나간 의무병을 치고 의무병 둘이 서로 치유하는데 대공 마린은 목줄 끝에서 사거리 밖(15분). `campGoalFor` 안.
  ⛔ 목줄을 아예 풀지 말 것 — 「자리를 지킨다」가 캠프의 뜻이다(2026-08-31 사용자 확정).
고친 뒤 90분 판 ×2 교착 0 · camp-trace 지표 변화 없음. 실측 표는 `BALANCE.md` §3-2-14.

##### 🩸 최소 피해 바닥(0.5)을 캠프에서만 걷는다 — `campPatchHit` (2026-09-05)

`strikeHit` 은 한 대를 `max(0.5, 공격−방어)` 로 센다. 오토배틀 스케일에서는 뜻이 없지만 캠프(1/10 스케일)에서는
R1 적의 한 대 0.09 가 0.5 로 올라가 **5.6배**였다 — 혼자서는 R5 에서 죽었고 `CAMP_FOE_ATK0` 가 죽은 축이었다.
캠프 전투 동안만 `strikeHit` 을 감싸 **0.5 미만의 한 대는 설계값 그대로** 넣는다(0.5 이상·실드는 원본). 켜고
끄는 자리는 `campPatchFront` 와 같다. ⛔ 18-strike.js 를 고치지 말 것(유즈맵 공유). 스모크가 캠프 안 0.1→0.1 ·
나가면 0.1→0.5 를 잰다. 실측: 혼자 R5 → **R13+**(BALANCE §3-2-11).

##### 👹 적의 눈은 아군보다 훨씬 넓다 — `CAMP_FOE_ACQ` (2026-09-05 사용자 확정)

⭐ **아군과 적은 규칙이 다르다.** 아군은 「제자리를 지키다 마중 나간다」라 눈이 좁아야 하고
(제 사거리 + `CAMP_ACQ_PAD`), 적은 **아군을 찾아오는 것이 일**이라 좁은 눈은 그냥 버그로 보인다.

⛔ **무엇이 문제였나** — 적도 `campAcqBase`(제 사거리 + 100)를 썼다. 근접 적은 사거리 47 이라
눈이 **147**(실효 147×`STK_ACQ_FAR` = 206)이었다. 진형 폭은 500~840px 이라 **측면 차선으로
내려오는 적이 아군을 못 보고 그대로 지나쳐** 건물로 갔다.

| 실측 (60초 · 11기 · D1R30 · `scripts/camp-trace.mjs`) | 전 | 후 |
|---|---|---|
| 적 인식 (실효) | 287 (402) | **1200 (1680)** |
| 👹 적이 표적을 가진 비율 | 20.9% | **69%** |
| 👹 아군 600 안을 **표적 없이** 지나간 비율 | 34.5% | **0%** |
| ⚔ 사거리 안 비율 | 19% | 22.2% |
| 📐 못 닿는 정도 | ×1.96 | ×1.66 |
| 💫 방향 뒤집힘 | 19회/유닛 | 16.7회/유닛 |
| 🔫 아군이 쏜 횟수 / 준 피해 | 236 / 749 | 252 / 804 |
| 💀 60초 뒤 남은 적 | 12마리 | **1마리** |

⚠ **받은 피해가 는다**(48 → 69) — 적이 실제로 아군을 때리기 때문이다. 맞바꾼 것이다.

⛔ **아군 쪽(`CAMP_ACQ_PAD` · `campAlertTick`)을 함께 넓히지 말 것** — 그건 「나오자마자 우르르」를
막는 장치다(바로 위 절). `campAlertTick`/`campAlertApply` 는 **아군만** 돌기 때문에, 적 값은
`campScaleFoes` 에서 한 번 심으면 프레임 중에 덮이지 않는다.

⛔ 유닛 표(`U`·`STK_UNITS`)의 `range` 를 고치지 말 것 — 멀티 대전과 오각형 상성이 같이 바뀐다.
여기서 만지는 것은 **소환된 적 개체의 `acq` 하나**뿐이다.

🎬 적 쪽을 재는 자[尺]는 `scripts/camp-trace.mjs` 에 있다 — `👹` 세 줄. `ROUND=n` 으로
라운드를 갈아 끼워야 적이 여러 마리 나온다(기본 R1 은 1마리라 측면 통과를 못 잰다).

##### 🪜 진형 — 세로는 촘촘히 · 가로는 넓게 · 짧은 사거리가 앞 (2026-09-01 사용자 확정)

⚠ 인식을 「사거리 + 100」(`CAMP_ACQ_PAD`)으로 조인 **뒤에** 실효 화력이 0.28 로 주저앉았다.
처음엔 「적이 안 내려온다」고 읽었는데 **틀린 진단이었다** — 아군 11기의 **평균** 거리로 쟀기 때문이다.
적 기준으로 다시 재니 적은 정상적으로 내려와 앞줄과 **52px** 까지 붙어 싸우고 있었다.

⭐ 진짜 원인은 **진형이 세로로 길었던 것**이다. 인식 260 · 전파 150 으로는 층을 못 넘어
앞의 두세 기만 싸웠다(제 사거리 안에 적이 있는 아군 **2 / 11**).

⛔ **여기서 두 번 헛짚었다.**
① 가로를 가운데로 모았다(`campLanePost` · `CAMP_LANE_K`) → 실효 0.29 로 **그대로**. 없앴다.
② 반대로 가로 3818px 에 15기를 흩었다 → 실효 **0.26 으로 더 나빠졌다**. 적 세 마리가 한 곳에
  오는데 부대가 그만큼 벌어지면 아무도 못 만난다. **「넓게」는 「흩어지게」가 아니다.**
⭐ 답은 셋을 **동시에** 만족시키는 것이었다 — 적이 **위에서** 내려오므로:

| | 값 | 왜 |
|---|---|---|
| 세로 | `CAMP_LAYER_MAX` 300 → **60** | 여러 층이 **동시에** 때린다 |
| 가로 | 자리 그대로(안 건드림) · 어깨 간격 60px | 옆으로 펴야 앞줄이 막지 않는다 |
| 순서 | 사거리 짧은 유닛이 **앞** | 앞뒤가 곧 교전 순서 |

⛔ **옛 `campLayerPost` 는 「제 자리에서 뒤로 민다」였다 — 층이 생기지 않았다.**
① 기준이 생산 건물 자리라, 짧은 사거리 유닛이 뒤쪽 건물에서 나오면 **그대로 맨 뒤**에 남았다.
  「짧으면 앞」이라 해 놓고 앞으로 **당기지는 않았기** 때문이다.
② `CAMP_LAYER_R0 = 100` 이라 화력병(70)은 물론 레인저(187)조차 21px 밖에 안 밀렸다.
  ⭐ 궤적 그림에 보이던 4층은 전부 **테스트 배치**가 만든 것이었다(4행 × 275px = 세로 825px).
  ⚠ 「배치가 결과를 만들고 있었다」 — 자[尺] 자체가 틀렸던 사례다.
⭐ 지금은 **기준선 `CAMP_LINE_GY`(0.50) 하나에서 사거리 순으로 줄을 세운다**(절대 정렬).
어느 건물에서 나오든 순서가 같다. 실측 층 폭 **17px** — 사실상 한 줄이다.

| 실측(60초 · 15기 보병 · 연구 45 · 3판 중앙값) | 전 | 후 |
|---|---|---|
| 실효 화력 | 0.29 | **0.62** |
| 표적을 가진 비율 | 32% | **49%** |
| 아군이 쏜 횟수 | 254회 | **385회** |
| 실제로 준 피해 | 1254 | **2623** |
| 받은 피해 | 62 | **32** |
| 60초 뒤 적 | 1~3마리 생존 | **0마리(전멸)** |

⚠ **떨림이 늘었다** — 방향 뒤집힘 4.3 → **12.7회/유닛**(순수 오토배틀 기준 0.9).
가로로 늘어선 유닛들이 각자 다른 방향의 적에게 달려가며 좌우로 방황한다. 궤적 그림에서 보인다.
아직 안 고쳤다.
⚠ 판별 편차가 크다(사거리 안 13.2~27.1%). 한 번 재고 결론 내지 말 것.

##### 📣 전파 반경 150 → 400 — 헛걸음이 사라진다 (2026-09-01 사용자 확정)

⚠ 눈은 「사거리 + 100」(레인저 287)인데 진형은 **840px** 이라, 왼쪽 끝 유닛은 오른쪽에 온 적을
아예 못 본다. 적이 끊이지 않는 던전에서도 **표적을 가진 비율이 33~52%** 였다.

⛔ **화력 수치로는 이 변화가 안 보인다** — 던전 1은 실효 0.61~0.62 로 전후가 같다.
적이 3마리뿐이라 이미 다 잡고 있어서 **천장에 걸린 상태**이기 때문이다.
⭐ 보이는 것은 **행동**이다(사용자가 짚었다 — 「다 쏘기 전에 적이 죽는 것 아니냐」):

| 던전 1 · 60초 · 보병 15기 | 닿음 | **못 닿음** | 가는 중 적이 죽음 |
|---|---|---|---|
| 전파 150 | 23 · 20 | **16 · 30** | 23 · 19 |
| 전파 400 | 21 · 25 · 24 | **2 · 10 · 0** | 39 · 26 · 39 |

⇒ **끝내 못 닿는 헛걸음이 23건 → 4건으로 사라졌고**, 그만큼이 「가는 중에 적이 죽음」으로 옮겨갔다.
헛수고가 정상적인 결말로 바뀐 것이다.

⛔ **진형 폭(840)까지 주지는 않는다** — 어디에 두든 전군이 달려가 **배치의 뜻이 사라진다**
(2026-09-01 사용자 확정). 400 은 「곁의 두세 명 건너까지」다.
⚠ 자리 이탈은 안 늘었다(평균 120~143 → 134~165) — 우르르 몰려가지 않는다.
⚠ **던전 2 에서는 갈리지 않는다** — 실효 150: 0.25·0.25·0.43·0.33 / 400: 0.35·0.45·0.38·0.24.
거기는 적이 하나도 안 죽고(「가는 중 적이 죽음」 0건) **아군이 전멸한다** — 다른 문제다.

##### 📊 30분 벤치 — 네 단계 전부 **사실상 같다** (2026-08-31 · 판별 편차가 변화보다 크다)

| 단계 | 도달 라운드 | 비고 |
|---|---|---|
| 옛 전투 구조(기준선) | R10 · R8 · R9 · R8 | ⚠ 다른 작업과 동시 실행 |
| 새 구조 + `CAMP_ENG_OUT` 1200 | R13 · R9 · R8 | ⚠ 동시 실행 |
| + 인식 조임(450/750) | R13 · R10 | ⚠ 동시 실행 |
| + 의무병 복귀 | R8 · R8 · R6 · R7 · R9 | ⚠ 동시 실행 |
| **+ 시스템 점검 6건 · 단독 실행** | **R11 · R14 · R10 · R9 · R6** (중앙값 10) | ✅ 얼어붙음 0 · 예외 0 |
| **+ 초반 경제 재설계 · 진형 한 줄 · 인식 「사거리+100」** | **R15 · R18 · R15** · 번 돈 **726~776만** · **환생 가능** | 단독 3판 · 2026-08-31 저녁 |
| + 채취 비용 「세 레벨마다 ×10」 | R14 · R15 · R9 · 번 돈 **81~87만** | 단독 3판 · 2026-09-02 (수입 구성을 갈랐다 — BALANCE §3-2-5) |
| **+ 적의 눈 `CAMP_FOE_ACQ` 1200** | **R33 · R39 · R38 · R36 · R41** (중앙값 **38**) · 번 돈 97~108만(중앙값 99만 · 한 판만 58만) | 단독 5판 · 2026-09-05 |
| + 건물 좌표계 통일 | R42 · R34 · 번 돈 103만 · 79만 | 단독 2판 · 2026-09-05 · **위 범위 안 — 달라진 것 없다** |
| **+ 이동 속도 가운데(0.1087) · 건물 길찾기** | **R27 · R32** · 번 돈 **119만 · 144만** · 환생 27.6분 · 25.0분 | 단독 2판 · 2026-09-05 · 돈은 늘고 라운드는 줄었다 |

⭐ **마지막 줄이 지금 값이다** — 이동 속도를 기지와 가운데(0.1087)에서 맞추고 전장에 길찾기를
붙인 뒤, **번 돈이 119~144만으로 늘고 두 판 다 30분 안에 환생이 됐다**(그 전엔 5판 중 2판).
⚠ 대신 **도달 라운드는 R33~R42 → R27~R32 로 줄었다.** 라운드가 느려진 만큼 경제에 더 쓴 것으로
보이는데 **2판뿐이라 단정하지 않는다.** 방향만 일관됐다.

⭐ 그 앞 단계: 적의 눈을 넓히자 도달 라운드가 **R14 → R38**(2.7배)로 뛰었다 —
적이 건물을 갉는 대신 아군에게 와서 죽기 때문이다. 번 돈은 86.5만 → **99만**(+14%)으로 조금만
늘었다: 라운드는 빨리 넘어가지만 채취 축이 세 레벨마다 ×10 이라 수입이 그만큼 못 따라온다.
⚠ **한 판(58만)이 크게 튀었다** — 나머지 넷은 97~108만으로 모인다. 원인은 안 봤다.
⚠ 앞의 두 줄은 **경제 재설계 전/후가 섞여 있다** — 견줄 때 조건을 함께 볼 것
(726~776만 줄은 옛 채취 곡선이라 지금 값과 직접 비교할 수 없다).

📊 같은 시점 궤적(30초 · 3판 중앙값): 방향 뒤집힘 **4.1회** · 자리에서 벗어난 거리 **평균 99**
(전 388) · 적을 알아챈 거리 **377**(전 851) · 목줄·순간이동 **0** · 실효 0.50~0.55.
⚠ 다만 **표적을 가진 비율이 47.5%**(전 83%)다 — 인식이 짧아진 대가로 절반이 논다.
궤적이 말하는 병목은 「닿는 것」이 아니라 **「찾는 것」**이다: 방향을 꺾은 것의 100% 가 사거리
밖이고, 표적을 잡으면 0.3초 만에 사거리 안에 들며, 못 닿은 것은 전부 「가까워지곤 있었다」.
지금 균형에서는 손해가 아니지만(벤치가 훨씬 좋다) 인식을 다시 만질 때 볼 자리다.

⚠ **벤치는 단독으로 돌려야 한다.** 다른 무거운 작업과 겹치면 판이 중간에 멎는 사례가 있었다
(다섯 판에 한 판꼴). 단독 5판에서는 한 번도 안 났다. 원인은 미특정이지만, 얼어붙은 판이 표본에
섞이지 않도록 `camp-bench.mjs` 가 **최종 줄에 「🧊 얼어붙음 — 표본으로 쓰지 말 것」을 붙인다**
(`FREEZE_MIN` · 기본 5 시뮬분).

⛔ **두세 판으로 「좋아졌다/나빠졌다」를 말하지 말 것.** 같은 설정에서 R6~R13 이 나온다 —
편차가 어떤 변화보다도 크다. 이 프로젝트에서 그렇게 두 번 잘못 읽었다:
① 기준선을 한 판(R10)만 재고 「1200 이 더 좋다」 ② 인식 조임 뒤 두 판(R13·R10)만 재고 「안 떨어졌다」.
⭐ **확실히 달라진 것은 움직임뿐이다** — 떨림(96.4 → 6.5회/유닛) · 자리 이탈(639 → 388) ·
의무병 낙오(조용한 프레임의 50% → 0%).
⚠ 벤치가 가끔 **D0R0(번 돈 4천)** 으로 끝난다 — 던전에 못 들어간 판이다. 전투가 아니라 초반
경제가 멎은 것이고, 이 전투 개편 **전에도 나왔다**. 원인 미특정 — 그런 판은 표본에서 뺀다.

##### 🚧 자리 제한 `CAMP_ENG_OUT` 500 → 1200 (같이 바뀐 값)

⛔ 옛 「500 이 최적」은 **그 제한이 실제로는 안 지켜지던** 판의 값이다 — 오토배틀의 무제한 추격이
먼저 돌았고 campEngageStep 이 넘긴 유닛은 그대로 계속 나갔다. 이제 글자 그대로 지켜지므로
같은 500 을 두면 병력이 자리에 묶여 화력이 반토막 난다.
📊 30분 벤치: **500 → D1R5 · 800 → D1R7 · 1200 → D1R13/R9/R8 · 1800 → D1R7**(너무 나가면 흩어진다).
⚠ **판별 편차가 커서 한 판으로는 못 고른다** — 같은 1200 에서 R8~R13 이 나온다. 처음에 기준선을
한 판만(R10) 재고 「1200 이 더 좋다」고 읽었는데, 네 판을 재니 기준선도 R8~R10 이었다.
⭐ 정직한 결론: **화력은 기준선과 같고, 달라진 것은 떨림이다**(96.4 → 6.5회).
⚠ 이 값은 **방어 건물의 뜻과 맞바꾼 것이다** — 전선이 넓게 움직일수록 벙커·포탑 자리의 의미가 옅어진다.

#### 🏰 적 기지 **화면** — 엔진 뒤에 따로 얹었다 (2026-09-09)

엔진(`campFoeBase` · 릴레이 · 전리품)이 먼저 들어갔고 **그리는 코드가 없어** 플레이어 눈에는 적 기지가 없었다.
화면은 셋으로 갈라 얹었다 — 모델은 전부 `js/23-camp-dungeon.js`, DOM 은 `19-camp.js`, 3D 는 기존 파이프라인.
...
| 배치 | `campFoeLayout(d, seed)` · `CAMP_FOE_ROW` · `C.foeSeed` | **거울 + 랜덤**(2026-09-09) — 광맥 호 → 본진 → 테크 → 생산 · 탑은 구간 앞 · 자리만 씨앗(mulberry32) · 원정마다 새 씨앗 · 광맥·가스·일꾼(`campFoeWorkers3D`)은 연출 · 줄은 gy −0.43~+0.02(높은 건물·광맥은 고원 −0.45~−0.14 안 · 생산·앞 탑은 통로 · 2026-09-10) · 적 3D 는 rimCol=PLAYER_VIEW_COLORS[1] → M3D.applyTeamTint(붉은 적 진영색) · ⚠ `campG2W` 는 레인 위를 전장 y 0 까지 허용(`CAMP_LANE_TMIN`) |
| 안개 | `campFogSync()` · 훅 `campFogExtra(f,asp,cpt)` · `campFogHidesAt(x,y)` | 관리자 건설 탭의 `G.tech.fog` 를 그대로 빌린다(10-engine) — 세로 범위 `f.wy0~wy1` 만 열어 격자 위까지 덮고, 전장 병력이 시야를 낸다 · 내 격자는 늘 활성 · 안개 속 적은 3D·HP 바 **엔트리에서 뺀다**(오버레이가 3D 아래라) |
| 바닥 그림 | `.bmapFloor::before{top:-58%}` (css/30-home.css) · `scripts/map-crop.mjs` | 격자 위 0.58 까지 깐다 — 바닥 요소 자체는 격자라 배경을 ::before 로 뺐다 · 그림은 9:16 → 1:2 크롭(ART.md §17) |
...
⛔ `techMap3DSync`(90-m3d)를 확장하지 말 것 — **아무도 안 부르는** 관리자 맵 경로다(한 번 거기 얹었다가 되돌렸다).
...
#### 💀🏥 죽음과 회복 — 2026-09-10 사용자 확정 (옛 「아군 부활」을 대체)

규칙이 **넷**이고 서로 맞물린다. 하나만 되돌리면 나머지가 깨진다.

| | 규칙 | 어디 |
|---|---|---|
| ① | **죽으면 영구히 죽는다.** 부활이 없다 — 건물에서 다시 뽑는다 | `campReapDead` |
| ② | 죽으면 **인구와 재구매 배수가 되돌아온다** | `campUnlist` |
| ③ | **회복 존** — **내 구역 입구 아래**면 **5초마다 체력 가득**(유닛·건물) | `campHealZone` |
| ④ | **패배 = 본부 파괴 하나뿐.** 전멸은 패배가 아니다 | `campCombatStep` |

⭐ **왜 이 모양인가.** 사용자의 설계는 「평범하게는 절대 못 깬다 — 캠프에서 엄청나게 돈을 벌고
화력을 올려놔야 겨우 올라간다」다. 그러면 **오래 버티는 구간**이 게임의 본체가 되므로 그 바닥을
받쳐 줄 것이 필요하다. 마린키우기의 회복 존과 같은 장치이고, 난이도의 문턱이 **한 자리로 모인다**:
「적의 화력이 회복 속도를 넘는가」. 넘는 순간부터 유닛이 실제로 죽기 시작한다.

⭐ **①과 ④가 짝이다.** 2026-09-09 에 「전멸 = 원정 끝」(`_wiped`)을 넣었던 이유는 **순환**이었다 —
부활이 「관문을 깰 때」로 옮겨졌는데 병력이 0 이면 관문을 못 깬다(BALANCE §5-5: 30분 내내 아군 0기).
②가 그 순환을 끊는다. 죽으면서 인구와 값을 돌려주므로 **전멸해도 다시 살 수 있고**, 캠프 경제는
원정 중에도 돈다. 시간 제한 노릇은 **적이 내 건물을 부수며 다가오는 것**이 한다.
⛔ `_wiped` 를 되살리지 말 것 — 「전멸했지만 돈이 있어 다시 세운다」는 반격이 통째로 사라진다.

⭐ **③의 경계는 맵 그림의 「내 구역 입구」**다 — `CAMP_HEAL_GY`(격자 y **0.36**).
📏 눈금을 그려 실제로 쟀다(2026-09-10 · 캠프 배경): **난간·문설주 0.33~0.36 · 노란 빗금 0.36~0.39**.
0.36 은 그 문간 한복판이라 플레이어가 「여기까지가 내 땅」이라고 읽는 선이다.
⛔ **안개 경계(`techY0` 0.18)로 되돌리지 말 것** — 그건 격자 위끝이라 그림상 **통로 한복판**이고,
사용자가 바로 그것을 「안개 없는 땅이 내 땅보다 넓다」고 지적했다(2026-09-10).
⛔ ART.md §17-1 표의 「석판 위끝 0.46」을 그대로 쓰지 말 것 — 던전 그림 기준의 옛 값이라 석판 한참 안쪽이다.
판단의 단일 소스는 셋이고 **같은 상수**를 본다: `campInHome(u, W)`(유닛) · `campHealGyOk(gy)`(격자) ·
`campBuildNoHeal(gy)`(건설 미리보기).

🧱 **건물은 회복 구역 밖에도 지을 수 있다**(2026-09-10 사용자 확정) — 벙커·포탑을 앞에 세우라고 남긴
자리다. ⛔ 막지 말 것. 대가는 「맞아도 안 고쳐진다」 하나이고, 화면이 그것을 미리 말한다:
**노란 격자**(`.bfoot.warn` · `.bghost.warn`) + **반투명 경고문**(`.bhint.warn` — 안내 문구 컴포넌트를
그대로 쓴다. 캠프는 `.bhint` 를 통째로 감추므로 `css/30-home.css` 가 이 조합만 다시 켠다).
⛔ 경고용 바를 새로 만들지 말 것.
🔨 **건물은 맞는 중에 안 고쳐진다** — 직전 주기에 체력이 줄었으면 이번 주기를 건너뛴다.
⛔ 무조건 채우지 말 것: 2026-09-10 실측에서 그 때문에 ④가 **통째로 죽어 있었다**(전멸 뒤 10분을
돌려도 본부 750 ↔ 728 · 회복이 적 화력의 35배). 고친 뒤 전멸 → 본부 파괴가 **165~185초**이고
사다리 전체에서 같다(BALANCE §5-10-2).
⚠ **유닛에는 이 규칙을 걸지 말 것** — 「자리를 지키면 안 죽는다」가 회복 존의 전부다.
갈리는 지점이 여기다: **유닛은 지켜 주고 건물은 소모된다.**
⛔ **부서진 건물은 회복 존이 안 살린다** — 살리면 적이 본부까지 못 와 ④가 영영 성립하지 않는다.
⛔ **자리(`_post`)가 아니라 지금 서 있는 곳**을 본다. 자리로 재면 적진에 나가 있어도 회복돼
드래그 진격의 대가가 사라진다(스모크가 잰다).

⚠️ **죽은 유닛은 전장 배열에 남지 않는다.** `strikeStepUnits` 끝에서 `me.units=me.units.filter(u=>!u.dead)`
로 걷어낸다(`18-strike.js:1301` · 공유 파일이라 ⛔ 못 고침). 그래서 캠프가 **걷히기 전후를 비교해** 붙잡는다:

```
const _b4 = CAMPB.me.units.slice()   // 스냅샷
campWithStk(campStepUnits)            // 여기서 죽은 것이 걷힌다
campReapDead(_b4)                     // 사라진 것 = 이번 프레임에 죽은 것 → 명부에서 지운다
```

붙잡지 않으면 **인구도 재구매 배수도 안 돌아온다** — 죽음이 조용한 영구 손실이 된다.

💰 **재구매 배수의 단일 소스는 `G.tech.units` 하나다**(`campUnitOwned`).
⛔ 거기에 전장 명부(`CAMPB.me.units`)를 **더하지 말 것** — 2026-09-10 실측: 던전 안에서 같은 유닛이
두 번 세어져 배수가 1.30 이 아니라 **1.69/기**로 붙었다(프로브 owned 1 → 2 → 4). 던전에 오래
있을수록 병력이 안 늘던 것이 이것이다.

| 상수 | |
|---|---|
| `CAMP_HEAL_S` 5 | 회복 주기(초) — 사용자 확정 2026-09-10 |
| `CAMP_UNIT_R` 1.30 | 재구매 배수 — 죽으면 지수가 한 칸 내려간다 |

⛔ **되살아나면 안 되는 것들**(스모크가 잰다): `campReviveStep` · `campReviveSec` ·
`campRoundRevive` · `campCatchDown` · `CAMPB._down`(부활 대기) · `CAMP_WIPE_GRACE`.
남은 것은 **`campHealAll()`**(관문을 깰 때·던전이 바뀔 때 판 전체 체력 회복)뿐이고,
그것은 회복 존과 **다른 자**다.

#### 🔬 연구·계열 업그레이드 — 캠프 전투 배선 (2026-08-27)

⛔ **오래 비어 있던 자리다.** 캠프에서 연구를 사도 **전투에 아무 일도 일어나지 않았다.**
`campScaleAllies()` 가 환생 트리 배수만 얹었기 때문이다.

**빌리면 안 되는 것 —** `_upgAtk`/`_upgDef`(`js/11-cmdcard.js`)는 이름이 맞아 보이지만
**샌드박스 전투실험(`stepSbCombat`) 전용**이고 「+1/티어」 **덧셈**이다.
오토배틀은 또 다른 축(`side.atkLv`/`hpLv`)을 보는데 캠프는 그것을 채우지 않는다.
셋이 서로 다른 축이라는 것을 전제로 읽을 것.

```
campScaleAllies(list)
  ├ campDesignStats()          ⚔ 설계 능력치(_campStat 로 한 번만)
  ├ campRtMul('atk'|'hp')      🌳 환생 트리 — 전 유닛 공통
  └ campResMul(uid,'atk'|'hp') 🔬 연구 — **유닛의 계열별**
      → 곱해서 한 번에 얹는다(_campRtOn 로 한 번만)
```

| | |
|---|---|
| 계열 찾기 | `UNIT_UPG[uid]` → `.atk`(공격) / **`.def`(체력)** |
| 왜 체력이 방어 키인가 | §3-1 에서 방어(armor)를 뺐다 — 방어구 업그레이드 자리를 체력이 대신한다(`HUNT_R1` §3-4) |
| 레벨 | `G.tech.research[G.tech.race + '_' + 계열키]` |
| 배수 | `CAMP_RES_STEP` **1.065**^Lv — ⛔ 덧셈으로 되돌리지 말 것(레벨이 무제한이라 후반에 무의미해진다) |
| 종족 | `RACE_OF` 가 아니라 **`G.tech.race`** — `techDoResearch` 가 그 키로 쓴다 |

스모크 「캠프 연구: 계열 업그레이드가 전투에 걸린다」가 지킨다 —
찍은 계열만 세지는가 · 체력이 방어 키를 읽는가 · 트리와 곱해지는가 · 두 번 안 걸리는가.

#### ⛽ 연구 값 — 캠프는 가스만 받는다 (2026-08-27)

⭐ **미네랄 = 양(유닛·일꾼·건물) / 가스 = 질(강화·해금).**
미네랄은 지수로 자라서 어떤 가격표를 붙여도 결국 공짜가 된다 — 그래서 미네랄로 매긴 강화 비용은
「비싼 미네랄」일 뿐이고 두 번째 자원을 둔 뜻이 사라진다. 가스는 안 자라므로 끝까지 모자란 것으로 남는다.

| | |
|---|---|
| 단일 소스 | **`campResearchCost(r, lv)`** (`js/19-camp.js`) — 캠프가 아니면 **`null`** |
| 계열 업그레이드 | 미네랄 0 · 가스 `CAMP_RES_GAS0(1) × CAMP_RES_GAS_R(1.08)^Lv` · **상한 없음** |
| 단발 연구 | 미네랄 0 · 가스 `CAMP_RES_ONE` = 10/15/20 — **등급은 원본 `r.m`(100/150/200)이 그대로** |
| 트리 할인 | `campUpgDisc()` 를 태운다(건물·유닛과 같은 문) |

⛔ **호출부는 공유 파일 둘이다** — `techDoResearch`(`17-build-cards.js`)와
`_techResearchCard`(`16-build.js`). 둘 다 **`null` 이면 원본 값**으로 떨어지게 짜여 있다.
그쪽에 캠프 값을 박으면 관리자 탭·오토배틀이 함께 망가진다.

⛔ **3티어 하드캡을 되살리지 말 것.** `r.tier` 는 3칸뿐이지만 설계 §3-4 는 「무제한」이다 —
적이 던전당 ×2 세지는 것을 11레벨씩 따라잡는 구조라, 3에서 막으면 던전 2에서 멎는다.
`techApplyResearch` 는 그냥 `+1` 이라 3을 넘어도 그대로 쌓인다.

스모크 「캠프 연구: 값이 가스만이고 계열 업그레이드에 상한이 없다」가 지킨다 —
값·등급·Lv7 · 실제 구매에서 가스만 깎이는가 · **캠프를 나가면 원본 값으로 돌아가는가**.

#### ⛽ 정제소 「가스 생산」 — 캠프에서만 꽂는 연구 카드 (2026-08-27)

⛔ **오래 비어 있던 자리다.** 값을 매기는 `campUpgCost('refinery')` 는 있었는데
**누를 데가 없어서** 플레이어는 가스 생산을 올릴 수 없었다(벤치만 값을 직접 만졌다).

**UI 를 새로 만들지 않는다** — 캠프에 들어올 때 정제소의 `research` 배열에 항목을 하나 꽂고
나갈 때 뺀다(`campSyncUnitCost` 와 같은 빌림-반납). 카드·구매·진행바·비용 표시를 공짜로 얻는다.

| | |
|---|---|
| 꽂기/빼기 | `campPatchRefinery()` / `campRestoreRefinery()` — `campEnter` · `campExit` |
| 항목 | `CAMP_REF_RES` = `{k:'gasup', tier:[]}` |
| 값 | `campResearchCost` 가 `r.k==='gasup'` 을 보고 **미네랄**로 돌려준다 |
| 레벨 | `G.tech.research[race+'_gasup']` — `campRefLv()` 이 옛 `C.upg.refinery` 와 **max** 로 읽는다 |

⛔ **`TECH_TREE` 는 관리자 탭·오토배틀과 공유**다. 항목을 영구히 넣지 말 것 —
`tier:[]` 는 캠프에서만 성립하고(값을 `campResearchCost` 가 대신 낸다), 캠프 밖에서 그리면
`r.tier[lv]` 가 undefined 라 터진다. 스모크가 **나갔을 때 빠졌는지**까지 검사한다.

⚠ **미네랄로 사는 유일한 강화**다 — 미네랄(양) → 가스(질)로 가는 다리이자 후반에 남아도는
미네랄의 출구다. ⛔ 값을 너무 싸게 하지 말 것 — 실측에서 **경제 축이 통째로 멎었다**
(효율Lv 32→10 · 일꾼 14→7 · `BALANCE.md` §3-2-3).

#### 🩸 스킬 체력 코스트 — 캠프 자릿수로 낮춘다 (2026-08-27)

⛔ **캠프 설계 능력치는 SC 의 약 1/8 이다**(레인저 체력 5 vs SC 마린 40).
`SKILLS` 의 코스트는 **SC 자릿수 그대로**라 그냥 두면 조건에 늘 걸린다 —
광폭화 `hpCost:10` 이면 `u.hp <= 20` 이 항상 참이라 **영영 안 나간다**
(실측: 16분에 `strikeSkillTick` 5,139회 · 시전 0회).

`campPatchSkillCost()` / `campRestoreSkillCost()` — 진입 때 `hpCost × CAMP_SK_HP_K(0.125)`,
나갈 때 원복. ⚠ `SKILLS` 는 관리자 탭·오토배틀과 **공유**다.

⚠ **마나(`enSc`)는 안 건드린다** — 에너지는 `campDesignStat` 이 손대지 않아 SC 자릿수 그대로다.
체력만 자릿수가 달라졌기 때문에 체력 코스트만 어긋났다.

⚠ **스킬이 도는지 재려면 `campScaleAllies` 를 먼저 씌울 것.** 안 씌운 유닛은 SC 체력을 들고 있어
**테스트가 가짜로 통과한다**(그렇게 한 번 속았다). 스모크가 `u0.maxHp<=10` 으로 그것을 막는다.

#### 🌳 환생 포인트 트리 (2026-08-25 · 6단계)

> ⚠️ **효과 배선은 18/32 계열이다**(2026-08-25). 화면·비용·사슬은 완성이고,
> **나머지 15계열은 사도 아무 일이 일어나지 않는다** — 표에만 있고 소비하는 곳이 없다.
> 코드 주석도 이를 전제로 쓰여 있다: `f=효과 종류(**배선된 것만** 아래에서 쓴다)`.
>
> | 갈래 | 배선 | 어디서 소비되나 |
> |---|---|---|
> | **enemy**(적 약화) | **8/8** ✅ | `campRtFoeMul()` 이 갈래 8계열을 **일괄로** 곱한다 → `campScaleFoes`. 추가로 `foeN` 만 `campFoeCount` 에서 마리 수로도 쓰인다 |
> | **army**(아군 강화) | **7/8** | `campScaleAllies`(atk·hp) · `campBattleOpen`(bldg=본부 체력) · `campCombatStep`(skCd) · `campApplySupCap`(sup) · `campUpgDisc`(upCost) · **`campEndureP`(endure = 버팀)** |
> | **econ**(재화) | 2/8 | `gather`(일꾼 채취) · `tapMul`(탭 배수) |
> | **start**(시작 도움) | 1/8 | `tap`(탭당 미네랄) |
>
> **army 에서 남은 하나 — 왜 안 붙였나**
> - **생산 속도**(`prod`) — 생산 시간은 `techTick(dt)` 이 진행하는데 **건설·수리·나이더스·융합이 같은 틱에 물려 있다.**
>   `dt` 를 부풀리면 전부 같이 빨라진다. 생산만 떼려면 `16-build.js` 를 고쳐야 해서 ⛔ 에 걸린다.
>
> ⭐ **`rebuild`(자동 재생산)는 「버팀」(endure)이 됐다.** 2026-08-25 엔 「부활 단축」으로 읽었는데
> **부활 자체가 없어져서**(2026-09-10) 그 읽기가 무효가 됐다. 지금은 **치명 피해를 1회 무시하고
> 체력 1 로 버틴다**(유닛당 한 관문 1회 · `CAMP_RT_END` 0/15/30/45/60/75%).
> ⛔ T5 를 100% 로 올리지 말 것 — 전멸이 안 나고 실질 체력 ×2 라 체력 축과 겹친다.
>
> ⚠ **아군 강화는 개체 값에 그대로 곱한다** — 적(`campScaleFoes`)은 '무리 총량'을 난이도에 맞추지만
> 아군은 기준 총량이 없다. 같은 유닛에 두 번 걸리지 않게 `u._campRtOn` 으로 막는다(안 막으면 라운드마다 눈덩이).
>
> ⚠️ **`campRtMul(k)` 는 계열 키(`k`)를 받는다 — `f` 가 아니다.** `campRtMul('gatherMul')`(f) 는 늘 1 을
> 돌려주고 `campRtMul('gather')`(k) 라야 사다리 값이 나온다. 실제로 여기서 한 번 헛짚었다.
>
> 🔢 **계열은 제 값을 가질 수 있다**(2026-09-02) — `lad`(값 사다리) · `cs`(차수별 비용) · `mx`(차수 상한).
> 가진 것이 이기고, 없으면 공용 규칙(`CAMP_RT_LADDER` · 티어 공식 · 5차)으로 떨어진다.
> ⛔ 계열 하나 때문에 `CAMP_RT_BASE`·`CAMP_RT_MUL` 을 흔들지 말 것 — 나머지 전부가 함께 움직인다.
> ⚠ **빈 묶음은 그리지도 팔지도 않는다**(`campRtGpLive`) — 「시작 도움」의 묶음 「다」가 그렇다.
>
> 🔍 **공용 SVG 뷰 엔진 `svv*`**(2026-09-03 · `js/19-camp.js` 환생 트리 절 바로 앞).
> 「한 장의 판을 밀고 벌린다」를 한 곳에 모았다 — 화면은 상태 객체(`svvNew()`)만 갖는다.
> ⭐ **이동 방식은 캠프 메인 화면과 같다**(2026-09-03 사용자 확정) — 손가락은 **목표 뷰(tx/ty/tz)만** 바꾸고,
> 매 프레임 지금 뷰가 그 목표로 다가간다: `k = min(1, dt × SVV_FOLLOW)`, `SVV_FOLLOW = 9`.
> 메인맵 `nemoViewTick`(14-input-fx.js) · 건설 `techViewTick`(17-build-cards.js)과 **같은 식**이다.
> 가속·감속은 이 보간 하나가 전부 만든다 — ⛔ 이징 곡선을 따로 만들지 말 것.
> ⛔ **관성(손을 떼면 미끄러지는 것)을 넣지 말 것.** 한 번 넣었다가 되돌렸다(2026-09-03) —
> 이 게임의 다른 화면은 전부 「목표를 따라간다」라 혼자만 미끄러지면 조작감이 갈라진다. 스모크가 잡는다.
> `svvBind(svg, {v, g, lim, hit, onTap, onEmpty, onDouble, alive})` 하나로 드래그·핀치·휠·더블탭이 붙고,
> `svvGoto`(목표) · `svvZoomAt`(앵커를 붙잡고 목표 배율) · `svvLookAt`(월드점을 화면 어디에) ·
> `svvFit`(점 목록으로 전체 보기) · `svvTick`/`svvKick`(보간과 그 루프)이 계산을 맡는다.
> 쓰는 곳: 💠 룬 성좌 판(`campRuneBindMap`).
> ⛔ 판 `<g>` 에 CSS `transition:transform` 을 걸지 말 것 — 보간과 이중이 되어 늦게 따라온다(스모크가 잡는다).
> ⚠ **이관 부채** — 환생 트리(`campTree*`)는 아직 제 사본을 쓴다. 먼저 만들어져 실측으로 다듬어진
> 코드라 이번엔 손대지 않았다. 트리의 팬·줌을 손볼 일이 생기면 **그때 이리로 옮길 것**.
> ⛔ 세 번째 사본을 만들지 말 것 — 새 판이 필요하면 이 엔진을 쓴다.
> 🤏 **판을 쓰는 화면은 `touch-action:none` 을 걸어야 한다**(2026-09-12 사용자 신고 「두 손가락으로
> 벌리면 바깥 인터넷 창까지 같이 확대된다」). 이게 없으면 **브라우저가 핀치를 먼저 먹어** 페이지째
> 커졌다 작아지고, 엔진에는 pointer 이벤트가 제대로 오지도 않는다.
> ⚠ **판만 막으면 모자라다** — 판 밖(가방·상단 띠) 위에서 벌려도 페이지가 커진다. **구역 전체**에 걸고
> 스크롤 목록만 `touch-action:pan-y` 로 돌려준다(`pan-y` 는 세로 밀기만 허용하므로 거기서도 확대는 안 먹는다).
> HOME(`#homeScreen` + `.uiScroll`)이 쓰던 수법 그대로다. 휠(데스크톱 트랙패드 확대)은 `svvBind` 의
> `wheel` 핸들러가 `preventDefault` 로 이미 막는다. ⛔ `auto` 로 되돌리지 말 것(스모크가 잰다).
> ⏳ **뷰가 움직이는 동안 화면 좌표를 재지 말 것.** 보간이 도는 중에 `getBoundingClientRect()` 로 잰
> 자리는 **곧 틀린 값**이 된다 — 룬이 칸으로 날아가는 연출이 그걸로 옛 자리에 내렸다(2026-09-12).
> 룬 쪽은 `_runeAfterLook(fn)`(22-camp-rune.js)이 **보간이 멎은 뒤에** 재고 날린다.
> ⏰ 그 기다림에는 **타이머 안전망**이 함께 있어야 한다(`RUNE_LOOK_MAX`) — rAF 만 믿으면 탭이 가려졌을 때
> 콜백이 영영 안 와서 연출이 통째로 사라진다(실측).
>
> ⛓ **사슬 갈래**(2026-09-02) — `CAMP_RT_CHAIN` 에 든 갈래는 **관문(갈래·묶음)이 없다.**
> 가운데에서 바로 첫 별로 가고, 별 하나가 여러 다음 별을 연다(부모는 계열의 `pa`, 2차 이상은 앞 차수).
> 좌표는 `campRtChainMap()` 이 부모에서 부채꼴로 뻗어 한 번 계산해 캐시한다.
> ⚠ 아직 안 옮긴 갈래는 옛 관문 구조 그대로다 — **한 갈래씩** 옮긴다(한꺼번에 옮기면 28계열 1차가 가운데에 몰린다).
> ⛔ 새 CSS 클래스는 이름부터 겹치는지 볼 것 — `.ctStep` 으로 지었다가 비용 줄의 같은 이름에 먹혔다.
>
> 확인 명령 — 어떤 계열이 실제로 소비되는지:
> `grep -rho "campRt(Mul|Cut|Has)('KEY')" js/*.js`
...
##### 🖐 밀고 키울 때의 부드러움 (2026-09-02 사용자 요청)

⭐ **캠프 맵과 같은 수법**이다(`techViewTick`) — 코드는 안 빌리고 **방식만** 같다.
손가락·휠은 **목표 뷰(`_ctViewT`)만** 바꾸고, 실제 뷰는 매 프레임 목표 쪽으로 지수 보간으로 따라간다.

```
campTreeViewT()       목표 뷰 — 입력이 만지는 유일한 값
campTreeSmoothStep()  rAF 루프 · k = min(1, dt×9) · 배율만 등비로 당긴다
campTreeViewSettle()  보간을 즉시 끝낸다 — 값을 바로 읽어야 하는 곳(스모크·계산)
campTreeViewSync()    목표를 지금 뷰에 맞춘다 — 연출이 뷰를 직접 밀었을 때
```

| 규칙 | |
|---|---|
| ⛔ 배율은 **등비** | 확대는 곱셈 축이다. 선형으로 끌면 앞이 훅 커지고 뒤가 느려 보인다(트윈이 같은 이유로 등비) |
| ⚠ 연출이 돌면 **양보** | `_ctTween` 중에는 보간이 쉰다. 둘이 같은 값을 밀면 서로 싸워 떨린다 |
| ⚠ 연출 뒤 `campTreeViewSync()` | 안 부르면 연출이 끝나는 순간 보간이 **옛 목표로 되돌리며** 화면이 튄다 |
| ⚠ 붙잡기 계산도 **목표 기준** | 따라가는 중인 값으로 잡으면 휠을 연달아 굴릴 때 붙잡은 점이 흘러간다 |

⭐ **뷰를 옮기는 모든 길이 이 한 방식으로 모인다** — 휠·드래그뿐 아니라 **별 누르기·해제·전체 보기**도
목표만 옮기고 보간에 맡긴다(`campTreeTweenTo` 가 그렇게 바뀌었다). 같은 화면에서 조작마다 감이 달라지지 않는다.

⛔ **5제곱 이징 연출로 되돌리지 말 것**(옛 `campTreeEase`·`campTreeTweenStep` · 함수는 남겨 뒀다).
그 곡선은 처음과 끝 30%가 거의 안 움직이고 중간 40%에 몰려 **「굼뜨다 → 확 간다 → 질질 멈춘다」**로 읽힌다 —
사용자가 「딱딱 끊긴다」고 한 것이 그 느낌이다(2026-09-02).

🎬 **재는 법: `node scripts/tree-smooth.mjs`** — 프레임마다 뷰 값을 받아 적어
「한 프레임에 튀는지(계단) 여러 프레임에 걸쳐 붙는지(곡선)」를 가른다.
...
⚠ **케이스마다 같은 자리에서 출발시켜야 한다**(스크립트의 `reset()`). 앞 케이스가 남긴 배율에서
시작하면 「확대가 0프레임」 같은 값이 나와 원인을 잘못 짚는다 — 실제로 한 번 그랬다.
⚠ 그리고 프레임 기록 루프의 **정지 플래그는 루프를 시켜 놓기 전에 내린다.** 뒤에 내리면
큐에 남은 rAF 하나가 살아남는 타이밍에만 값이 나와서 「어떤 케이스는 되고 어떤 케이스는 0」이 된다.

입구는 **하단 네비 「환생 › 트리」 하나**다. 띠(`#campBar`)에도 칩이 있었는데 없앴다(2026-09-01) —
네비에 제 칸이 있어 입구가 둘이었다. 🔁 환생 칩도 같은 이유로 없앴다(2026-09-03).

##### 🚪 마디 능력 — 관문도 세진다 (2026-09-02 사용자 확정)

갈래·묶음 관문 15개가 문만 여는 게 아니라 **그 안 계열들과 같은 축에 작게 얹힌다**.
갈래 `+10%` · 묶음 `+5%`(`CAMP_RT_NODE_BR`/`_GP` — 상수 둘이 단일 소스).

⭐ **새 효과 종류를 만들지 않는다.** 마디 몫이 들어가는 길은 **둘뿐**이다:

| 길 | 어떤 축 | 어떻게 |
|---|---|---|
| `campRtMul(k)` | 기준이 **1** 인 배수형(채취·공격력·적 약화 …) | 마디 몫을 **더한다** (1 → 1.15) |
| `campRtNodeMul(k)` | 기준이 1 이 **아닌** 축(확률·시간·간격·할인·인구) | 마디 몫을 **곱한다** (5% → 5.75%) |

⛔ 확률 5% 에 `+0.10` 을 더하면 15% 가 된다(3배). **축의 기준값을 보고 길을 고를 것.**
⚠ 기준이 **0** 인 축(할인·버팀·인구)은 계열을 사야 마디가 일한다 — `0 × 1.15 = 0` 이다.
거기서는 「미리 맛본다」가 아니라 「산 것을 더 세게」가 된다.
⛏ **채굴 간격만 나눈다** — 작을수록 좋은 값이라 곱하면 마디를 살수록 느려진다.

⛔ 상한을 반드시 걸 것: 업그레이드 할인 ≤ 0.95(1 이면 공짜) · 버팀 확률 ≤ 1 · 피버 확률 ≤ 1 ·
적 약화는 계열 상한 −40% 그대로. 스모크가 넷 다 검사한다.

🔍 **누가 닿는지 재는 법: `node scripts/node-fx.mjs`** — 마디 15개가 각각 어느 계열에 닿는지 뽑는다.
⚠ 「`campRtMul(k)` 값이 변하나」로 재면 **틀린다**. 그 값을 안 쓰는 계열이 있어서다 —
한 번 그렇게 재서 「전부 닿는다」는 거짓 보고를 냈다. 도구는 **소스에서 호출을 뽑아** 판정한다.

지금 안 닿는 셋과 그 이유:

| 계열 | 왜 |
|---|---|
| 피버 타임 `fever` | **on/off 스위치**다(`vk:'on'`). 배수가 뜻을 갖지 않는다 |
| 일꾼 상한 `wkCap` | 일꾼 상한 **40 고정**이라 계열 자체를 배선하지 않았다 |
| 던전 보상 `dgRw` | 던전 클리어 보상 **시스템이 아직 없다** |
...
> 🎨 **렌더 = 기지 맵 「위쪽 레인」에 겹쳐 그린다** (2026-08-25 · A안). **화면을 바꾸지 않는다** —
> 적은 격자 위끝에서 내려오고 내 병력이 맞으러 올라간다. 던전 중에도 광맥·건설을 그대로 만진다.
>
> ⛔ **공유 코드를 고치지 않았다**(`renderBuildTab` · `18-strike` · `16/17-build`). 캠프 프레임 동안만
> **`M3D.syncBuild` 를 감싸** 기지 리스트 뒤에 전투 유닛을 덧붙여 통과시킨다 — `campWithStk` 가 전역 `STK` 를
> 바꿔 끼우는 것과 같은 관용구다. ⭐ **캔버스도 sync 호출도 프레임당 하나** 그대로다(두 번 부르면 뒤엣것이 앞엣것을 지운다).
>
> ⚠️ **`M3D.sync` 가 아니라 `M3D.syncBuild` 다**(`14-input-fx.js:950`). 건설 맵은 전용 입구를 쓴다 —
> `sync` 를 감쌌다가 **0건**이 나와 한 번 헛돌았다.
>
> | | |
> |---|---|
> | 좌표 | `campW2G(sx,sy,W)` — 전장 세로축(적 `W*0.14` ↔ 내 본부 `W*0.86`)을 레인에 선형 대응 |
> | 레인 | `CAMP_LANE_TOP 0.18`(격자 위끝) ~ `CAMP_LANE_BOT 0.62`(본부 `y≈0.642` 바로 위) · 폭 `0.88` |
> | 엔트리 | 기지 유닛과 **같은 규약** — `scl=TECH_USCALE×TECH_UVIS×cellK` · `yoff` · `yawFix` · `z=_zOf` 재계산 |
> | 컬링 | 화면 밖(`±0.2`)은 리스트에 안 넣는다(오토배틀 `STK_CULL` 과 같은 뜻) |
>
> **위험 11 실측(2026-08-25)** — 동시 **111기** 렌더에서 **28.7fps**. 캠프 프레임 상한이 30fps 대라 사실상 유지된다.
...
##### 🔮 예외 둘 — 스킬 자동 시전은 **공유 파일에 넣었다** (2026-08-27)

⛔ 위 「`18-strike.js` 를 고치지 말 것」의 예외다. `strikeSkillTick` 이 self/toggle/aura 만
처리해서 마법 유닛이 아무것도 안 했고, **효과를 적용하는 코드 자체가 없었다.**
캠프 전용으로 감싸 봐야 오토배틀은 그대로 고장난 채라 **본체에 넣었다.**

- 대상 고르기 `_stkPickAlly`/`_stkPickFoe`/`_stkPickSpot` · 효과 `_stkApply*` · 지속 피해 `_stkDotTick`
- ⛔ 엔진에 걸 곳이 없는 효과는 **시전하지 않는다** — 목록이 `STK_SK_DEAD`
- ⚠ 이 변경은 **멀티 대전의 승패를 바꾼다.** `duo` 그룹까지 돌려서 확인할 것
- 상세·실측은 `BALANCE.md` §3-4

##### 🪖 유닛은 **한 번만** 태어난다 · 자리는 내가 정한다 (2026-08-28)

⛔ 예전엔 두 번 태어났다 — 생산하면 기지(`G.tech.ents`)에 서고, `campSortie` 가 3초 뒤
그것을 지우고 전장에 새로 만들었다(스폰 지점에). 오토배틀 출격 구조를 빌리며 딸려 온 것이다.

| 지금 | |
|---|---|
| 생산 완료 | `campPatchFinish` 가 가로채 `campDeploy` 로 **그 자리에** 전장 유닛을 세운다 |
| 인구 | 반환하지 않는다 — 살아 있는 동안 묶인다(생산이 인구 200 에서 막힌다) |
| 자리 | `u._post` — 내가 옮길 때마다 갱신 · 표적이 없으면 그 자리로 돌아온다 |
| 복귀 | ⚠ 위치를 덮어쓰지 않는다. 프레임 시작 위치로 무른 뒤 `strikeMoveToward` 로 다시 민다 |
| 조작 | 탭=지정 · 드래그 박스=여러 기 · 빈 바닥 탭=이동. 캠프가 up 에서 먼저 보고 `_btDown`·`_btBox` 를 비운다 |
| 던전 전환 | `campDungeonSwap()` — 적만 갈아 끼운다. `campBattleOpen()` 을 쓰면 병력이 증발한다 |
| 0단계에서 뽑은 병력 | `campAdoptBaseUnits()` 가 전장이 열릴 때 데려온다 |

⛔ **되살리지 말 것** — `campSortie` · `campTrimArmy`. 둘 다 위 구조의 **증상을 막던 코드**다.
  스모크가 이름으로 잡는다(`typeof campSortie === 'undefined'`).
⚠ 복귀 판정은 `u.tgtUid` 가 있는지가 아니라 **그 표적이 살아 있는지**를 본다 — 적이 죽어도
  번호는 남아서, 안 그러면 유닛이 영영 자리로 안 돌아온다(실측으로 잡았다).

##### 💰 캠프가 갈아 끼우는 가격 셋 — 전부 `campFrame()` 이 민다

`campSyncHire()`(일꾼) · `campSyncSupply()`(보급소) · `campSyncUnitCost()`(전투 유닛 ×1.15^보유).
값은 `TECH_TREE` 안에 있고 **관리자 탭·오토배틀과 공유**라 나갈 때 `campRestore*` 로 되돌린다.

⚠ **프레임을 끄고 직접 미는 코드(벤치)는 이 셋을 같이 불러야 한다.** 안 부르면 값이
기본가에 얼어붙는다 — 실제로 반복 구매가 통째로 안 걸려 마린 101기가 나왔다.
...
| **띠 `#campBar`** | **적 수** · ⚡ 피버 — **읽는 것만** | 맵 위 · `campBarRender()` |
...
- **전역 하단 네비 `#navBar` = 2층 드릴다운**(2026-08-14). 단일 소스는 **`NAV_TREE`** 하나 — 최상위 5구역과 각 구역의 하위 항목을 다 갖고 있고, 마크업은 `navPaint()`가 만든다(HTML에 칸을 손으로 쓰지 말 것).
  ...
  - 🚪 **구역을 떠나면 그 구역의 오버레이 화면을 닫는 일도 `navShow` 가 한다** — 환생 구역(화면 넷)과 룬이 그 대상이다. 둘 다 `#phone` 직속 z 120 전체 화면이라, 안 닫으면 **다음 화면을 통째로 덮어 「나갈 수가 없다」**가 된다.
    - ⭐ 환생 구역의 화면 목록은 **`NAV_TREE` 의 `reb.subs` 하나**다(2026-09-12) — 칸마다 `scr`(요소 id)과 `shut`(닫는 함수)을 들고 있고, `rebZoneSubs()`/`rebZoneScreens()`/`rebZoneIsOn()`/`rebZoneShut(keepK, keepArt)` 넷이 그것만 읽는다.
    - ⛔ **화면 id 를 다른 곳에 또 적지 말 것.** 실제로 목록이 셋으로 갈려 있었다(`navShow` · `curSplitSync` · `campRebEnter`). 그중 `navShow` 것만 옛 둘(`campReb`·`campTree`)에 멈춰 있어, 2026-09-10·11 에 붙은 **유즈맵 강화(`#mapUpgScreen`)·환생 강화(`#rebUpgScreen`)** 가 구역을 떠나도 `.on` 인 채 남았다(사용자 신고: 「나오려고 하면 화면이 계속 남아 다른 구역으로 못 가거나 겹쳐 보인다」). 지금은 셋이 같은 목록을 읽는다.
    - ⚠ `keepArt` 는 **구역 안에서 칸만 바꿀 때만** 참이다 — 떠날 때 주면 배경 그림이 남는다. 그림 끄기 자체는 `campRebArtOff` 가 한 박자 뒤에 `_rebArtAnyOn()` 을 다시 보므로, 넷을 전부 닫으면 알아서 꺼진다(룬으로 넘어갈 때는 룬이 같은 그림을 쓰므로 안 꺼진다).
    - ⚠ 스모크가 잠그는 것은 **칸이 아니라 규칙**이다: ① 구역 **안에서** 칸을 옮기면 늘 하나만 열려 있다 ② **어느 칸에서 나가도** 넷이 전부 닫힌다. ⚠ ①을 「칸마다 나갔다 들어오며」 재면 **통째로 거짓**이다 — 나가면서 다 닫히므로 겹칠 수가 없다(그렇게 짰다가 겹침 주입이 그냥 통과했다).
...
SMOKE_CPU=4 node test/run-smoke.mjs lobby                # 🐢 주 스레드를 4배 느리게(크롬 CPU 스로틀링) — 「느린 기기」 흉내
...
- 🔁 **흔들리는 스텝은 한 판으로 못 잡는다**(2026-09-07). `SMOKE_REPEAT` 로 집계하고 `SMOKE_ONLY` 로 좁혀 고친 뒤 다시 센다.
  ...
  🐢 **느린 기기는 `SMOKE_CPU=4` 로 잰다.** 바깥에서 CPU 를 바쁘게 하는 것(루프 12개)은 크롬 실행 시간을 1초도 못 늘렸다 —
  크롬은 제 코어를 얻는다. 스로틀링에서는 평상시 0 실패였던 스텝 열한 개가 더 넘어졌고 **전부 같은 병**이었다:
  「n ms 잔 뒤 한 번 읽기」·「25ms 폴링으로 중간 낚기」— 프레임이 1초 넘게 벌어지면 표본이 연출 **뒤**에 떨어진다.
  고치는 어법은 하나다: **`el.getAnimations()[0]` 을 `pause()` 하고 `currentTime` 을 원하는 지점에 놓고 읽는다**(그림이 몇 프레임이든
  같은 답) · 「순간」은 `MutationObserver` 로 적어 두고 이미 지났으면 **순서**로 판정한다 · 잠든 시간은 잠들기 **전**부터 잰다.
  ⚠ 상한(「너무 오래 머문다」) 검사는 멈춘 시간(60ms 넘는 프레임의 초과분)을 빼고 잰다 — 안 빼면 느린 기기가 다 실패한다.
  ⚠ 스로틀링에서 같은 브라우저로 REPEAT 하면 2판째부터 4~5배 더 느려졌다(281→1029→1440초) — 판마다 새로 띄워 재라.
...
- 👆 **막대가 다 차면 저 혼자 넘어가지 않는다 — 손가락을 기다린다**(2026-09-12 사용자 요청 「터치해서 게임을 시작하도록」 · `js/08-ui-parts.js`).
  `enterAfterWarm()` 의 순서가 `warmAll → opBarDone() → `**`bootTapWait()`**` → titleToBlack() → openHome()` 이 된다 —
  ⭐ **기다리는 한 걸음만 끼워 넣었다.** 페이드아웃·페이드인은 이미 있던 것을 그대로 탄다(⛔ 새 전환을 만들지 말 것).
  - 문은 **로그인 뒤의 부팅 로딩 하나**다(사용자 확정) — 유즈맵 게임 진입(`#opening.counting` · `#gsRoot`)에는 안 붙는다.
  - 여는 길 셋: ① 진짜 `pointerdown`(문서 캡처) 또는 Enter/Space ② `bootTapGo()` — **프로그램이 여는 뒷문**(스모크·`scripts/shot.mjs flick` 이 쓴다) ③ `BOOT_TAP_GATE=false` — 문 자체를 끈다.
    ⛔ `BOOT_TAP_GATE` 를 끈 채 내보내지 말 것 — 스모크가 상수를 직접 잰다.
  - ⚠ **열지 않고 두면 멈추는 데서 끝나지 않는다.** 문은 대기 중 문서에 리스너를 걸어 두므로, 스모크가 안 열면 한참 뒤 다른 스텝의 탭이 「게임 시작」으로 먹혀 부팅이 되살아난다(실측: 캠프 스텝 둘이 같이 터졌다). 그래서 `enterAfterWarm` 을 기다리는 곳은 **전부** 문을 열어 준다(지금 네 곳).
  - 🎨 얼굴: 안내 문구 `.opTap`(「화면을 터치하여 게임을 시작하세요」)이 **막대가 있던 바닥 자리**에 들어오고, 막대 도크(`.opDock`)는 물러난다.
    🔀 **차례가 겹치면 안 된다** — 둘은 같은 자리라 동시에 트면 100% 막대 **위에 글자가 겹쳐** 지나간다(2026-09-12 프레임으로 잡았다 · 평균 밝기로는 안 보인다).
    값은 `#opening` 의 `--opTapOut`(.30s 막대가 물러난다) · `--opTapIn`(.42s 문구가 들어온다) 둘이고, 문구의 `transition-delay` 가 `--opTapOut` 이다. 숨 애니는 그 뒤에 시작한다.
    스모크가 「문구 들어오는 때 ≥ 막대 물러나는 때」를 **정적으로** 잠근다(⛔ 한쪽만 바꾸지 말 것).
