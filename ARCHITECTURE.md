...
- **개인 플레이 난이도 = 세그먼트 + 상세**(2026-08-19). `#soloDiffPanel` 안이 `sdNav`(공용 `segNavHTML`) + `sdDet`(고른 하나만 크게) + `sdInf`(무한 모드) 로 나뉜다. ⛔ 목록을 훑어 **바로 시작**하던 방식(`.soloDiffBtns`)은 폐지 — 오탭으로 시작되고 난이도끼리 비교가 안 됐다.
  ...
  - ⚠ 탭 글자 여백은 **`padding:8px 0`** 이다 — 350px 카드에서 5칸이면 한 칸이 59px 이고 `NORMAL` 이 58px 이라 좌우 여백을 2px만 줘도 잘린다. 스모크가 `scrollWidth`로 잘림을 검사한다.
  ...
  - **시작(`​.sdGo`)은 이 화면의 주 액션이라 면을 난이도 색으로 채우는 유일한 예외**이고, 취소는 글자만 남긴다(`#soloDiffPanel .cpCancel`). ⚠ 난이도 색이 형광(EASY `#5dff8f`)이라 그대로 채우면 눈부시다 — 위에 어둡게 하는 그라디언트를 덮어 톤을 낮춘다.
  - ⚠ **카드 높이 524px 는 내용에서 나온 값이다**. `.sdBody`(이름 28px + 수치 2칸 + 설명 한 줄)가 담기려면 상세 판에 그만큼이 필요하고, 470px 이던 시절엔 설명 줄이 시작 버튼 위로 흘러 **잘린 것처럼** 보였다. `.sdBody` 에 `overflow:hidden` 을 두고 스모크가 모든 난이도에서 `scrollHeight`/버튼 겹침을 검사한다.
