...
- **`.msPanel` = 이중 테두리 + 모서리 컷**(2026-08-18). 바깥 1px(금속 엣지) + 안쪽 1px 액센트 프레임(`::before`, inset 4px), 라운드 대신 **좌상·우하 11px 컷**. DESIGN §2 '선을 굵히지 말고 겹을 늘린다' 그대로다.
  - ⚠ **안쪽 프레임의 컷 길이는 카드와 똑같이 11px** 이어야 한다 — 짧게 주면 두 대각선이 평행하지 않아 잘린 모서리에서 어긋난다. 스모크가 두 `clip-path`의 px 값을 비교한다.
  - ⚠ **`clip-path` 가 걸린 카드는 바깥 `box-shadow` 를 못 쓴다**(그림자까지 잘려 나간다). 이 카드는 원래 그림자가 없다 — 나중에 넣지 말 것.
  - ⚠ 팝업(`.setCard` 계열)에서는 이 이중선을 **의도적으로 걷어냈다**(주석에 남아 있다). 여기는 '목록 상자'로 되살린 것이니 그 주석을 근거로 같이 지우지 말 것.
  - 안쪽 여백은 8 → **12px**(`.msSortTabs`/`.msList`) — 프레임(5px)과 내용이 3px까지 붙어 답답했다.
