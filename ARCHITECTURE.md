...
> ⚠️ **효과 배선은 17/32 계열이다**(2026-08-25). 화면·비용·사슬은 완성이고,
...
> | **army**(아군 강화) | **6/8** | `campScaleAllies`(atk·hp) · `campBattleOpen`(bldg=본부 체력) · `campCombatStep`(skCd) · `campApplySupCap`(sup) · `campUpgDisc`(upCost → `campUpgCost`·`campCost` 둘 다) |
> | **econ**(재화) | 2/8 | `gather`(일꾼 채취) · `tapMul`(탭 배수) |
> | **start**(시작 도움) | 1/8 | `tap`(탭당 미네랄) |
>
> **army 에서 남은 둘 — 왜 안 붙였나**
> - **생산 속도**(`prod`) — 생산 시간은 `techTick(dt)` 이 진행하는데 **건설·수리·나이더스·융합이 같은 틱에 물려 있다.**
>   `dt` 를 부풀리면 전부 같이 빨라진다. 생산만 떼려면 `16-build.js` 를 고쳐야 해서 ⛔ 에 걸린다.
> - **자동 재생산**(`rebuild`) — 「죽은 유닛 25~100% 자동 재구매」의 **해석이 미정**이다:
>   라운드 시작에 죽은 만큼 다시 출격시키는가, 미네랄을 실제로 차감하는가.
>
