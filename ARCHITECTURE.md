...
- **유즈맵 소셜 카드 테두리 = 금속 링**(`--metal-ring`, 회색). 탭 띠(`.pdSeg`)·하단 네비와 같은 값이라 화면 안 재질이 한 갈래로 모인다. 마을 시트로 돌아갈 때는 `.inTown` 이 테두리를 걷어내므로 이 규칙은 `#msSocialDock` 안에서만 건다.
  - ⚠ **면이 반투명한 카드에 `padding-box`/`border-box` 2겹 배경으로 그라데 테두리를 그리면 안 된다.** `border-box` 층은 면 **뒤에 깔리므로** 반투명한 면을 통해 회색 그라데가 그대로 비쳐 **배경 전체가 회색이 된다**(실제로 그랬다). 면이 불투명한 `.pdSeg`·`.hmUp` 에서는 드러나지 않는 함정이다.
  - 그래서 여기서는 `::before` + `mask-composite:exclude` 로 **1px 링만** 그린다(`.cpCard::before` 와 같은 수법). 스모크가 링이 마스크로 잘리는지와 면의 휘도가 어두운지를 함께 검사한다.
  - 8안 목업(모서리 컷·이중 테두리·금속 링·브래킷 등)을 거쳐 고른 값이다. 이중 테두리·`clip-path` 컷은 **되돌렸다** — 다시 넣지 말 것.
  - ⚠ **목록의 아래 여백은 `padding` 이 아니라 `margin` 이어야 한다.** 스크롤 영역은 **padding box** 에서 잘리므로, padding 으로 주면 카드가 그 여백까지 흘러들어와 판 끝선에 딱 붙어 잘린다('카드 밖으로 나간' 느낌의 원인). `margin-bottom` 이라야 잘리는 자리가 카드 안쪽으로 들어온다. 여기에 `mask-image` 로 끝 18px 을 흐려 '안쪽에서 자연스럽게 끝난다'로 읽히게 했다. 스모크가 `margin-bottom` 과 카드 아래 여백을 검사한다.
- **유즈맵 화면은 카드 두 장이다**(2026-08-18). `.msPanel`(정렬 띠 `#msSortTabs` + 목록 `#msList`)과 소셜 도크(`#msSocialDock`, 채팅·친구·파티)는 **서로 독립된 카드**다 — 한쪽이 다른 쪽 안에 들어가면 안 된다. 두 카드의 좌우 변은 같고(18~372), 사이는 `.msSocial` 자신의 `margin-top:9px` 이 띄운다. 목록이 끝나는 자리의 선은 `.msPanel` 자신의 아래 테두리가 맡는다 — `.msList` 에 또 그으면 두 줄로 겹친다. 스모크가 서로를 품지 않는지·각자 테두리가 있는지·좌우 변과 간격을 검사한다.
