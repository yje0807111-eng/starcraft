...
- **구역 자동 진입은 "도착"에만 걸린다**(`twCheckZones`): 반경 안이어도 **멈춰 섰거나 목적지 자체가 그 구역일 때만** 연다. 이 조건을 빼면 광장→모서리로 걸어가는 길에 생성소를 스쳐 팝업이 튀어나온다(실제로 밟았음 — 여유가 54px뿐이었다). 스모크 `마을: 구역 옆을 스쳐 지날 땐 안 열림`이 지킨다.
