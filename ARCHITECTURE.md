...
- **유즈맵 화면은 카드 한 장, 그 안이 두 구역이다**(2026-08-18). `.msPanel` 하나가 **정렬 띠 → 목록(`#msList`) → 소셜(`#msSocialDock`)** 을 전부 담는다. 붉은 헤어라인이 카드 위·아래를 감싸 '한 장'임을 말한다.
  - ⚠ **카드 안에 카드를 두지 않는다.** 아래 구역이 다른 구역임을 말하는 것은 둘뿐이다 — ① 판을 가로지르는 1px 칸막이(`#msSocialDock` 의 `border-top` + 윗변 광선 1줄) ② 살짝 가라앉은 면(`rgba(0,0,0,.34)`). 그래서 `.msSocial` 이 갖고 있던 테두리·면·바깥여백은 도크 안에서 전부 걷어낸다. 스모크가 '소셜이 자기 테두리·면을 갖지 않는지'와 '칸막이가 카드 폭을 가로지르는지'를 검사한다.
