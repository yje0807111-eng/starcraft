...
- **🤝 전직은 폐지됐고, 옛 상위 직업 12종이 그대로 '동료'가 됐다**(2026-08-12 설계 전환). `PROF_JOBS`는 **뿌리 3종만** 남았고(캐릭터는 만든 그대로 끝까지 간다), `profClassChange`·`profClassCost`·`hbGrowJobs`·`PROF_JOB_PARENT`는 **삭제됐다** — 되살리지 말 것.
  - **`HB_MATES`(12종)** = 옛 상위 직업의 이름·3D 모델(`unit`)을 그대로 가져온 표. `lv`(영입 해금 레벨)는 오름차순이고 `PROF_UNLOCKS`와 같은 규칙으로 벌려 뒀다. 밸런스는 표만 고친다 — 전투·UI에 수치를 박지 말 것.
  - **상태는 계정 축이다** — `hunt.mates{id:레벨}`(0/없음 = 미영입) + `hunt.party[]`(출전). **환생해도 남는다.** 정원은 `hbMateMax()` = 기본 3 + `ally_plus` 해금 2. ⚠ 정원 검사는 `hbMateToggle`이 **거절**하는 방식이어야 한다 — `hbParty()`의 slice만 믿으면 저장된 편성이 몰래 넘친다(스모크가 거절 여부까지 본다).
  - **영입과 강화는 같은 버튼**(`hbMateBuy`) — 처음 사면 Lv.1(=출전 가능), 이후 `HB_MATE_MUL(1.55)` 배씩 비싸지고 위력이 `HB_MATE_STEP(0.22)`씩 오른다.
