...
- **성장 축은 미네랄 업그레이드(`HB_UPG`) 하나다** (2026-08-12 확정). 캐릭터 스탯에 **직접 찍는 경로는 없다** — 레벨 포인트도, 자동 배분도 폐지했다(`profAllocStat`·`hmAllocStat`·`profDoAlloc`·`profGainStats` 전부 삭제).
...
- **벙커는 주둔 유닛 수만큼 쏜다.** 타일에 `n`(0~`HB_BUNKER_SLOTS`)을 저장하고 새 벙커는 1기로 시작한다. 화력 = `캐릭터 공격력 × HB_BUNKER_UNIT_DPS × n × hbBunkerAtkMul()`이고 배수는 **방어 탭 업그레이드 `bkatk`**(`HB_UPG.bkatk`, cat `def`)가 정한다. 필드에서 벙커를 누르면(`hbTapStruct` → `hbOpenBunker`) 창이 열린다 — `hbFieldTap`에서 **이동보다 먼저** 걸러야 한다. ⚠ 사거리는 `hbUnitFire`가 캐릭터 사거리를 쓰므로(`HB_BUNKER_RNG` 배수) 캐릭터 사거리를 0으로 만들면 벙커도 못 쏜다 — 테스트에서 실제로 헛돌았다.
