...
- **👾 사냥터 몹 여섯 역할**(2026-08-20) — `HB_FOE_KIND`(기본·돌격·사수·비행·중장갑·유령) 한 표가 스탯·크기·사거리·이동을 다 갖고, 얼굴은 던전별 `roster`(역할↔모델↔가중치)가 정한다. **역할과 외형이 분리**돼 있어 같은 '사수'가 던전마다 다르게 생긴다. 생성은 `hbPickFoe(D,S)` 가중 추첨 하나, 보스는 `hbBossEntry(D)` 가 편성표에서 가장 무거운 놈을 골라 키운다(**보스는 늘 지상 근접** — 날거나 통과하는 보스는 벽·기지 설계를 통째로 무의미하게 만든다).
  ...
  - 🧱 **배치 격자는 지으려는 건물 둘레 `HB_GRID_PAD`(1)칸까지만** — `clip()` 으로 좁히고 면 없이 선만(알파 .09). 예전엔 보이는 맵 전체를 푸른 면으로 덮고 격자를 다 그려서 전장이 안 보였다. 맵 경계(`strokeRect(-R,…)`)는 격자와 별개라 그대로 둔다. 스모크는 **캔버스 호출을 받아 적어 격자선이 실제로 그려진 범위**를 잰다(함수 소스 정규식은 주석만 남아도 통과해서 못 쓴다).
  ...
  - 💰 **경제는 `hbRwNorm(D)` 이 지킨다 — 던전의 평균 처치 보상은 늘 1.0.** `rw` 는 던전 안에서의 상대 크기만 뜻하고, 편성을 어떻게 바꿔도 시급(→`umRate()`→유즈맵 앵커)이 안 움직인다. *왜 '보상÷체력'이 아니라 '평균 보상'인지, 실측이 어떻게 나왔는지는 `BALANCE.md` §6 이 단일 소스다.*
