...
| | |
|---|---|
| 칸을 여는 것 | **최대 도달 라운드**(`campRuneBestRound()`) — 통산으로 편다(D2 R10 = 60) |
...
⛔ **전제 넷 중 하나라도 무너지면 젬이 곧 지수 축이 된다.** ① 칸은 라운드가 연다(젬이 아니다)
...
#### ⚔🏰 진군 규칙 — **칠 건물이 있으면 적을 쫓지 않는다** (2026-09-09)

`campStepUnits` 의 이동 패스에 조건이 하나 붙어 있다(`_march`). 아군에게 **칠 적 건물이 있으면**,
사거리 밖의 적 유닛은 표적으로 잡되 **쫓아가지 않는다** — 이동 목표는 건물이다.

⛔ **빼지 말 것.** 빼면 던전이 통째로 멈춘다. 이유는 두 목표의 자가 다르기 때문이다:

| 목표 | 자리 제한(`CAMP_ENG_OUT` 1200) |
|---|---|
| 적 **유닛**(`campGoalFor`) | **자른다** — 제자리 방어의 자 |
| 적 **건물**(`campBldGoal`) | 안 자른다 — 자르면 앞으로 못 나간다 |

그래서 적이 하나만 보여도 아군이 자리 쪽으로 되돌아갔다가, 적이 죽으면 다시 나아가기를 반복한다.
옛 라운드에는 「숨 고르기」가 있어 그 사이에 도착했지만, **릴레이는 적이 끊이지 않는 흐름**이라
그 틈이 영영 안 온다.
...
| | 진행 건물 | 건물 실효 화력 |
|---|---|---|
| 고치기 전 | **2/6** (60초 뒤 사거리 안 0/12 · 문지기가 90/90 으로 120초) | 이론의 **10.8%** |
| 고친 뒤 | **5/6** | **31.2%** |
...
**아군은 죽지 않는다.** 빈사로 누웠다가 **고정 30초** 뒤 그 자리에서 체력 만땅으로 일어난다.
그래서 §6-6 의 「가동률」(생존÷사이클)이 성립한다 — 사거리가 긴 유닛일수록 덜 눕고 더 오래 싸운다.

⚠️ **죽은 유닛은 전장 배열에 남지 않는다.** `strikeStepUnits` 끝에서 `me.units=me.units.filter(u=>!u.dead)`
로 걷어낸다(`18-strike.js:1301` · 공유 파일이라 ⛔ 못 고침). **'배열에 남아 있다'고 가정하면 안 된다** —
실제로 그렇게 짰다가 한 번 헛돌았다. 그래서 캠프가 **걷히기 전후를 비교해** 붙잡는다:

```
const _b4 = CAMPB.me.units.slice()   // 스냅샷
campWithStk(strikeStepUnits)          // 여기서 죽은 것이 걷힌다
campCatchDown(_b4)                    // 사라진 것 = 이번 프레임에 누운 것 → CAMPB._down 에 보관
campReviveStep(dt)                    // 타이머가 다 되면 배열에 되돌린다
```

객체는 배열에서 빠졌을 뿐 살아 있으므로 그대로 들고 있다가 되살린다.

⭐ **패배 규칙이 함께 바뀌었다.** 부활이 생긴 뒤로 **「전멸」은 패배가 아니다** — 다 누워도 일어난다.
패배는 **본부 파괴**뿐이고(`HUNT_R1` §6-5), 예외로 **되살릴 유닛이 하나도 없을 때**(출격 병력 0)만 진다.
그 예외가 없으면 라운드가 영영 끝나지 않는다.

| 상수 | |
|---|---|
| `CAMP_REV_S` 30 | 기본 부활 시간(초) — 유닛 종류와 무관 |
| `CAMP_RT_REV` | 트리 `rebuild` 단축률 25/50/75/90/100% |
| `CAMP_REV_MIN` 3 | ⛔ 하한 — 0 이면 눕는 것이 무의미해진다 |
...
> | **army**(아군 강화) | **7/8** | `campScaleAllies`(atk·hp) · `campBattleOpen`(bldg=본부 체력) · `campCombatStep`(skCd) · `campApplySupCap`(sup) · `campUpgDisc`(upCost) · **`campReviveSec`(rebuild = 부활 단축)** |
...
>
> ⭐ **`rebuild`(자동 재생산)는 「부활 단축」으로 읽었다**(2026-08-25). 설계의 「죽은 유닛 n% 자동 재구매」는
> '미네랄을 깎나'가 계속 애매했다. 부활 시간은 적 갈래의 「적 부활 시간」과 대칭이고 §6-6 의 **가동률**에
> 곧바로 붙어 효과가 읽힌다. 25/50/75/90/100% 단축 · ⛔ 하한 `CAMP_REV_MIN` 3초(즉시 부활이면 눕는 것이 무의미).
>
