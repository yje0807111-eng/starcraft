...
- **📈 성장 곡선(2026-08-19 전면 재설계)** — 사냥터는 **주 무대가 아니다**. 유즈맵이 중심이고 여기는 눌러 놓고 떠나는 곳이라, 자주 눌러야 하는 것은 **사냥터 업그레이드 하나**뿐이다. 나머지는 느리거나 자동이다.
  ...
  - **환생**: 요구 레벨은 **고정 사다리 `PROF_REB_LEVELS`(7칸)** 이고 마지막 칸이 곧 **레벨 상한 `PROF_LV_CAP`**. 2회차부터 **유즈맵 포인트 관문**(`PROF_REB_POINT`)이 붙는다. ⚠ 한때 '수식이라 끝이 없다'로 바꿨다가 되돌렸다(2026-08-19) — 상한이 없으면 경험치 곡선·RP 복리가 전부 성립하지 않는다.
