...
**남은 것.**
- 🚶 3단계(길막기) — `window._techNavRects` 를 감싸(캠프 전용 · `campPatchHit` 과 같은 자리) 지형 사각형을 덧붙인다.
  ⚠ 칸 하나당 사각형 하나로 주면 A\* 가 터진다(`_techFindPath` 는 꼭짓점 전부를 노드로 쓴다) — **큰 덩어리로 합칠 것**.
  ⚠ 그전에 **건물을 장애물로 만들었다가 아군이 갇힌 적이 있다**(19-camp.js 3017) — 원인은 길찾기 부재였고
  지금은 `campMove` 가 `_techFindPath` 를 쓴다. 벽도 반드시 그 경로를 타야 한다.
- ⚠ **2단계까지는 앞뒤가 조금 안 맞는다** — 절벽이 시야는 막는데 **걸음은 안 막는다**(유닛이 절벽을 뚫고 오른다).
  3단계에서 풀린다.
