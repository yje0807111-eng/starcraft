...
🏰 **건물에 닿은 유닛은 적보다 건물을 먼저 친다** — 사격 판정에서 건물이 유닛보다 **앞**에 있다.
⛔ 빼지 말 것: 릴레이는 적이 끊이지 않는 흐름이라 「적을 다 잡고 나서」가 영영 안 온다
(실측: 본진이 60초에 73 = 이론 화력의 **2%**). ⭐ 이 한 줄이 **분업**을 만든다 —
앞줄(건물에 닿은 유닛)은 벽을 치고 뒷줄은 적을 막는다. **자리가 역할을 정한다.**
