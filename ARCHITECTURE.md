...
- ⚠ **성장 축 통일이 두 갈래로 들어와 있다(미해결, 2026-08-12).** 같은 결정("레벨 포인트 폐지 → 미네랄로 통일")을 두 세션이 각각 구현했다.
  - `migrateProfile` v6 — 배분 스탯을 대응 업그레이드 레벨로 옮기고 남은 포인트는 미네랄 환급(1pt=20). 다만 **`c.unit.stats`를 0으로 만들지 않아** 옮긴 뒤에도 그대로 남는다(파워에 이중 계상).
  - `profGainStats` — 레벨업 때 직업 성장 프로필 비율로 `c.unit.stats`를 자동으로 올린다. 즉 스탯 축이 살아 있다.
  - 둘 중 하나로 정리해야 한다. 전자를 택하면 `profGainStats`를 없애고 레벨업 보상을 미네랄로, 후자를 택하면 마이그레이션에서 `unit.stats`를 비워야 한다.
