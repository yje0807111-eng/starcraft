...
| ③ | **회복 존** — 내 땅에 있으면 **5초마다 체력 가득**(유닛·건물) | `campHealZone` |
...
⭐ **③의 경계는 안개와 같은 자**(`techY0` · 격자 위끝 0.18 = 전장 y 0.50W)를 쓴다. 화면에서
「안개가 없는 땅」이 곧 회복되는 땅이라 규칙이 눈에 보인다. ⛔ 별도 경계값을 새로 두지 말 것.
판단의 단일 소스는 `campInHome(u, W)` 하나다.
