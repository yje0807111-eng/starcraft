...
**지금 상태(1단계 = 그림만).** 시야 차단·길막기는 **아직 배선 안 했다**:
- 👁 2단계 — 안개 엔진의 `f.height` 를 지형에서 채운다. `_fogReveal` 의 고저 차단은 **이미 동작한다**(지금은 `flat:true`).
- 🚶 3단계 — `window._techNavRects` 를 감싸(캠프 전용 · `campPatchHit` 과 같은 자리) 지형 사각형을 덧붙인다.
