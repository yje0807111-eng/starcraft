...
- **🎯 레벨 포인트(`LP_STATS`)** — 레벨업으로 얻어 **배수**에 직접 찍는 축. 총량 = `(레벨-1)×LP_PER_LEVEL`, 저장은 `c.unit.pts`. ⚠ 옛 배분(`c.unit.stats`)과 **다른 필드**다 — 그건 마이그레이션이 0으로 비운다. 배수는 `lpMul(k)` 하나만 부르고, 전투 반영은 **네 곳뿐**이다: `hbCharStats`(내 수치·치명타 피해) · `hbAllyMul`(동료·펫·터렛) · `hbBunkerAtkMul`(벙커) · 그리고 실제 피해식의 `c.critDmg`. 항목을 추가하면 반드시 이 중 한 곳에 배선할 것(스모크가 표의 전 항목이 실제로 수치를 바꾸는지 검사한다).
