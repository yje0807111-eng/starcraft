...
- **유즈맵 화면은 상자 하나다**(2026-08-18). `.msPanel` 안에 **정렬 띠(`#msSortTabs`) → 목록(`#msList`) → 소셜 도크(`#msSocialDock`, 채팅·친구·파티)** 가 차례로 들어간다. 소셜은 **자기 상자를 그대로 두고**, 도크의 안여백(`padding:7px 8px 8px`)이 목록에서 띄운다 — 맞닿게 두면 친구 줄이 지도 카드에 그대로 붙어 보였다. 좌우 8px 은 `.msList` 와 같아 소셜 상자·지도 카드·정렬 띠 세 변이 모두 맞는다(27~363). 스모크가 셋이 같은 상자 안인지, 목록과 소셜 사이 여백이 있는지, 좌우 변이 맞는지 검사한다.
