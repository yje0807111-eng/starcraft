...
- **📈 성장 곡선(2026-08-19 전면 재설계)** — 사냥터는 **주 무대가 아니다**. 유즈맵이 중심이고 여기는 눌러 놓고 떠나는 곳이라, 자주 눌러야 하는 것은 **사냥터 업그레이드 하나**뿐이다. 나머지는 느리거나 자동이다.
  - **경험치**: `PROF_XP_A·lv^PROF_XP_P` — **다항식 하나**(등비 구간 없음). ⛔ 지수로 되돌리지 말 것: 몇백 레벨에서 물리적으로 막혀 만 레벨을 못 간다. **레벨 상한 없음**(`PROF_LV_CAP` 삭제).
  - **환생**: 회차 N의 요구 레벨 = `PROF_REB_A·N^PROF_REB_P` — **수식이라 끝이 없다**(1→20 · 10→796 · 50→1.0만). ⛔ 고정 배열로 되돌리지 말 것(마지막 칸이 곧 레벨 상한이 된다).
  - **환생 보상 = 두 배수**: `profXpMul`(경험치) · `profCoinMul`(미네랄 획득). 둘 다 회차의 **누적합**(1+2+…+N)에 비례해 갈수록 크게 벌어진다. ⚠ 저장하지 않고 `c.reb` 에서 늘 다시 센다 — 계수를 바꾸면 소급 적용된다.
...
- **🔁 환생 포인트(`unit.rpts` / `rpMul`)** — 환생 1회당 `PROF_REB_RP` 점(회차와 무관) + 기록 보너스(`profRecordRp`, 전역 진행도 `PROF_REC_RP_PER` 칸당 1점). **선형 배수**(1p=+2%), 환생해도 남는다. ⛔ 복리로 되돌리지 말 것 — 회차가 수백까지 가므로 그대로 폭주한다(복리는 환생이 7회뿐이던 시절 설계였다).
- **🤖 자동 배분(`c.lpAuto` = 대상 축의 키, `''`=끔 · 기본 `LP_AUTO_DEFAULT`)** — 레벨업 때 **미리 골라 둔 한 축에만** 계속 찍는다(`lpAutoSpend`). 고르는 건 플레이어 몫이고 그 뒤로는 안 들러도 된다. 버튼(`lpAutoCycle`)은 `끔 → 첫 항목 → … → 끔` 으로 한 칸씩 돌린다 — 사냥터 수량 버튼(×1/×10/MAX)과 같은 방식이다. ⚠ 옛 저장은 `0/1` 이라 `fixChar` 가 키로 옮긴다.
