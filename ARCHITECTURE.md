...
- **유즈맵 소셜 카드 테두리 = 금속 링**(`--metal-ring`, 회색). 탭 띠(`.pdSeg`)·하단 네비와 같은 값이라 화면 안 재질이 한 갈래로 모인다. ⚠ 그라데 테두리는 `padding-box`/`border-box` **2겹 배경**이라야 나온다 — `border` 는 `transparent` 로 비워 둔다(색을 칠하면 링이 안 나온다). 마을 시트로 돌아갈 때는 `.inTown` 이 테두리를 걷어내므로 이 규칙은 `#msSocialDock` 안에서만 건다.
