...
- **유즈맵 화면은 카드 두 장이다**(2026-08-18 · 최종). 위 = 목록 카드(`.msPanel` — 정렬 띠 + 목록), 아래 = 소셜 카드(`#msSocialDock` — 채팅·친구·파티). 소셜은 목록 카드 **안에 들어가지 않는다**.
  ...
  - 소셜 카드의 **모양(면·테두리·라운드)은 도크가 직접 갖는다** — 안의 `.msSocial` 은 껍데기를 벗겨 둔다(카드 안에 카드가 겹치면 테두리가 두 줄로 보인다). 값은 목록 카드와 같은 토큰이라 두 카드가 같은 재질로 읽힌다.
  - 두 카드의 좌우 변은 같고, 사이는 도크의 `margin-top:17px` 이 띄운다.
