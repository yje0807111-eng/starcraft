...
| **하단 프로필 「고른 것 없음」** | 캠프는 시트를 늘 열어 두는데, 아무것도 안 고르면 `campSyncSheet` 가 **본부를 대신 골라** 준다 — 그래서 「고르지 않은 상태」가 없다. 그 자리에 들어갈 **기지 요약**을 `_campIdleModel()`/`renderCampIdleSheet()` 로 만들어 뒀다(`js/11-cmdcard.js` · 공용 `renderCmdGrid` 를 그대로 쓴다 · 캠프 상태는 **읽기만**). ⚠ **아직 안 이어져 있다** — `19-camp.js` 의 본부 자동 선택 세 줄을 `renderCampIdleSheet()` 한 줄로 바꿔야 화면에 나온다(그 파일은 다른 작업자 영역) |
