...
- **🎯 레벨 포인트(`LP_STATS` / `unit.pts` / `lpVal`)** — 레벨업으로 얻는다. **덧셈이다**: 1포인트 = 그 축의 사냥터 업그레이드 1레벨과 같은 양(`HB_UPG[S.upg].vs × LP_GAIN`). 값을 표에 적지 않고 `HB_UPG` 에서 읽으므로 카드에 적힌 수와 항상 같아진다. 세기 조절은 `LP_GAIN` 하나. 총량 = `(레벨-1)×LP_PER_LEVEL`, **환생하면 레벨과 함께 되감긴다**. ⛔ 배수(`1+n×step`)로 되돌리지 말 것 — 1점 +50%였을 때 Lv11에 한 축이 6배가 돼 전투가 무너졌다(2026-08-19).
  - CS 축(공격력·체력·…)은 `csAxis` 에서 `base+upg+gear+lp` 로 더해지고, 아군 축(동료·건물)은 `hbAllyMul`/`hbBunkerAtkMul` 에서 **업그레이드 레벨에 포인트를 더해** 반영된다. 스모크가 '포인트 N점 = 업그레이드 N레벨' 을 실측으로 검사한다.
