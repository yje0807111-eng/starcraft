...
#### 🗺 단계와 라운드 (2026-08-25 · 1단계)

**0단계 = 캠프**(적 없음) · **1~10단계 = 던전**(적이 내려온다) · 던전 하나 = **50라운드**.
화면은 하나뿐이다 — 단계가 바뀌어도 기지·광맥·일꾼은 그대로고 적만 달라진다.

| 함수 | 하는 일 |
|---|---|
| `campDgN()` / `campRoundN()` / `campCleared()` | 지금 단계 · 도전 중인 라운드 · 깬 라운드 수 |
| `campMineMul()` | 지금 미네랄 배율 — **탭과 일꾼 양쪽에** 걸린다 |
| `campEnterDungeon(n)` | 캠프 → 던전 진입 |
| **`campClearRound()`** | 라운드 하나 클리어. 50 채우면 **다음 던전으로 자동 이동** |
| `campFail()` | 졌다 → **캠프(0)로 탈락**. `best` 는 남는다 |

⭐ **배율은 라운드를 「클리어해야」 붙는다** — 50라운드면 **50번**(49번이 아니다). 그래서 증가량이
전부 딱 떨어진다(+0.02 · +0.06 · +0.2 · +1.2 · +6 · +28 · +180 …). 자세한 이유는 `HUNT_R1.md` §6-1-0-2.

⛔ **`CAMP_MINE` 표를 공식으로 바꾸지 말 것.** 옛 `×2^(단계-1)` 공식은 단계 5부터 문턱에서 배율이
**내려갔다**(`HUNT_R1.md` §6-1-0-1-1). 스모크 「캠프 던전: 단계·라운드·미네랄 배율」이 표 불변식을 지킨다.

⚠ **`C.cleared` 를 `campClearRound()` 밖에서 만지지 말 것** — 자동 이동과 `best` 기록이 거기 붙어 있다.
...
설계 단일 소스는 `HUNT_R1.md` §4-4(구조·비용) · §4-5(32계열). **시작점 하나에서 사방 넷으로**
퍼지고, 계열 하나가 **티어 5곳에 등장**한다. **산 노드에 붙은 것만** 살 수 있다(사슬).

```
CAMP_RT_LINES      32계열 — 갈래(start/econ/army/enemy) 8개씩
CAMP_RT_GRP        등장 티어 묶음 가[1,5,9,13,17] 나[2,6,…] 다[3,7,…] 라[4,8,…]
campRtTier(k,n)    계열 k 의 n차가 몇 티어인가
campRtGrade(k,n)   등급 — 귀함 계열은 자기 이정표 티어에서만 극상
campRtCost(k,n)    = 2 × 4^(티어-1) × 등급배수(흔함 .5 / 보통 1 / 귀함 3 / 극상 10)
campRtCanBuy/Buy   사슬 규칙 + 포인트 검사        저장 = C.rbTree{ root:1, 계열:차수 }
campRtReset()      전부 물리고 **100%** 환급 — 비용은 젬(GEM.md §4)
```

**32계열 × 5등장 = 노드 160개**(+시작점) · **티어마다 정확히 8개** · **극상은 갈래마다 하나씩**.
셋 다 스모크가 세어서 지킨다 — 계열 표를 손대면 바로 걸린다.

⛔ **적 약화 갈래는 두 겹으로 막혀 있다**(`HUNT_R1.md` §4-5-4).
계열마다 **−40%**(`CAMP_RT_CUT` 표) · 갈래 전체 실효 **하한 ×0.2**(`campRtFoeMul`).
**하한을 빼면 그 즉시 지수 축이 둘이 된다** — 전부 찍으면 ×0.017 까지 내려간다(실측).

⛔ **감소 폭을 공식으로 만들지 말 것.** 지수 감쇠는 5차에서 상한에 **정확히** 닿지 않는다
(실측 −37.99%). 「다 찍었다 = −40%」가 성립해야 하므로 표를 그대로 둔다.

**지금 실제로 걸려 있는 효과는 넷뿐이다** — `tap`/`tapMul`(탭) · `gather`(일꾼) ·
`foeN`(적 마리 수) · `foeHp`/`foeAtk`… 같은 적 약화 갈래. 나머지 계열은 **선언만 돼 있다**
(자동 재생산·시작 건물처럼 아직 없는 시스템을 건드린다). 붙일 때 `f` 키로 찾을 것.

#### 🌳 트리 화면 `#campTree` — 마인드맵 (2026-08-25 · 6단계-b)

시작점이 가운데, 갈래 넷이 사방으로. 계열 하나가 바깥으로 5칸 사슬을 이룬다.
**밀고(드래그) 확대(핀치)** 해서 본다 — 160노드를 세로 화면에 다 못 담기 때문이다.

```
campTreePos(k,n)   월드 좌표 — 갈래 각 + 부챗살 + 반지름. ⛔ 화면 픽셀로 잡지 말 것
campTreeState(k,n) own / buy / next(포인트 부족) / lock
campTreeSvg()      선 → 갈래 이름 → 시작점 → 노드 순서로 그린다(뒤에 깔릴 것부터)
campTreeOpen/Close/Render/Tap/BuySel
campTreeApplyView()  <g> 의 transform 만 바꾼다 — 확대·이동이 좌표 계산과 안 섞인다
```

| 규칙 | |
|---|---|
| ⛔ **`#phone` 직속** | `#vBuild` 안에 두면 재화 바(z=62)·시트가 위에 뜬다 — 실제로 그랬다 |
| 갈래 색 | `CAMP_TREE_BR` 이 단일 소스. `DESIGN.md` §2 역할표 그대로(재화=금·위험=적·정보=청·긍정=녹) |
| ⛔ 시안 `--acc-sel` | 「지금 고른 노드」 **한 곳** 전용 — 갈래 색으로 쓰지 말 것 |
| 조작 | 한 번 누르면 고르고, **다시 누르면 산다**. 민 뒤(8px 초과)에는 안 눌린다 |
| ⚠ `CAMP_TREE_R0` | 줄이면 8계열의 1차가 한 자리에 모여 **링처럼** 보인다(부챗살 느낌이 죽는다) |
| ⚠ 갈래 이름 | 사슬 **중간**에 둔다 — 끝에 두면 기본 배율에서 화면 밖이라 아무도 못 본다 |
| ⚠ 처음 배율 | `z=0.55` — 전체가 보여야 「이게 다인가」 하지 않는다 |

⛔ **캠프 맵의 팬·줌(`campPatchZoom`)을 빌리지 말 것** — 그건 건설 격자 좌표계에 묶여 있다.
여기는 SVG `viewBox` 하나뿐이라 훨씬 단순하다.

입구는 `#campBar` 의 🌳 칩이다. 배지 전체가 `pointer-events:none` 이라 **그 칩만** 되살려 놨다.

#### 🔁 환생 (2026-08-25 · 5단계)

설계 단일 소스는 `HUNT_R1.md` §4. 관문은 **매번 같고**(그 회차 재화점수 100만),
배수는 **로그**, 포인트는 **제곱근 × 깊이**다.

```
campWealth()      = C.earn + C.earnGas × 8        그 회차에 번 것
campCanRebirth()  = campWealth() >= 100만          ← 회차가 늘어도 안 오른다
campRebMulGain()  = max(0.2, 0.8 × log10(난이도))  → C.rebMul 에 **더한다**
campRebPtGain()   = √(재화 ÷ 100만) × 1.35^(던전-1) × 1.012^(깬 라운드)
campRebMul()      = 1 + C.rebMul                   → 탭·일꾼 양쪽에 걸린다
```

⭐ **관문 100만과 포인트 공식의 기준선은 같은 숫자다.** 그래서 관문을 채운 그 순간 기준량이
정확히 **1** 이고, 「지금 환생할까 더 벌고 환생할까」가 이 한 숫자에서 나온다.
재화를 **100배** 벌어야 포인트가 **10배** — 무한정 버는 것이 최적해가 되지 않는다.

⛔ **배수를 곱으로 두지 말 것** — 합이라야 지수 축이 하나로 남는다(`BALANCE.md` §0).

**되감기에서 두 번 틀렸다 — 둘 다 스모크가 잡았다:**
1. `campSave()` 는 **`G.tech` 를 `C` 로 복사한다.** 저장 상태만 되감고 저장하면 방금 지운 것이
   그대로 되살아난다 → **`campWipeBoard()` 로 살아 있는 판도 같이 비운다.**
2. 판을 새로 깔면 저장을 다시 읽을 수 있어 **남겨야 할 값(최고 기록·배수·포인트)이 날아간다**
   → 비우기 전에 손에 쥐고, 비운 뒤 다시 얹는다.

⚠ **환생 뒤에는 `campState()` 를 다시 읽을 것** — 판을 다시 까는 과정에서 상태가 갈릴 수 있다.

⚠ **누적은 `campApplyGatherMul()` 한 곳에서 센다** — 배수를 다 먹인 뒤의 실제 증가분이다.
지출은 빼지 않는다(「얼마나 벌었나」가 기준이지 「지금 얼마 있나」가 아니다).

🚧 **통신소 스캔은 아직 입구로 안 붙었다.** 통신소는 유니온 테크에만 있어 그대로 게이트로 쓰면
다른 종족이 통째로 막힌다 — UI 를 붙일 때 함께 푼다. 지금 조건은 재화점수 하나다.
...
#### 🗺 단계·라운드는 **칩**, 맵 띠는 적 수만 (2026-08-25 · 통합)

⛔ **던전·라운드를 두 곳에 두지 말 것.** 두 세션이 각자 만들어 중복이었다 —
`#curTitle` 칩(`js/12-appshell.js`)과 `#campBar` 띠가 같은 것을 보여 줬다. 칩을 남겼다.

| | 무엇을 보여주나 | 어디 |
|---|---|---|
| **칩 `#curTitle`** | 던전 이름 · **라운드 n/50** · 진행 막대 · **이동 드롭다운** | 재화 바 왼쪽 · `curPaintChip()` |
| **띠 `#campBar`** | **적 수** · 🌳 트리 입구 | 맵 위 · `campBarRender()` |

**상태는 캠프가 단일 소스다** — 칩은 `campDgN()`/`campRoundN()`/`CAMP_ROUND_MAX` 를 **읽기만** 한다.
`C.rnd` 는 `C.cleared` 를 **비추는 값**이고, 드롭다운 [이동]도 `C.cleared` 에 쓴다.

⚠ **`CAMP_RND_MAX`(12-appshell)는 `CAMP_ROUND_MAX`(19-camp)를 베낀 값**이다 —
12-appshell 이 먼저 로드돼 참조할 수 없어서다. **갈리면 스모크가 잡는다.**

⛔ **`CAMP_DG_MAX` 는 `12-appshell.js` 에만 선언한다.** 고전 스크립트라 전역이 하나뿐 —
양쪽에 두면 `19-camp.js` 가 통째로 안 읽힌다(머지에서 실제로 그랬고 검사 10개가 딸려 죽었다).

**띠 규칙** — 매 프레임 불리므로 바뀐 것만 쓴다(`_campBarS`) · 떠날 때 `campBarReset()` ·
전체는 `pointer-events:none` 이고 🌳 칩만 되살린다 · 보여줄 게 없으면 `.empty` 로 숨는다 ·
z-index **9**(맵 층 6~8 위) · `top:calc(var(--topPad) + var(--curH) + 8px)`(재화 바 아래).

#### 📈 적 난이도 곡선 (2026-08-25 · 4단계)

```
라운드 밑  = 1.07 + (던전-1) × 0.003        campRBase(dg)
던전 문턱  = (앞 던전 라운드 밑)^49 × 3      campDgThreshold(dg)   ← 어느 던전에서나 ×3
적 난이도  = Π(문턱) × (라운드 밑)^(깬 수)   campFoeDiff(dg, cleared)
```

**총 유입량은 난이도가 정하고, 몇 마리로 쪼갤지는 라운드가 정한다**(`HUNT_R1.md` §6-2-1).
기본값 체력 3.3 · 공격 0.33 에 난이도를 곱한 것이 그 라운드의 총량이고,
마리 수는 `min(100, 3 × 1.10^(라운드-1))`.

⭐ **`campScaleFoes` 는 개체 값을 덮어쓰지 않는다** — 무리 전체의 기본값 합 대비 배율로 민다.
그래야 탱크가 마린보다 단단하다는 유닛별 차이가 살아남는다.

⛔ **난이도를 미네랄(`CAMP_MINE`)과 같은 식으로 묶지 말 것.** 보상은 라운드마다 조금,
난이도는 크게 오른다. 묶으면 50라운드를 돌아도 적이 1.33배인데 아군 화력은 20배가 된다.

⚠ **스모크에서 상수를 상수 자신과 비교하지 말 것** — 문턱 검사를 `CAMP_DG_STEP` 와 비교했다가
상수를 1 로 바꿔도 통과했다(헛도는 단언). 설계값 **3** 을 그대로 박아 둔다.
...
| **좌상단 던전 칩** | 재화 바 왼쪽 빈 슬롯(`#curTitle`)에 얹는다 — 화면을 새로 만들지 않는다. 그리는 곳은 **`12-appshell.js` 의 `campChipInfo`/`curChipHTML`/`curPaintChip` 셋뿐**이고 `updateCurBar()` 가 부른다(캠프가 수입마다 그걸 부르므로 타이머가 없다). ⚠ 라운드(`C.rnd`)는 **드롭다운이 만든다**(2026-08-25) — 그전까지 캠프에는 없던 값이라, 없으면 `campEnsureRnd` 가 1 로 깐다. 라운드가 있으면 칩 둘째 줄이 「라운드 n/99」, 없으면 「던전 3/10」이다. ⚠ **라운드는 아직 게임플레이에 아무 영향이 없다** — 값을 만들고 저장·표시만 한다. ⚠ 칩 높이는 재화 바(`--curH`=34px)를 넘으면 안 된다 — 두 줄을 그냥 쌓으면 44.7px 로 삐져나온다(스모크가 잰다). ⚠ **폭도 예산이다(110px)** — 좌상단은 재화 바와 한 줄을 나눠 쓴다. 진행 막대를 라운드 오른쪽에 두었더니 칩이 134px 이 되어 **☰ 가 화면 밖으로 밀렸다** → 막대는 **칩 밑변**을 가로지르는 한 줄이다(테두리 **안쪽** · 좌우 6px 씩 잘라 판 안에 앉힌다 — 테두리 자리에 두면 둘 다 청록이라 붙어 보이고 진행도가 안 읽힌다), 라운드 줄은 **밑선 정렬**이고(글자 크기가 9.5/12/11px 로 달라 가운데로 맞추면 어긋나 보인다 · ⚠ rect 로는 못 잰다 — 밑선이 맞아도 하강부만큼 아래가 벌어진다), 칩은 `min-width:0` + 이름 말줄임으로 **재화가 커지면 알아서 줄어든다**(재화 바는 자릿수에 따라 261~305px 까지 자란다). 생김새는 `docs/mock/camp-dungeon-onechip-8.html` 7안 |
| **던전·라운드 드롭다운** | 칩을 누르면 **칩 아래로 자란다**(팝업·전체 화면 아님 · `campDropOpen`/`campDropRender`/`campDropGo`). 넓어지는 것은 판뿐이고 칩은 내용 폭 그대로다 — 칩까지 넓히면 오른쪽 재화를 덮는다. 함정 넷: ① 판은 칩의 **형제**다(칩 안에 두면 `updateCurBar` 가 칩을 다시 그릴 때 날아간다) ② 판 배경은 **불투명**이어야 한다(큰 판이라 3% 만 비쳐도 뒤 글자가 읽힌다) ③ 「지금 여기」 띠를 음수 `left` 에 두면 `overflow-y:auto` 가 x 도 auto 로 만들어 **통째로 잘린다**(`.cgTopOut` 과 같은 함정) ④ 라운드 가운데 선은 스크롤 상자 **밖**에 둔다(안에 두면 absolute 라도 내용과 같이 굴러 사라진다). ⛔ 던전 잠금을 여기서 짓지 말 것 — `CAMP_DG_UNLOCK='barracks'` 는 **유니온에서만 맞다**(swarm=pool · aetherial=gateway · feral=huntpen · colossus=assembly). 규칙이 정해지면 `campDgOpen()` 하나만 채운다. 생김새는 `docs/mock/camp-dgdrop-8.html` 2안 |
| **하단 프로필 「고른 것 없음」** | 캠프는 시트를 늘 열어 두는데, 아무것도 안 고르면 `campSyncSheet` 가 **본부를 대신 골라** 준다 — 그래서 「고르지 않은 상태」가 없다. 그 자리에 들어갈 **기지 요약**을 `_campIdleModel()`/`renderCampIdleSheet()` 로 만들어 뒀다(`js/11-cmdcard.js` · 공용 `renderCmdGrid` 를 그대로 쓴다 · 캠프 상태는 **읽기만**). 값 여덟 개를 **전폭 4열 두 줄**로 편다(`m.wide` + `statsWide`) — 빈 슬롯 4칸이 의미 없는 카드다. 머리줄은 **자간 넓은 작은 라벨**(`m.kicker` · `MY BASE`) — ⛔ 제목에 던전 이름을 쓰지 말 것, 좌상단 칩이 이미 말한다. ⚠ 두 옵션(`wide`·`kicker`)은 **안 쓰면 지금까지와 똑같이** 동작하므로 다른 시트에 영향이 없다. ✅ **이어졌다**(2026-08-25) — `campSyncSheet` 의 본부 자동 선택을 `renderCampIdleSheet()` 로 바꿨다. 함정 둘: ① 높이 클래스 **`.simple` 을 직접 붙여야** 한다 — `techPanelRender` 는 자기 모델이 있을 때만 붙이는데 요약은 그쪽 모델이 아니라서, 안 붙이면 시트가 내용대로 커져 **기지를 가린다**. ② `renderCampIdleSheet` 는 서명뿐 아니라 **지금 그려진 것이 요약인지**도 본다 — 건물을 보다가 해제하면 값이 그대로라 서명이 같아 영영 안 그려진다. ⚠ `campHQ()` 는 안 쓰이게 됐지만 **지우지 않았다**(유보 규칙) |
| **🧭 가이드 퀘스트** | 「이 게임을 어떻게 하는가」를 **15단계 순서**로 가르친다(`GUIDE_STEPS` · `js/06-daily.js`). 일일 퀘스트와 달리 **한 번만** 돌고 순서가 있다 — 앞 단계를 끝내야 다음 계측을 받는다. 화면에는 **지금 할 일 한 줄만**(`.guideBar` · `#phone` 직속 · 재화 바 아래), 전체 목록은 더보기 ☰ > 가이드(`#hbGuideSheet` · 일일 퀘스트와 같은 껍데기). ⚠ **유니온 전용이다** — 건물 키가 종족마다 달라(union=barracks · swarm=pool · aetherial=gateway · feral=huntpen · colossus=assembly) 다른 종족이면 `guideOn()` 이 false 를 준다. ⚠ **진행을 세는 곳은 캠프다** — 여기는 입구(`guideNote(kind,n)`)만 갖는다. 지금 이어져 있는 것은 던전 이동 하나뿐(`campDropGo`); 나머지는 캠프가 `guideNote('build:barracks',1)` 같은 한 줄씩 넣어야 차오른다 |
| **☰ 더보기** | 캠프에서 **실제로 도는 것만** 남겼다 — 가이드 · 출석 · 부스트 · 설정(`HB_MORE` · `js/05-home.js`). 뺀 것: 마을(상점은 하단 네비와 중복 · 나머지는 유보) · 성장(§5-A) · 토벌(§5-D) · 일일 퀘스트(14종 중 9종이 옛 사냥터 계측이라 캠프에선 안 찬다) · **건설(이미 죽은 칸이었다 — `hbBuildStart()` 가 `if(!_hb) return`)**. ⛔ 코드는 전부 살아 있다(길만 닫았다). ⚠ **부스트는 남겼지만 지금 효과가 없다** — 배율이 옛 사냥터 전투에만 걸려 있고 캠프 수급(`campTapGain`/`campGatherMul`)에는 안 들어간다 |
...
- **🏁 던전 = `HB_ROUND_MAX`(50)라운드짜리 챕터다**(2026-08-18 · **2026-08-25 에 99 → 50**). 50을 깨면 `hbAdvanceDungeon()`이 자동으로 다음 던전 1라운드로 넘긴다(등반 모드만 — 반복은 그 라운드를 계속 돈다). ⭐ **던전 배수는 라운드 곡선을 50칸 이어 붙인 것**이다(`hbCurve(base,dg,round)`) — 경계가 계단이 아니라 '한 칸'이다. 전역 진행도 = `hbProg(dg,round)`. **던전 최초 진입에는 보너스(`HB_DG_ENTER`)** 를 준다 — 경계가 매끄러운 대신 '올라섰다'는 순간이 없어서다(`hunt.dgIn` 에 기록, 한 번뿐). ⚠ **내려오는 길도 있어야 한다**: `hbDie` 에서 라운드가 1 밑이면 `hbRetreatDungeon()` 이 이전 던전 마지막 라운드로 물린다 — 없을 때 자동 이동으로 올라간 던전 1라운드에 **영영 갇혔다**(실측 40시간 헛돔). ⛔ 옛 고정 배수(체력 8·공격 5·보상 24)로 되돌리지 말 것 — 라운드 상한까지 민 뒤 다음 던전이 8배로 떨어져 난이도가 통째로 무너진다. 던전 해금 `HB_DG_UNLOCK`도 라운드 상한과 같다. ⚠ 곡선이 지수라 재화가 던전 3에서 큰 수가 된다 → `CUR_SUF`가 3자리 단위로 Vg(1e63)까지 잇고 그 위는 지수 표기다. ⚠ **상한을 바꾸면 `BALANCE.md` §3 실측 표가 통째로 무효가 된다**(99에서 잰 값이다) — 다시 재고 갱신할 것. ⛔ **S자 리듬 주기(`HB_S_PERIOD`)를 이 상한에 다시 묶지 말 것** — 토벌(`dgHbStart`)이 같은 라운드 축을 `dg=1`로 빌려 쓴다. 99→50 때 토벌 20층 직접 클리어율이 **88%→13%**로 무너졌다(`BALANCE.md` §3-1 실측).
