...
- 직스 성능 비교는 **반드시 같은 세션 안에서** 구현을 런타임으로 갈아끼워 A/B 할 것. 리로드 사이에는 발열·GPU 상태·`resScale`·랜덤 유닛 구성이 달라져 편차가 효과보다 크다(리로드 비교로 "한계가 300→450으로 올랐다"고 잘못 보고한 적 있음).
- **프레임 시간을 ms 상수와 비교하지 말 것.** vsync 때문에 60Hz의 프레임은 아무리 가벼워도 16.7ms 아래로 안 내려간다. `strikeAutoQuality`의 복구 임계값이 15ms였던 탓에 한 번 낮아진 `resScale`이 유닛이 줄어도 영원히 복구되지 않았다(화면이 흐린 채로 고정). 지금은 관측된 최저 **평활** 프레임(`S._aqMin`=주사율 바닥)의 배수(`STK_AQ_BAD/GOOD`)로 판정한다 — 단발 이상치에 바닥이 끌려가지 않게 raw가 아닌 평활값의 최소를 쓴다.
