...
##### ⛔ 지금 진단 — 떨림의 주범은 **둘**이다 (2026-08-31 실측 · 30초 · 11기 · 연구 45)

| | 지금(캠프 개입) | `ENGAGE=0`(손 뗌) | 순수 오토배틀(기록) |
|---|---|---|---|
| 방향 뒤집힘 / 유닛 | **96.4회** | 40.1회 | **0.9회** |
| 사거리 안 비율 | 37.2% | 15.7% | — |
| 실효(낼 수 있던 화력 중 실제) | **0.50** | 0.32 | — |
| 목줄이 위치를 **자른** 횟수 | 5회 | **3760회** | — |

⭐ 원인은 상수가 아니라 **구조**다 — 캠프의 자리 제한 장치 둘이 **이동의 결과를 나중에 덮어쓴다**:
- `campEngageStep` — `u.x = u._sx` 로 오토배틀 이동을 **무르고** 제 목표로 다시 민다 → 96회
- `campLeash` — 600 경계에서 위치를 **직접 자른다**. 손을 떼면 이게 홀로 일해서 초당 4회씩
  끊어 당긴다(3760회 / 30초) → 40회. **손을 떼도 안 매끄러운 이유가 이것이다.**

⛔ 그래서 **간격·거리 상수를 돌려서는 못 고친다**(네 번의 실패가 그 증거다). 미는 주체를 하나로
만들어야 한다 — 오토배틀의 **목표 계산 단계에** 자리 제약을 넣고, 나중에 덮어쓰는 두 장치를 없앤다.
