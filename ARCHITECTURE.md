...
| `장비창(상=페이퍼돌 / 하=가방)` | `PROF_SLOT_ICON`(슬롯 라인아트 12종)·`_slotGlyph` · `PROF_GEAR_PAGES`/`profPageSlots`/`_profPageNav`(장비↔장신구 페이지) · `_profPaperdoll`(아바타 위 오버레이) · `_profGearGrid`(6열) / `_profGearInfo`(가방 위 팝업) · `_gearPick`/`_gearSel`/`_gearPage` · `bagScrollHint` |
