...
- **유즈맵 화면은 카드 두 장이다**(2026-08-18 · 최종). 위 = 목록 카드(`.msPanel` — 정렬 띠 + 목록), 아래 = 소셜 카드(`#msSocialDock` — 채팅·친구·파티). 소셜은 목록 카드 **안에 들어가지 않는다**.
  ...
  - 소셜은 **아예 다른 섹션**으로 보이게 간다 — 화면 좌우를 꽉 채우는 띠(`margin:17px -13px 0` 로 `#mapSelect` 의 좌우 여백 13px 을 되돌린다) · 라운드 0 · **실버 링**(`--metal-ring`) 1px · **검정 반투명 면**(`rgba(0,0,0,.55)`, 뒤 배경이 비친다). 목록 카드와 폭·면·테두리를 전부 다르게 둬야 '다른 섹션'으로 읽힌다.
  - ⚠ 면이 반투명이라 링을 `padding-box`/`border-box` 2겹 배경으로 그리면 **링이 면 뒤로 깔려 배경 전체가 회색이 된다**(앞서 그랬다). `::before` + `mask-composite:exclude` 로 테두리 1px 만 그린다.
