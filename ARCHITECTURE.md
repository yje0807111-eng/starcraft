...
- **🏁 던전 = `HB_ROUND_MAX`(99)라운드짜리 챕터다**(2026-08-18). 99를 깨면 `hbAdvanceDungeon()`이 자동으로 다음 던전 1라운드로 넘긴다(등반 모드만 — 반복은 그 라운드를 계속 돈다). ⭐ **던전 배수는 라운드 곡선을 99칸 이어 붙인 것**이다: `HB_DG_HP(dg) = HB_ROUND_HP^(99×(dg-1))`. 그래서 '던전 d 라운드 99 → 던전 d+1 라운드 1'이 한 칸 오른 것과 **정확히 같다**(스모크가 경계 이음새를 검사한다). 전역 진행도 = `hbProg(dg,round)`. ⛔ 옛 고정 배수(체력 8·공격 5·보상 24)로 되돌리지 말 것 — 99라운드(체력 240만 배)를 민 뒤 다음 던전이 8배로 떨어져 난이도가 통째로 무너진다. 던전 해금 `HB_DG_UNLOCK`도 99다. ⚠ 곡선이 지수라 재화가 던전 3에서 1e12를 넘는다 → `CUR_SUF`가 3자리 단위로 Vg(1e63)까지 잇고 그 위는 지수 표기다.
- **📈 라운드 난이도는 지수다**(2026-08-18) — `HB_ROUND_HP`(1.16) / `HB_ROUND_ATK`(1.13) / `HB_ROUND_REW`(1.14) / `HB_ROUND_XP`(1.06), 전부 `hbRoundK(mul, round)` 한 함수를 지난다. 순서(**HP > REW > ATK > XP**)가 곡선의 전부다: 시급은 오르되 조금씩 빡세지고, **XP가 제일 완만해야 레벨(=포인트)이 적 체력을 못 따라가 '벽'이 생긴다**. 옛 선형 곡선(체력 `14+5R`)은 성장이 배수가 된 뒤 몇 라운드 만에 무의미해졌다. ⚠ 새 라운드 스케일을 만들면 **반드시 `hbRoundK`를 쓸 것** — `hbChestHp`를 선형으로 남겨 뒀더니 라운드 60에서 상자가 적의 1/300이라 한 대에 깨졌다. 실측(던전 1 방치): 4분 R10 · 9분 R20 · 18분 R40 · 23분 R50 · 40분 R59.
...
- **🎯 레벨 포인트(`LP_STATS` / `unit.pts` / `lpMul`)** — 레벨업으로 얻어 배수에 찍는다. 총량 = `(레벨-1)×LP_PER_LEVEL`. **환생하면 레벨과 함께 되감긴다.**
  - ⭐ **상한 있는 축은 넘긴 몫이 버려지지 않는다**(2026-08-18) — `csAxis()`가 `over`(상한 초과분)를, `csOver(k)`가 '상한 대비 비율'을 낸다. 행선지 셋: **사거리→상자 피해**(`HB_OV_CHEST`) · **공격속도→멀티샷 확률**(`HB_OV_MULTI`) · **치명타 확률→치명 피해**(`HB_OV_CRITD`). 환생 포인트는 영구라 잘못 넣으면 되돌리기 어렵다 — '넣으면 손해인 축'을 남기지 않는 것이 목적이다. 공격속도는 상한이 쿨다운 하한으로만 막혀 있었어서 `CS_AXES.aspd.cap=HB_ASPD_MAX`를 명시했다(초과분을 셀 수 없으면 환산도 못 한다).
  - ⭐ **레벨 1회 = 포인트 1(`LP_PER_LEVEL`) · 1포인트 = +50%(`LP_STEP`)** (2026-08-18 재조정). 잘게 나눠 조금씩 세지는 대신 **한 레벨이 한 번의 도약**이 되게 했다 — 한 축에 10포인트면 ×6이다. ⚠ **항목별 step 을 다시 다르게 두지 말 것** — 어디 찍는 게 이득인지가 숨은 지식이 된다. 과열은 자연 상한이 막는다(공속 쿨다운 `HB_CD_MIN` · 사거리 `HB_RNG_MAX` · 치명타 60%).
  - ⚠ **이 값과 라운드 난이도 지수(`HB_ROUND_*`)는 한 세트다.** 한쪽만 만지면 곡선이 무너진다 — 성장이 배수라 옛 선형 난이도로는 몇 라운드 만에 저항이 사라졌다.
  - ⚠ 파생 피해: `profPower()`가 지수로 튄다. 방치 수입의 '첫 클리어 전' 폴백을 파워 기반으로 두면 **아직 한 판도 못 깬 쪽이 실제로 도는 쪽보다 수입이 높아진다**(실제로 그랬다) → `PROF_IDLE_BASE` 고정값으로 바꿨다.
- **🔁 환생 포인트(`unit.rpts` / `rpMul`)** — 환생과 기록으로 얻고 **환생해도 남는다**. ⚠ 레벨 포인트와 **다른 필드**다 — 같이 담으면 환생 때 같이 날아간다.
  - ⭐ **배수는 곱셈(복리)이다** — `rpMul(k) = (1+RP_STEP)^n`, `RP_STEP=0.06`. ⛔ `1+n×step`(가산)으로 되돌리지 말 것: 미네랄 축이 매 회차 0에서 다시 쌓이는 구조라 **회차 간 배수를 RP가 담당해야 하는데**, 가산이면 n이 커질수록 1점의 가치가 0으로 희석돼 후반 회차가 손해가 된다. 곱셈은 밑수가 같아 **여러 축에 나눠 찍어도 총 배수가 같다**(함정 빌드가 없다 — 스모크가 검사).
  - **총량 = 환생 지급분(`c.rp`) + 기록분(`profRecordRp`)**. 기록분은 **저장하지 않고 늘 다시 센다** — 그래야 기록이 갱신되는 순간 별도 지급 처리 없이 늘어나고, 중복 지급이 원천적으로 불가능하다. 마지막 환생 전에는 0이다.
...
- **유즈맵 화면은 카드 두 장이다**(2026-08-18 · 최종). 위 = 목록 카드(`.msPanel` — 정렬 띠 + 목록), 아래 = 소셜 카드(`#msSocialDock` — 채팅·친구·파티). 소셜은 목록 카드 **안에 들어가지 않는다**.
  - 목록 카드 안에서 정렬 띠 + 목록은 `.msTop` 이 **한 톤 어두운 판**(`rgba(0,0,0,.2)`)으로 한 번 더 묶는다.
  - 소셜은 **아예 다른 섹션**으로 보이게 간다 — 화면 좌우를 꽉 채우는 띠(`margin:17px -13px 0` 로 `#mapSelect` 의 좌우 여백 13px 을 되돌린다) · 라운드 0. 면·모서리·윗선은 **사냥터 하단 패널(`.hmUpg`)과 같은 처리**다: 반투명 회색 면(`rgba(25,25,25,.9)→rgba(14,14,14,.92)`) + **위쪽 두 모서리 10px 컷**(`clip-path`).
  - ⚠ 모서리를 컷하면 **테두리가 잘려 안 보인다** — `.hmUpg` 와 똑같이 윗변에 1px 선을 `::before` 로 따로 얹어 '판'으로 세운다. 선은 **컷 길이(10px)만큼 안쪽**에서 시작해야 잘린 모서리와 어긋나지 않는다.
  - 스모크가 `clip-path` 가 `.hmUpg` 와 같은지·윗변 선이 있는지·면이 반투명 어두운 회색인지 검사한다.
...
- **`.msPanel` = 붉은 헤어라인 위·아래 두 줄**(2026-08-18). 바깥 1px(금속 엣지) + **위·아래 변에 각각 1px 붉은 광선**(`::before`=위 / `::after`=아래, `rgba(255,59,59,.72)` + 약한 글로우). DESIGN §0 '강조는 두께가 아니라 빛' 그대로다. 라운드는 `--r-bar`(3px). ⚠ 아래쪽 어두운 inset 선은 뺐다 — 붉은 선 바로 옆에 검은 선이 겹쳐 두 줄로 보였다.
- **유즈맵 화면은 카드 한 장, 그 안이 두 구역이다**(2026-08-18). `.msPanel` 하나가 **정렬 띠 → 목록(`#msList`) → 소셜(`#msSocialDock`)** 을 전부 담는다. 붉은 헤어라인이 카드 위·아래를 감싸 '한 장'임을 말한다.
  - 위 구역은 `.msTop`(정렬 띠 + 목록)이 **한 톤 어두운 판**(`rgba(0,0,0,.2)`)으로 묶는다. 소셜은 이 판 **밖**이라 카드 면 그대로고, 둘 사이 17px 틈에서 카드 색이 드러나 구역이 갈린다. 스모크가 `.msTop` 이 목록만 품는지·소셜을 안 품는지·면이 한 톤 어두운지 검사한다.
  - ⚠ **카드 안에 카드를 두지 않는다. 면도 자기 것을 깔지 않는다** — 카드(`.msPanel`)의 회색 그라데가 아래 구역까지 그대로 이어져야 한 장으로 읽힌다. 어두운 판(`rgba(0,0,0,.34)`)을 한 겹 덮었더니 위아래가 딴 화면처럼 보였다. 아래 구역이 다른 구역임을 말하는 것은 둘뿐이다 — ① **양끝이 흘러 사라지는** 1px 선(`#msSocialDock::before`, 좌우 9% 안쪽) ② 위에서 드리우는 그늘(`inset 0 9px 11px -11px`). `.msSocial` 이 갖고 있던 테두리·면·바깥여백도 도크 안에서 전부 걷어낸다. 스모크가 도크가 자기 면을 깔지 않는지 검사한다.
  - ⚠ **칸막이를 `border-top` 으로 끝까지 긋지 말 것.** 카드 양 벽에 T자로 부딪혀 '판 두 장을 포갠 이음새'로 보인다(실제로 그랬다). 밝은 하이라이트를 같이 얹으면 더 심해진다. 선보다 **그늘이 더 많이 말한다**. 스모크가 `border-top` 이 없는지·그늘이 있는지·선(`::before`)이 있는지를 검사한다.
  ...
  - ⛔ 유즈맵 목록은 끝을 흐리지 **않는다**(2026-08-18 제거) — 안쪽에서 끝난다는 것은 여백이 이미 말한다. 소셜 목록에는 아직 `mask-image` 가 남아 있다.
  - 이 화면은 '한 상자 → 분리된 두 카드 → 한 카드 두 구역'을 오갔다. 지금 결론은 **한 카드 두 구역**이다.
...
- **👥 유즈맵 소셜 — 친구는 한 목록, 파티는 게시판이 앞에 선다**(2026-08-18).
  ...
  - 파티 칸 8개(2열×4행)는 소셜 구역 안에 **스크롤 없이** 들어와야 한다 — 칸 높이 28px · 격자 간격 3px · 아바타 18px · 닉 10px / 역할 8.5px. 계산은 `구역 안높이 153 − 머리줄 31 = 122 ≥ 4×28 + 3×3 = 121`. 도크 `min-height:174px` 가 이 값을 받친다. 스모크가 `scrollHeight <= clientHeight` 로 검사한다. ⚠ 구역 높이를 건드리면 이 계산부터 다시 할 것.
...
- **📈 성장 곡선·환생·해금 = 설계 문서가 아니라 코드 상수다**(2026-08-12 설계 확정). 사용자 설계: *초반은 빠르게 → 뒤로 갈수록 배로 → 막히면 환생 → 막힐 때쯤 새 요소가 하나씩*.
  ...
  - **환생 `profRebirth(c)`** (2026-08-18 전면 개편) — **정해진 레벨에서 한 번씩만**. 요구 레벨은 `PROF_REB_LEVELS = [20,45,75,110,145,175,200]`(간격 체증 7회)이고 마지막이 레벨 상한 `PROF_LV_CAP`이다. `c.rebLvMax`가 같은 레벨 재환생을 막는다. ⛔ 옛 계단식(`floor(lv/20)`)으로 되돌리지 말 것 — 같은 레벨에서 무한 반복이 가능해 RP가 폭주했다.
  - 회차 N 지급 = `profRebGrant(N)` = `PROF_REB_RP0(10) + PROF_REB_RP1(8)×N` · 경험치 배수 `+PROF_REB_GAIN(0.2)×N` 누적.
  - **되돌리는 것**: 레벨·경험치·유닛 레벨·레벨 포인트 · **`hunt.upg`(미네랄 업그레이드 레벨)** · **`p.pcoin`(미네랄 재화)** · `hunt.dg`/`hunt.round`.
  - **남는 것**: **`hunt.unl`(업그레이드 해금)** · **`hunt.best`(최고 기록 → 던전 해금·라운드 선택)** · 환생 포인트 · 장비 · 펫 · 가스 · 젬 · 뽑기권 · 진화★.
  - ⚠ 미네랄 **재화**를 안 지우면 환생 직후 즉시 되사서 리셋이 무의미해진다. 반대로 **해금**까지 지우면 매 회차 초반이 해금비로 막힌다 — 이 둘의 비대칭이 설계의 핵심이다.
  - ⚠ `hunt.dg`를 남기면 안 된다 — 곡선이 이어져 있어(hbProg) 던전 5 라운드 1은 라운드 397과 같은 세기다. Lv1 캐릭터로는 절대 못 이긴다.
  - 실측(자동 플레이 · 던전1 시작): **7회차 완주 8.05시간** · 최종 RP 308(환생 294 + 기록 14) · 구간 소요 0.20 / 0.31 / 0.44 / 0.68 / 1.23 / 2.60 / 2.58h · 도달 1-25→1-45→1-66→1-82→2-15→2-27→2-46.
  - ⚠ 회차 수를 바꾸면 **총 RP가 같이 바뀐다**(지급이 `10+8N`이라 6회 228 → 7회 294). 총량을 유지하려면 `PROF_REB_RP1`을 함께 내릴 것. 6회 구성일 때는 마지막 런이 6.6h(전체의 68%)로 한 구간에 몰렸고, 145·175를 끼워 7회로 쪼개자 최대 구간이 2.60h(32%)로 고르게 갈렸다.
  - ⚠ **환생 구간 길이를 늘리는 레버는 `PROF_XP_GEO`가 아니라 `HB_ROUND_XP`다.** XP 요구를 6.2배(GEO 1.03→1.045) 올려도 총 시간은 1.28배(1.97h→2.52h)밖에 안 늘었다 — 플레이어가 더 깊이 밀면 XP 수입이 `HB_ROUND_XP^진행도`로 따라 올라 요구량을 상쇄해 버린다. `HB_ROUND_XP`를 1.06→1.03으로 내리자 같은 조건에서 2.52h → 9.67h가 됐다.
