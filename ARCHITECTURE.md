...
- **👥 멀티 대기실(`#lobby`) = 전체 화면 · 내 종족은 공용 탭 띠**(2026-08-19).
  ...
  - ⚠ **슬롯 8칸은 스크롤 없이 다 보여야 한다.** 채팅이 남는 높이를 먹는 구조라 행 높이를 조금만 키워도 8번째가 조용히 잘린다 — 슬롯 32px(팀전 31px)·팀 라벨 20px 이 그 한계에서 나온 값이고 스모크가 `scrollHeight`로 지킨다.
  - 머리줄 오른쪽(`renderLobbyHeadR`) = 방 번호 + 배지. 팀전이면 대진(`4 vs 4`), 아니면 난이도. **사용자 지정 방이면 배지가 하나 더 붙는다** — 들어와 있는 사람도 기본 밸런스가 아님을 알아야 한다.
  - ⚠ 실방(Supabase presence)으로 **내 종족을 전파하는 경로는 아직 없다** — 로컬/봇 방에서만 반영된다.
