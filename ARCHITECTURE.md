...
- **포인트 UI는 `_ptListHTML(kind, c)` 한 함수**다(`kind`='lp' 캐릭터>스탯 / 'rp' 캐릭터>환생). 조작도 `ptTap`/`ptDoReset` 하나로 모인다 — 화면이 둘이라고 동작을 두 벌 만들지 말 것. **카드는 사냥터 업그레이드와 같은 `hmUpCardHTML()`** 을, 초기화 버튼은 사냥터 수량 버튼(`.hmUpQty>.hmUpQ`)을 그대로 쓴다(스모크가 '카드 뼈대가 사냥터 것과 같은가'를 검사한다).
