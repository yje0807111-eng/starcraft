 ...
 *
 * ⛔⛔ **2026-09-11 부터 안 돈다 — 잰 규칙이 통째로 없어졌다.**
 *    이 스크립트는 「환생 포인트 = √(번 돈) × 깊이 · 배수 = 로그」를 전제한다.
 *    지금은 관문·포인트가 **도달 깊이**(통산 관문 수)이고 배수는 **1 + 환생 횟수**다
 *    (GAME_DIRECTION §0-A 「새 뼈대」). 없어진 이름을 부른다: CAMP_REB_COST · CAMP_RP_BASE ·
 *    CAMP_REB_K. 돌리면 그 자리에서 죽는다.
 *    ⭐ **고쳐서 되살리려면** 묻는 것 둘(① 트리가 유한한가 ② ×2 가 몇 회차를 앞당기나)을
 *      새 자로 다시 써야 한다 — 트리는 이제 **레벨 포인트**로 사고(한 회차 17점 · BALANCE §5-11)
 *      환생 포인트는 쓰는 곳이 아직 없다. ⛔ 지우지는 않았다(되살릴 때의 자산).
 *
