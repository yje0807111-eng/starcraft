...
const fin=await pg.evaluate(()=>({ price:(function(){ const T=TECH_TREE[G.tech.race], out=[];
    ...
    return out; })(), sk:__CB.sk||{}, skTick:__CB.skTick||0, skTickU:__CB.skTickU||0, medHp:Math.round(__CB.medHp||0), healHp:Math.round(__CB.healHp||0), log:__CB.log, wealth:__CB.wealth, mile:__CB.mile||{}, mileOrder:(__CB.MILE||[]).map(m=>m[0]), jam:__CB.jam||null, vanish:__CB.vanish||null, dead:__CB.dead||null, t:__CB.t, dbg:__CB.dbg||[], gateT:__CB.gateT||0, holdT:__CB.holdT||0, holdMax:Math.round(__CB.holdMax||0), hold:Math.round((G.tech&&G.tech.credit)||0), earn:Math.round(campWealth()),
