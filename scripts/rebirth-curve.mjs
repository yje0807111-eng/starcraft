#!/usr/bin/env node
// 환생 곡선 계산기 — HUNT_R1.md §4-2 · §4-4 · §4-6 · §6-1 의 표를 만드는 원본.
// 숫자를 바꾸려면 여기 상수를 고치고 다시 돌린 뒤 문서 표를 갱신할 것.
//   node scripts/rebirth-curve.mjs            전체 표
//   node scripts/rebirth-curve.mjs 0.006 4.5  라운드밑 상승폭 · 티어 배수를 바꿔서 비교
...
const tierMix    = t => isMile(t) ? MIX_MILE : MIX;
const tierFactor = t => Object.entries(tierMix(t)).reduce((a,[g,n]) => a + GRADE[g]*n, 0);
...
const mineral  = (d, r) => (1.5 + (r - 1) * 0.01) * Math.pow(2, d - 1);
...
console.log('\n=== A. 던전별 (§4-6-A) ===');
console.log('던전 라운드밑 문턱  미네랄R1   미네랄R50  난이도R1     난이도R50    포인트R1   포인트R50');
for(let d = 1; d <= DUNGEONS; d++)
  console.log(`D${P(d,3)} ${P(rbase(d).toFixed(3),8)} ${P(d===1?'-':'×'+(diff(d,1)/diff(d-1,ROUNDS)).toFixed(2),6)} ${P('×'+mineral(d,1).toFixed(2),10)} ${P('×'+mineral(d,ROUNDS).toFixed(2),10)} ${P(F(diff(d,1)),12)} ${P(F(diff(d,ROUNDS)),12)} ${P(F(gainPts(d,1)),10)} ${F(gainPts(d,ROUNDS))}`);

console.log('\n=== B. 던전 안에서 라운드가 오를 때 (§4-6-B) ===');
for(const d of [1, 5, DUNGEONS]){
  console.log(` 던전 ${d} (라운드밑 ${rbase(d).toFixed(3)} · 난이도 +${((rbase(d)-1)*100).toFixed(1)}%/칸 · 포인트 +${(Math.sqrt(rbase(d))*100-100).toFixed(1)}%/칸 · 50라운드 누적 ×${F(Math.pow(rbase(d),ROUNDS-1))})`);
...
for(let d = 3; d <= DUNGEONS; d++){ process.stdout.write(P('D'+d,7));
