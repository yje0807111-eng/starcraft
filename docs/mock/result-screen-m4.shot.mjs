...
await pg.addStyleTag({url:'/docs/mock/result-screen-m4.css'});
// 목업 골격 — 확정 구조: 제목(가운데보다 살짝 위) / 게임 안 기록 줄 / 큰 숫자 = 획득 미네랄 / 버튼 둘
...
await pg.evaluate(()=>{ const ov=document.getElementById('ov'); if(ov.querySelector('.rsX')) return;
  ...
  d.innerHTML='<div class="rsTop"><div class="rsTtl">VICTORY</div>'
    ...
    +'<div class="rsRows">'
      ...
      +row('최고 유닛','전략 폭격기')
      +row('획득 포인트','+3,150','hi')
      +row('내 캐릭터 XP','+412 · Lv.24','xp')
    +'</div>'
    +'<div class="rsHero"><span class="rsHeroL">'+RI('미네랄')+'획득 미네랄</span>'
      +'<span class="rsHeroV"><b>+8,420</b><span class="rsHero2">'+RI('가스')+'<b>+1,684</b></span></span></div>'
    +'<div class="rsSub2"><span>'+RI('가스')+'<b>+1,684</b></span><span>XP <b>+412</b></span></div>'
...
async function shoot(cls, kind, file){
  await pg.evaluate(({c,k})=>{ const ov=document.getElementById('ov'), x=ov.querySelector('.rsX');
    ...
    ov.className=ov.className.replace(/\b[nm][1-8]\b/g,'').trim();
...
await shoot('m1','win','_warm.png');
const V=['m1','m2','m3','m4'];
for(const v of V){ await shoot(v,'win', 'rs-'+v+'-win.png'); }
await shoot('m1','lose','rs-m1-lose.png');
