...
2. ~~적 기지 화면이 새 그림이다~~ → ✅ **목업 나왔다**: `docs/mock/camp-foebase-4.html`(4안 · 2026-09-09).
   네 안은 **「깨야 할 여섯을 어떻게 읽히나」**만 다르다 — ① 번호 배지 · ② 깃발 · ③ 밑변 광원 · ④ 나머지를 물리기.
   공통(확정): 칩 `이름 · 3/6` + 붉은 진행선 · 안개는 **실루엣** · 다음 표적은 붉은 모서리 + 맵 띠에 이름 ·
   본진은 나머지 다섯이 죽어야 열린다 · 체력 선은 맞은 건물에만 · 부순 것은 **잔해로 남는다** · 하단 시트는 그대로.
   🔶 **사용자 확정 대기.** 남은 물음 셋도 목업 아래에 적었다(방어탑을 물릴까 · 잔해를 몇 채까지 · 12채가 화면에 다 드나).
