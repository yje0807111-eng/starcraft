...
| 12채가 화면에 다 드나 | ✅ **든다** — 적 기지를 **격자 위 한 화면**으로 올렸다(`CAMP_LANE_TOP` −0.26 · 2026-09-09 사용자: 「화면 완전 위」) · 진입 뷰는 위 끝(12/12 · 축소 하한 1.0) · 아래로 끌면 내 기지 · 🎲 배치는 **거울 + 랜덤**(`campFoeLayout` · 광맥 호 → 본진 → 테크 → 생산 · 원정마다 씨앗 · 적 광맥·가스·일꾼은 연출) · ⚠ 위 한 화면의 바닥 그림은 아직 없다(ART.md §17 프롬프트 준비됨) |
| 맵 띠 「다음 표적」 | ✅ `.cbTgt` |
...
> **남은 것**: ① **스킵**(던전 시작 지점·시작 자원/구성) ② **자동화**(구매·건설·연구 — 벤치 정책을
> 게임 안으로 · 벤치와 단일 소스) ③ **1차 트리 단순화·가격 하향**(사용자와 표를 새로 짜야 한다)
> ④ 값 실측(2차 포인트 눈금 18 위에서 트리 비용이 맞는지).
...
> | | 2026-09-11 현재 |
> |---|---|
> | ✅ 끝남 | **레벨 축**(`C.lv`·`C.xp`·`C.lvPts` · `campAddXp`) · 지급은 캠프 전투의 죽은 유닛 정리 한 곳 · 성장 트리가 레벨 포인트를 쓴다 · 회차 되감기에 레벨·포인트·트리 포함 |
> | ⏳ 남음 | 환생을 하나로 정리(관문 = 무한층에서 막힘 · 포인트 = 도달 깊이 · `C.rbPts` 소비처 넷) · 성장 트리 비용표를 **레벨 포인트 단위로 재조정** · 룬 칸 해금을 레벨로 · `CAMP_RT_PTS_FREE` 끄기 |
> | ⚠ 안 쟀다 | `CAMP_XP_KILL`·`CAMP_XP_A`·`CAMP_XP_R` — 근거는 「요구량 배수 > 관문 평균」 하나뿐(BALANCE §0) |
>
