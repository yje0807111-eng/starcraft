...
async function groupLobby(){
  ...
  await step('환생 구역: 환생·업그레이드 두 탭 · 네비 자리를 비운다', async()=>{
    ...
    //   ⚠ 트리 쪽 사본(.ctX 「닫기」)도 같이 없앴다 — 누르면 트리만 닫히고 배경이 남았다(잔상).
    assert(!document.querySelector('#campTree .ctX'),'환생 트리에 「닫기」가 남아 있다');
    ...
    // 🌌 배경은 **환생 구역 전용 한 장**이다 — 로그인·부팅의 키 아트를 빌리지 않는다
    assert($('campRebBg') && !$('campRebBg').classList.contains('hide'),
      '환생 구역 전용 배경이 안 켜진다');
    { const bg=getComputedStyle($('campRebBg')).backgroundImage;
      assert(bg.indexOf('backgrounds/reb/')>=0,'전용 배경 그림이 안 걸렸다: '+bg);
      assert(bg.indexOf('boot.webp')<0,'부팅 키 아트를 여기서 또 그린다'); }
    // ⑦ 🎬 **페이드는 들어올 때 한 번만** — 탭 이동은 즉시다(2026-08-31 사용자 지적).
    //   ⛔ `.on` 에 애니를 걸면 환생 ↔ 업그레이드 를 오갈 때마다 화면이 껌뻑인다.
    //   ⭐ 애니는 `.crIn` 이 갖고, campRebEnter 가 **구역 밖에서 들어온 경우에만** 붙인다.
    campRebClose(); campTreeClose();
    campRebEnter('info');
    assert($('campReb').classList.contains('crIn'),'밖에서 들어왔는데 페이드가 안 걸린다');
    campRebEnter('tree');
    assert(!$('campTree').classList.contains('crIn'),'탭만 바꿨는데 페이드가 다시 돈다(업그레이드)');
    campRebEnter('info');
    assert(!$('campReb').classList.contains('crIn'),'탭만 바꿨는데 페이드가 다시 돈다(환생)');
    { const cs=getComputedStyle($('campReb'));
      assert(cs.animationName==='none','페이드가 .on 에 붙어 있다 — 탭마다 다시 돈다: '+cs.animationName); }
    // ⑧ 🖼 **배경은 구역의 것** — 탭을 오가는 동안 껐다 켜지 않는다(2026-08-31 「번쩍」 신고).
    //   ⛔ 끄면 트리는 제 배경에 가려 멀쩡해 보이지만, 환생 탭으로 돌아올 때 그림만 뒤늦게 떠오른다.
    { const art=()=>!$('campRebBg').classList.contains('hide');
    ...
    { const ph=$('phone'), had=ph.classList.contains('artLift'); ph.classList.add('artLift');
      const a=parseFloat(getComputedStyle($('campRebBg')).transitionDuration)||0;
      const u=parseFloat(getComputedStyle(document.documentElement).getPropertyValue('--t-screen'))||0;
      if(!had) ph.classList.remove('artLift');
      assert(a>0 && u>0 && a < u*0.75,
        '배경이 UI 와 같은 속도로 뜬다 ('+a+'s vs '+u+'s) — 겹치는 동안 아래 화면이 새어 나온다'); }
    campRebClose(); campTreeClose();
    return '환생·업그레이드 · 네비 자리 '+gap('campReb')+' · 서로 배타 · 페이드는 진입 1회';
  });
  ...
  await step('캠프: 종족 선택 · 진입 · 저장 · 복원', async()=>{
    ...
    { const sy=(wx,wy)=>_techW2S(wx,wy).y, sx=(wx,wy)=>_techW2S(wx,wy).x;
      ...
      { const gimg=[...document.querySelectorAll('.bGasZone.spr .gzSpr')];
        ...
        //   ⬇ 부지는 거기서 **CAMP_GAS_ROW_OFF 칸 아래**다(2026-09-04) — 그림은 CSS(--gzDY)가 되돌려 제자리다.
        assert(TECH_GAS.r0===Math.max(0,Math.round(campRow(CAMP_ROW_BASE)+CAMP_GAS_FOOT-TECH_GAS.h)+CAMP_GAS_ROW_OFF),
      ...
      { const g2x=sx(TECH_GRID.x0+(CAMP_GAS2.c0+TECH_GAS.w/2)*cw, 0);
        ...
        // ⛽ **오른쪽은 지금 꺼져 있다**(CAMP_GAS2_ON=false · 2026-09-02 「가스는 왼쪽 하나뿐」).
        //   ⛔ 꺼진 자리를 인정하지 말 것 — 판정을 감싼 채로 두면 화면에 아무것도 없는 칸이
        //     건설 금지로 남는다(2026-09-04 사용자 신고로 드러났다).
        if(CAMP_GAS2_ON) assert(at(CAMP_GAS2.c0,CAMP_GAS2.r0),'오른쪽 가스에 정제소를 못 짓는다');
        else { G.tech.arm='supply';
          assert(at(CAMP_GAS2.c0,CAMP_GAS2.r0),'꺼진 오른쪽 가스 자리가 아직 건설 금지다');
          G.tech.arm='refinery'; }
    ...
    { const gasBefore={c0:TECH_GAS.c0, r0:TECH_GAS.r0};
      ...
      // ⛽ 오른쪽 가스가 **켜져 있을 때만** 감싼다(2026-09-04) — 꺼져 있으면 애초에 안 감싸는 것이 맞다.
      //   ⛔ 꺼진 채로 감싸면 화면에 아무것도 없는 칸이 건설 금지로 남는다(사용자 신고로 드러났다).
      if(CAMP_GAS2_ON)
        ...
        assert(techArmValid===fnBefore.a && _techGasOverlap===fnBefore.b && _techInGasZone===fnBefore.c,
          '오른쪽 가스가 꺼졌는데 판정 함수를 감쌌다 — 맵에 없는 자리가 건설 금지로 남는다');
    ...
    { const vb=document.getElementById('vBuild'), r=vb.getBoundingClientRect();
      ...
      { const mr=document.getElementById('cstMain').getBoundingClientRect();
        ...
        { const sf=(typeof techSheetFrac==='function')?techSheetFrac():0;
          ...
          // ⚠ **축소 하한(=CAMP_ZOOM) 미만은 넣지 말 것** — 도달할 수 없는 배율이라
          //   클램프가 zoom 을 하한으로 올려 버리고, seen() 의 (0.5-sf)/z 와 어긋나 헛돈다.
          //   옛 목록은 1.7 이 박혀 있어서 CAMP_ZOOM 을 1.3 → 1.8 로 올리자 그대로 터졌다(2026-08-30).
          const zs=[CAMP_ZOOM, (CAMP_ZOOM+techMaxZoom())/2, techMaxZoom()].filter(z=>z>=CAMP_ZOOM-1e-9);
  ...
  // 🌟 새로운 시작 — 가운데(root)를 산 회차는 **빈손이 아니게** 시작한다(2026-09-01 사용자 확정).
  //   ⚠ 위 계약이 「시작 일꾼 0기 · 미네랄 0」을 지키는 이유는 root 를 **안 샀기 때문**이다.
  //     그러니 여기서는 root 를 켜고 재고, 끝나면 얹은 것을 도로 걷는다(뒤 계약이 오염되면 안 된다).
  await step('환생 트리: 새로운 시작이 실제로 캠프에 얹힌다', async()=>{
    skipIf(typeof campRootGrant!=='function'||typeof G==='undefined'||!G.tech,'캠프가 안 떠 있음');
    const C=campState(); skipIf(!C,'캠프 상태 없음');
    const keepT=JSON.parse(JSON.stringify(C.rbTree||{}));
    const c0=G.tech.credit|0, e0=(G.tech.ents||[]).slice(), b0=Object.assign({}, G.tech.built);
    try{
      const wk=()=>(G.tech.ents||[]).filter(e=>e.type==='worker').length;
      const bar=()=>(G.tech.ents||[]).filter(e=>e.type==='bldg'&&e.bk===CAMP_ROOT_BLD).length;
      // ① root 가 없으면 아무 일도 없다 — 지금 화면이 「빈손 시작」인 이유가 이것이다
      const w0=wk(), r0=bar();
      C.rbTree={};
      assert(campRootGrant()===null,'root 없이도 새로운 시작이 얹힌다');
      assert(wk()===w0&&(G.tech.credit|0)===c0,'root 없이 자원·일꾼이 늘었다');
      // ② root 를 사면 자원·일꾼·건물이 함께 온다
      C.rbTree={root:1,_m2:1};
      const got=campRootGrant();
      assert(got,'새로운 시작이 아무것도 안 줬다');
      assert((G.tech.credit|0)===c0+CAMP_ROOT_MIN,'미네랄이 안 들어왔다: '+G.tech.credit);
      assert(wk()-w0===CAMP_ROOT_WK,'일꾼이 '+CAMP_ROOT_WK+'기 안 늘었다: +'+(wk()-w0));
      if(CAMP_ROOT_BLD) assert(bar()>r0,'시작 건물이 안 섰다');   // 유보 중이면 건너뛴다
      if(CAMP_ROOT_BLD) assert(!!G.tech.built[CAMP_ROOT_BLD],'건물은 섰는데 테크가 안 열렸다');
      // ③ 두 번 불러도 건물이 겹치지 않는다(캠프를 다시 열 때마다 쌓이면 안 된다)
      const r1=bar(); campRootGrant();
      assert(bar()===r1,'같은 건물이 또 섰다: '+r1+' → '+bar());
      return '미네랄 +'+CAMP_ROOT_MIN+' · 일꾼 '+CAMP_ROOT_WK+'기'+(CAMP_ROOT_BLD?' · '+CAMP_ROOT_BLD+' 1채':' · 건물 유보');
    } finally { C.rbTree=keepT; G.tech.credit=c0; G.tech.ents=e0; G.tech.built=b0; } });
  ...
  // 🌌 별자리 트리 — 해금 규칙 · 관문 · 아이콘 · 확대축소 (2026-09-01 사용자 확정)
  ...
  await step('환생 트리: 계열마다 아이콘 · 밀고 확대가 한 점을 붙잡는다', async()=>{
    ...
    try{
      ...
      assert(!$('campTree').querySelector('.ctRootTx'),'가운데에 글씨가 다시 들어왔다');
      // 🔍 확대는 **붙잡은 점**을 제자리에 둔다 — 중심 기준이면 보던 별이 흘러간다
      const svg=$('ctSvg'), r=svg.getBoundingClientRect();
      ...
      assert(!$('campTree').querySelector('.ctDesel'),'⊘ 가 되살아났다 — 빈 곳 탭이 해제다');
      assert(!!$('campTree').querySelector('.ctSheet .ctBuy'),'하단에 사기 버튼이 없다');
      ...
      { const a=$('campTree').querySelector('.ctTitle'), b2=$('campReb').querySelector('.crTitle');
        assert(a&&b2,'제목 요소가 없다');
        const g=e=>{ const c=getComputedStyle(e); return { f:c.fontFamily, s:c.fontSize, l:c.letterSpacing, w:c.fontWeight }; };
        const ti=getComputedStyle(document.documentElement).getPropertyValue('--font-ti').trim();
        for(const [nm,e] of [['트리',a],['환생',b2]]){ const v=g(e);
          assert(v.s==='16px',nm+' 제목 크기가 유즈맵·상점(16px)과 다르다: '+v.s);
          assert(v.w==='700',nm+' 제목 굵기가 유즈맵·상점(700)과 다르다: '+v.w);
          assert(parseFloat(v.l)>1.5,nm+' 제목 자간이 유즈맵·상점(.12em)과 다르다: '+v.l);
          assert(ti&&ti.indexOf(v.f.split(',')[0].replace(/['\"]/g,''))>=0,
            nm+' 제목이 제목 글꼴(--font-ti)을 안 쓴다: '+v.f); }
        assert(g(a).f===g(b2).f&&g(a).s===g(b2).s,'트리와 환생의 제목이 서로 다르다');
        assert(!/[🌀-🫿]/u.test(a.textContent),'제목에 이모지가 들어갔다 — 유즈맵·상점엔 없다'); }
      ...
      assert(!$('campTree').querySelector('.ctZoom'),
        ...
        { const src=String(campTreeBind);
          assert(src.indexOf('campTreeFit()')>=0 && src.indexOf('_ctTapT')>=0,
      ...
      campTreeDesel();
      // 👆 **손가락으로 누를 수 있어야 한다** (2026-09-01 사용자 신고: 가운데가 안 눌림).
      //   원인이 셋이었다: ① capture 때문에 click 의 target 이 <svg> 가 되어 어느 별인지 사라졌다
      //   ② 별이 화면에서 15px 이라 6px 만 빗나가도 헛쳤다 ③ 「민 것」 임계 8px 이 손가락에 빡빡했다.
      { const src = String(campTreeBind);
        assert(!/svg\.addEventListener\('click'/.test(src),
          'click 으로 별을 고른다 — capture 때문에 target 이 <svg> 가 되어 못 고른다');
        assert(/pointerup/.test(src)&&/campTreeTap/.test(src),'pointerup 에서 별을 열지 않는다');
        assert(CAMP_TREE_TAP_SLOP>=12,'「민 것」 임계가 손가락에 빡빡하다: '+CAMP_TREE_TAP_SLOP); }
      // 🎬 뷰 이동은 **연출**이다 — 천천히 출발 → 아주 빠르게 → 마지막에 살며시(2026-09-01 사용자 지정)
      { assert(typeof campTreeEase==='function','이징이 없다');
        const e2=campTreeEase(.2), e5=campTreeEase(.5), e8=campTreeEase(.8);
        assert(e2<.03,'앞이 안 느리다(20% 시점 '+(e2*100).toFixed(1)+'%)');
        assert(e8>.97,'뒤가 안 느리다(80% 시점 '+(e8*100).toFixed(1)+'%)');
        assert(Math.abs(e5-.5)<1e-6,'가운데가 절반이 아니다: '+e5);
        assert(CAMP_TREE_TWEEN_MS>=380&&CAMP_TREE_TWEEN_MS<=900,'연출 길이가 이상하다: '+CAMP_TREE_TWEEN_MS);
        // 즉시 배치는 now 인자로만 — 인자 없이 부르면 연출로 간다
        campTreeFit(true); const zA=_campTreeView.z;
        campTreeTap('gather',3); const zB=_campTreeView.z;
        assert(Math.abs(zB-zA)<1e-6,'별을 골랐는데 배율이 즉시 튄다 — 연출이 아니다');
        // ⚠ 손이 닿으면 연출은 멈춘다(안 그러면 미는 동안 화면이 저 혼자 흘러간다)
        campTreeZoomAt(_campTreeView.z*1.1, null);
        assert(!_ctTween,'손으로 확대했는데 연출이 계속 돈다');
        campTreeFit(true); }
      { // 보이는 별보다 **누르는 면이 커야** 한다
        const gem=$('campTree').querySelector('#ctG .ctGem'), hit=$('campTree').querySelector('#ctG .ctHit');
        assert(gem&&hit,'별에 누르는 면(.ctHit)이 없다');
        assert(+hit.getAttribute('r') > +gem.getAttribute('r'),
          '누르는 면이 보이는 별보다 작다: '+hit.getAttribute('r')+' vs '+gem.getAttribute('r'));
        assert(getComputedStyle(gem).pointerEvents==='none','보이는 별이 눌림을 가로챈다');
        // 그래도 빗나가면 가장 가까운 별을 잡아 준다
  ...
  // 🛡 **방어 건물은 전선에 지을 수 있다** (2026-08-30 사용자 확정)
  ...
  await step('캠프: 벙커에 태우면 그 자리에서 쏘고 피해는 벙커가 받는다', async()=>{
    ...
    campWipeField();
    let bunk=(CAMPB._bld||[]).find(b=>b&&b.bk==='bunker');
    ...
    // ⚠ 10초 사이에 라운드가 넘어갔으면 campRoundRevive 가 전장 건물을 **새로 만든다** —
    //   옛 참조를 그대로 쓰면 「벙커가 대신 안 받았다」로 헛되이 넘어진다(2026-09-04 초반 램프로 드러났다).
    bunk=(CAMPB._bld||[]).find(b=>b&&b.bk==='bunker')||bunk;
  ...
  await step('캠프 던전: 적 웨이브 · 전투 · 승패', async()=>{
    ...
    try{
      ...
      { const live=campBldAlive();
        ...
        // ⭐ **패배는 본부 파괴 하나뿐**(2026-08-30 사용자 확정) — 다른 건물은 길목에서 시간만 번다.
        //    ⛔ 옛 규칙(건물 전부 파괴)으로 되돌리지 말 것.
        { const base=CAMPB.me.base;
          const other=campBldAlive().filter(b=>b!==base);
          if(other.length){                      // 본부가 아닌 건물은 부서져도 지지 않는다
  ...
  // 🎓 강제 튜토리얼 — 대상만 열고 나머지는 막는다. 진행도는 **게임 상태에서 직접 읽는다**.
  //   ⚠ 검사 내내 꺼 두므로(runSmoke 첫 줄) 여기서만 잠깐 켠다.
  await step('강제 튜토리얼: 대상만 열리고 · 진행도가 상태를 따라간다', async()=>{
    skipIf(typeof tutoPaint!=='function','튜토리얼 없음');
    const ph=$('phone'), S=(typeof guideState==='function')?guideState():null;
    skipIf(!S,'가이드 상태 없음');
    ...
    try{
      ...
      const tgt=document.querySelector('[data-minemode]');
      skipIf(!ov||!tgt||!tgt.getClientRects().length,'캠프 하단이 안 떠 있음');
      // ① 링이 대상을 감싼다
      const R=ov.querySelector('.tuRing').getBoundingClientRect(), Tg=tgt.getBoundingClientRect();
      assert(R.left<=Tg.left+1 && R.right>=Tg.right-1 && R.top<=Tg.top+1 && R.bottom>=Tg.bottom-1,
        '스포트라이트가 대상을 안 감싼다');
      // ② 대상 밖은 막고 · 대상은 통과한다(네 판이 입력을 먹는다)
      const pr=ph.getBoundingClientRect();
      const out=document.elementFromPoint(pr.left+pr.width*0.5, pr.top+pr.height*0.35);
      const inn=document.elementFromPoint(Tg.left+Tg.width/2, Tg.top+Tg.height/2);
      ...
      { S.t=TUTO_STEPS.length; S.skip=1; if(typeof openGuide==='function') openGuide();
        const row=document.querySelector('.gqTuto');
        assert(row,'가이드 시트에 튜토리얼 줄이 없다');
        assert(row.querySelector('.gqTutoGo'),'「해 보기」 버튼이 없다');
        tutoRestart();
        assert(tutoIdx()===0 && !S.skip,'다시 하기가 처음으로 안 돌린다: t='+S.t+' skip='+S.skip);
        tutoStop();
        assert(S.skip===1 && !$('tutoOv'),'그만두기가 안 먹는다');
  ...
  // ⛽ 가스 자리 — 안 보이는 금지 구역이 없어야 하고, 짓는 자리와 그림이 짝을 이뤄야 한다.
  await step('캠프 가스: 안 보이는 금지 구역 없음 · 짓는 자리는 그림보다 한 칸 아래', async()=>{
    skipIf(typeof CAMP_GAS2_ON==='undefined'||typeof TECH_GAS==='undefined','가스 구역 없음');
    // ① 🚧 오른쪽 가스를 끈 동안에는 **그 자리를 막지 않는다** — 화면에 아무것도 없는데 건설이
    //   막히던 버그(2026-09-04 사용자 신고). 판정을 감싸는 패치 자체가 안 걸려야 한다.
    if(!CAMP_GAS2_ON) assert(!_campGasPatched,
      '오른쪽 가스가 꺼졌는데 판정 패치가 걸려 있다 — 맵에 없는 자리가 건설 금지로 남는다');
    // ② ⬇ 짓는 자리(TECH_GAS.r0)와 그림 보정(--gzDY)은 **짝이다** — 하나만 바꾸면 그림이 통째로 움직인다.
    //   부지 높이가 2칸이라 한 칸 = 50%p. 기본 −27% 는 「반 칸 내림(23%) − 한 칸(50%p)」이다.
    assert(CAMP_GAS_ROW_OFF===1,'가스 부지 오프셋이 1칸이 아니다: '+CAMP_GAS_ROW_OFF);
    { let ok=false;
      for(const ss of document.styleSheets){ try{
        for(const r of ss.cssRules){ if(!r.selectorText||!/gzSpr/.test(r.selectorText)) continue;
          const t=(r.style&&r.style.transform)||'';
          if(/--gzDYs*,s*-27%/.test(t)) ok=true; } }catch(_e){} }
      assert(ok,'그림 보정 기본값이 −27% 가 아니다 — 부지를 한 칸 내렸으면 그림도 그만큼 되올려야 제자리다'); }
    return (CAMP_GAS2_ON?'오른쪽 켜짐':'오른쪽 꺼짐 · 금지 구역 없음')+' · 부지 −'+CAMP_GAS_ROW_OFF+'칸';
  });

  // ❤ 전장 HP 바 — 맞으면 화면에서 체력이 닳는 것이 보여야 한다(2026-09-04 사용자 요청).
  await step('캠프 전장: 맞으면 화면에서 체력이 닳는다 (HP 바)', async()=>{
    skipIf(typeof campBattleBars!=='function'||typeof campDeploy!=='function','전장 HP 바 없음');
    const C=campState(); const back={dg:C.dg, cleared:C.cleared};
    try{
      campEnterDungeon(1); CAMPB=null; campCombatStep(0.05);
      skipIf(!CAMPB,'전장이 안 열림');
      campWipeField();
      const u=campDeploy('marine', 0.5, CAMP_LINE_GY);
      assert(u,'유닛을 못 세웠다');
      const parse=(s)=>{ const d=document.createElement('div'); d.innerHTML=s;
        return [...d.querySelectorAll('.bentBar')].map(b=>{ const i=b.querySelector('.uH i');
          return { w:i?i.style.width:'', col:i?i.style.background:'' }; }); };
      // ① **가득 찬 것은 안 그린다** — 수십 기가 늘 바를 달고 있으면 화면이 바 투성이가 된다
      u.hp=u.maxHp;
      { const bars=parse(campBattleBars());
        assert(!bars.some(b=>/^100/.test(b.w)),'가득 찬 것에도 바가 그려진다'); }
      // ② 맞으면 그 비율로 뜬다
      u.hp=u.maxHp*0.4;
      { const bars=parse(campBattleBars());
        const hit=bars.find(b=>Math.abs(parseFloat(b.w)-40)<1);
        assert(hit,'40% 로 깎았는데 그 바가 없다: '+bars.map(b=>b.w).join(',')); }
      // ③ 건물도 같은 규칙 — 적이 건물을 때리면 보여야 한다
      { const b0=(CAMPB._bld||[]).find(x=>x&&!x.dead);
        skipIf(!b0,'전장에 건물이 없다');
        const mx=b0.maxHp||b0.max; b0.hp=mx*0.5;
        const bars=parse(campBattleBars());
        assert(bars.some(b=>Math.abs(parseFloat(b.w)-50)<1),'건물 바가 안 그려진다');
        b0.hp=mx; }
      // ④ 적은 붉은색 — 오토배틀(10-engine.js)과 같은 규약이라 아군과 눈으로 갈린다
      { const f=(CAMPB.ai&&CAMPB.ai.units||[]).find(x=>x&&!x.dead);
        if(f && f.maxHp>0){ f.hp=f.maxHp*0.5;
          const bars=parse(campBattleBars());
          assert(bars.some(b=>/255,\s*90,\s*90|ff5a5a/i.test(b.col)),
            '적 바가 붉지 않다: '+bars.map(b=>b.col).join(' | ')); } }
      return '가득참 숨김 · 깎이면 그 비율 · 건물 포함 · 적은 붉은색';
  ...
  await step('튜토리얼: 건물 짓기가 손동작 단위로 갈라져 있다', async()=>{
    ...
    // 🗺 던전 이동도 셋 — 목록을 열고 · 고르고 · 「이동」 을 누른다(고르기에서 끝내면 화면이 열린 채 남는다)
    ...
    for(const rk in TUTO_BLD){ const t=TECH_TREE[rk];
      ...
      const keys=(t.buildings||[]).map(b=>b.k);
      for(const bk of TUTO_BLD[rk].b.concat([TUTO_BLD[rk].u])){
        if(bk==null) continue;
        assert(keys.indexOf(bk)>=0, rk+' 에 그런 건물이 없다: '+bk); } }
    // 💸 유니온의 둘째 칸은 비어 있어야 한다 — 캠프 보급소는 3만이라 첫 안내에서 모을 수 있는 돈이 아니다
    assert(TUTO_BLD.union.b[1]==null,'튜토리얼이 보급소를 시킨다 — 캠프 보급소는 '+campSupplyCost(0).toLocaleString()+' 이다');
    // 🎯 카드 셀렉터가 **실제 마크업**과 맞나 — 건설 카드는 data 속성이 없어 onclick 으로 찾는다
    { const r0=G.tech.race; try{
        G.tech.race='union';
        const sel=_tutoBSel(0), m=techBuildListModel();
        const it=(m.items||[]).find(x=>x&&/techArm\('barracks'/.test(String(x.act||'')));
        assert(it,'건설 목록에 병영 카드가 없다');
        const box=document.createElement('div'); box.innerHTML=_cgSlotHTML(it, true);
        assert(box.firstChild.matches(sel),'카드 셀렉터가 마크업과 안 맞는다: '+sel);
    ...
    { const off0=TUTO_OFF; try{ TUTO_OFF=true;
        assert(tutoFreeUnit('marine')===false,'튜토리얼이 꺼져 있는데 유닛이 공짜다');
        assert(tutoNoWait()===false,'튜토리얼이 꺼져 있는데 즉시 생산이다');
      } finally { TUTO_OFF=off0; } }
    // ⏱ **건물을 지정하는 단계(selB1)부터** 공짜다 — 카드가 열리는 그 순간 0 이라야 한다.
    //   (2026-09-04 사용자 지적: 5,000 으로 잠겨 있다가 뒤늦게 0 이 됐다 — 값 갱신이 한 박자 늦었다)
    ...
    { const d=document.createElement('div'); d.id='campMineStopProbe'; d.className='tuLive';
      const ph2=$('phone'), had=ph2.classList.contains('tutoOn');
      // 실제 선택자를 그대로 쓰려면 id 가 같아야 한다 — 진짜 버튼이 없을 때만 대역을 세운다
      const real=$('campMineStop'); const el=real||d;
      if(!real){ d.id='campMineStop'; ph2.appendChild(d); }
    ...
    { assert(typeof _tutoTapAt==='function','맵 두드리기 단계의 공용 대상이 없다');
      const f0=window.campMineModeOn;
      try{ window.campMineModeOn=()=>false;
        assert(_tutoTapTip('두드리세요')!=='두드리세요','채굴이 꺼졌는데 안내가 안 바뀐다');
        window.campMineModeOn=()=>true;
    ...
    // 🎬 첫 등장은 **로고가 걷힐 때**다 — 맵이 커지는 연출(campInClip) 을 기다리지 않는다
...
window.runSmoke=async function(group){
  // 🎓 **튜토리얼은 검사 내내 꺼 둔다.** 켜져 있으면 스포트라이트가 화면을 덮고 입력을 막아
  //   맵·네비·시트 검사가 통째로 「덮였다」로 넘어진다(실측 2026-09-04). 튜토리얼 자체는
  //   전용 스텝이 잠깐 켜서 잰다 — CAMP_DEV_NOFAIL 과 같은 어법이다.
  try{ if(typeof TUTO_OFF!=='undefined') TUTO_OFF=true; if(typeof tutoPaint==='function') tutoPaint(); }catch(_e){}
  const t0=performance.now();
