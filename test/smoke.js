...
async function groupLobby(){
  ...
  await step('캠프 광맥: 가운데 두 칸이 큰 그림 · 3D 노드와 겹치지 않는다', async()=>{
    ...
    try{
      ...
      // 지금은 1·2번만 쓴다 — 가운데 두 칸이 1, 양 끝 네 칸이 2
      const files=sp.map(e=>e.src.split('/').pop());
      assert(new Set(files).size>=2,'모든 칸이 같은 그림이다 — 격자무늬로 돌아간다: '+files.join(','));
      assert(files.every(f=>f==='stage1.webp'||f==='stage2.webp'),'1·2번 외의 그림이 쓰였다: '+files.join(','));
      if(files.length===6){
        assert(files[1]==='stage1.webp' && files[4]==='stage1.webp','가운데 두 칸이 1번이 아니다: '+files.join(','));
        assert([0,2,3,5].every(i=>files[i]==='stage2.webp'),'양 끝 네 칸이 2번이 아니다: '+files.join(',')); }
      ...
      return '가운데 1번 · 양끝 2번 ('+files.length+'칸)';
