...
function campStepUnits(dt){
  ...
  for(const side of ['me', 'ai']){
    ...
    for(const a of act){
      ...
      // ⚔ **적이 먼저다** (2026-09-10 사용자 확정 — 「적을 먼저 쳐야 아군이 덜 맞는다」).
      //   ⛔ 「건물에 닿은 유닛은 건물부터」로 되돌리지 말 것. 그건 릴레이가 **틈 없는 흐름**이던
      //     시절의 우회다(적이 늘 사거리에 있어 건물을 영영 못 쳤다 → 본진이 60초에 이론의 2%).
      //   ⭐ 지금은 **스폰에 틈을 만들어** 푼다: 한 번에 많이 · 주기는 길게(CAMP_FOE_RELAY_S/_N).
      //     무리를 정리하면 다음 무리까지 남는 시간이 곧 **벽을 치는 시간**이다 —
      //     그래서 「우리 화력이 무리를 제때 잡을 만큼 되는가」가 그 관문의 물음이 된다.
      if(tgt){
