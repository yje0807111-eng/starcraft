  ...
  campWithStk(function(){
    for(const pair of byTgt){
      ...
      for(let i = 0; i < cnt; i++){
        ...
        // 🚫 **「닿으면 더 안 다가간다」는 여기서 풀 문제가 아니다** (2026-08-31 · 두 번 실패).
        //   목표: 긴 사거리 유닛이 앞으로 안 나가게 해서 **층**을 만든다.
        //   ⛔ 시도 ① 「닿으면 아예 안 움직인다」 → 층은 생겼지만(공성전차 66 → 478)
        //     **앞줄이 굳어 뒷줄이 막혔다.** 12기 중 4기만 사거리에 들어 스모크 71% → 0%.
        //   ⛔ 시도 ② 「다가가는 거리만 지금 거리로 묶고 옆으로는 벌린다」 → 33%. 여전히 부족.
        //   ⭐ 원인은 교전 이동이 아니라 **자리(_post)** 다. 뭉쳐 세운 부대는 앞줄이 멈추는 순간
        //     뒷줄이 갈 곳이 없다 — 사거리별로 **애초에 다른 줄에 서 있어야** 층이 유지된다.
        //   → 다음에 손댈 곳은 배치(_post)이지 여기가 아니다.
...
//
//   ⛔ **레인저 사거리는 자리 제한(CAMP_ENG_OUT 500)과 짝이다** (2026-08-31 실측).
//     4칸 → 3칸 으로 내려 봤더니 **짧은 사거리 유닛이 적에게 영영 못 닿았다**:
//     아군이 전부 자리에서 500(상한)까지 나가 굳고, 적까지는 342~375 였다(사거리 140).
//     그 사이 아군만 12기 → 5기로 죽었다. 벤치로도 D1R18 → D1R14 로 나빠졌다.
//     ⭐ 500 은 **레인저가 4칸일 때** 최적으로 측정된 값이다. 한쪽만 바꾸면 조합이 깨진다.
//     → 레인저를 내리려면 **CAMP_ENG_OUT 도 함께** 다시 재야 한다.
//
//   🏃 **s = 이동 속도(칸/초)** — 값이 없으면 엔진 원본을 그대로 쓴다.
//     ⚠ 캠프는 라운드마다 자리를 다시 잡아야 해서, 느린 유닛은 **싸우지도 못하고 라운드가 끝난다.**
//       실측(2026-08-31): 500px 이동에 공성전차 4.0초 · 레인저 1.5초(속도 234 vs 342).
//     ⭐ 그래서 느린 유닛의 바닥을 올렸다 — 공성전차·기갑병 5.0 → **6.5칸/초**, 전함 → 6.0.
//     ⭐ 그러고 화면에서 보니 **이번엔 나머지가 너무 빨랐다**(사용자 판단 2026-08-31):
//       「공성전차 속도가 맞는 속도고 다른 유닛들이 너무 빠르다」.
//       → **눈에 띄게 빠른 것만** 내렸다: 전투기 9.2 → **7.0** · 폭격기 8.5 → **6.5** ·
//         보병 계열(레인저·의무병·저격수·레이서) 7.3 → **6.8**.
//         화력병(6.2)·기갑병·공성전차(6.5)는 그대로 둔다.
//     ⚠ **s 를 안 준 유닛은 엔진 원본이 그대로 튄다** — 폭격기(8.5)와 수송선(10.0)을 그렇게
//       빠뜨렸다. 속도를 손볼 땐 **TECH_TREE 의 produces 를 훑어 전 유닛을 찍어 보고**
//       튀는 값이 없는지 확인할 것. 눈에 보이는 몇 개만 고치면 반드시 남는다.
//     ⚠ 일꾼(5.4)·지원정찰기(5.0)는 느린 쪽이라 그대로 둔다. 핵미사일(nuke)은 유닛이 아니다.
//     ⚠ 기준은 **공성전차 6.5** 다 — 그보다 빠르되 크게 차이 나지 않게 모은다.
//       ⛔ 전차가 가장 빠른 축이 되지 않게 할 것(원작 감각이 뒤집힌다).
//
...
const CAMP_UNIT_STAT = {
  ...
  marine:{a:1,h:5,r:4.0,c:1.0,s:6.8}, machinegun:{a:2.2,h:8,r:1.5,c:0.9}, racer:{a:2.5,h:7,r:4.0,c:0.8,s:6.8},
  goliath:{a:3,h:14,r:6.0,c:1.0,s:6.5}, ghost:{a:4,h:9,r:8.0,c:1.6,s:6.8}, medic:{h:23,r:2.0,s:6.8},
  pelican:{h:27,s:7.0}, aegis:{h:23}, tank:{a:12,h:22,r:9.0,c:2.2,s:6.5}, skyguard:{a:4.5,h:22,r:5.0,c:1.0,s:7.0},
  hellfire:{a:10,h:28,r:6.5,c:1.6,s:6.5}, dreadnought:{a:31,h:47,r:7.0,c:2.0,s:6.0},
  // 스웜 §3-A
...
function campDesignStat(u){
  ...
  // 🏃 이동 속도 — 값이 있을 때만 덮는다(없으면 엔진 원본 그대로).
  //   ⚠ 캠프는 라운드마다 자리를 다시 잡아야 해서 느린 유닛은 **싸우지도 못하고 끝난다.**
  if(d.s != null) u.spd = d.s * CAMP_STAT_TILE;
  return true;
}
