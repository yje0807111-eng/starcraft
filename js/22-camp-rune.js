...
//   ⭐ 숫자를 손으로 스물일곱 개 찍지 않는다 — **성좌마다 「첫 칸 ~ 마지막 칸」 두 값**과
//     유니크 셋만 적고 나머지는 그 사이를 균등하게 나눠 낸다(반지름 하나로 자리를 내는 것과 같은 어법).
//     ⛔ 생성된 배열을 리터럴로 되돌리지 말 것 — 폭을 바꾸려면 아래 두 줄만 고치면 된다.
//   📐 **성좌가 끝나야 그 한가운데가 열린다**(위 규칙) — 그래서 각 성좌의 마지막 칸은
//     제 유니크보다 **낮아야 한다**(14 < 15 · 29 < 30 · 49 < 50). ⛔ 겹치게 두지 말 것.
//   📈 간격이 **저절로 벌어진다**: 성좌① 1~2레벨 · ② 2레벨 · ③ 2~3레벨.
...
const RUNE_SLOT_LV = (function(){
  const norm = [];
  for(const sp of RUNE_LV_SPAN)
    for(let j = 0; j < RUNE_CONS; j++)
      norm.push(Math.round(sp[0] + (sp[1] - sp[0]) * j / (RUNE_CONS - 1)));
