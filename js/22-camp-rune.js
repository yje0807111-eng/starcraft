...
// ⬛ **낀 칸의 검은 바닥은 카드보다 한 뼘 작다**(2026-09-13) — 카드 그림을 판의 **윤곽대로** 다시 깎으면서
//   카드가 네모 상자보다 좁아졌다(가로 109~115/128 · 꼭짓점도 1.4px 들어갔다). 같은 크기의 바닥이면 그 틈으로
//   다시 비치고, 카드의 흐린 가장자리 밑에 깔려 **검은 테를 도로 만든다**. 그래서 바닥은 가장자리에서 떨어뜨린다.
//   ⚠ 카드 속은 불투명이라 바닥은 **날아오는 동안(.veil)** 칸 자리를 잡아 주는 일만 한다.
const RUNE_FLOOR_K = 0.90;
...
function _runeCell(kind, i, x, y, r, key, open, at, sel){
  ...
  if(!key){
    ...
    //   📐 **반지름은 카드 안쪽에 숨는다**(r × RUNE_CELL_OUT × RUNE_FLOOR_K · 2026-09-13 사용자: 「판 뒤의 검은 배경이 보인다」).
    //     옛 값 r 은 높이 2r 인데 카드는 1.86r 이라, 위아래 꼭짓점과 빗변에서 바닥이 7.5% 삐져나와
    //     카드 둘레에 검은 테가 남았다. ⛔ r 로 되돌리지 말 것(스모크가 **보이는 카드 윤곽**과 잰다).
    g.push('<polygon class="rnHxFloor" points="' + _runeHexPts(x, y, r * RUNE_CELL_OUT * RUNE_FLOOR_K) + '"/>');
