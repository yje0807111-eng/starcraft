...
| 마디 | 파일 | 그림 |
|---|---|---|
| **가운데**(새로운 시작) | `root` | 사방으로 빛살을 뻗는 **점화 코어** — 주황 `#ff7a4a` |
...
⭐ **가운데는 갈래 넷 어디와도 색이 안 겹치는 주황**이다. 모양은 다른 별과 같은 어두운 육각이고,
다른 것은 셋뿐 — 더 크고(rr 16), 색이 주황이고, 그림이 점화 코어다.
⛔ 가운데를 색으로 꽉 채우지 말 것 — 그러면 그 위의 그림이 묻힌다(옛 붉은 마름모에 그림이 없던 이유다).
...
## 16. 룬 아이콘 계열 — 판 4 × 문양 11 = 25 (2026-09-04)

⭐ **한 장씩 뽑지 않는다.** 룬은 「등급(판) × 종류(문양)」의 곱이라 25장이 필요한데,
그걸 다 뽑으면 크기·위치가 제각각이 된다(실제로 그렇게 나왔다). 그래서 **재료 15장만 뽑고
스크립트가 겹친다** — `node scripts/rune-compose.mjs`.

⛔ **AI 로 합성하지 말 것.** 두 번 시도해 두 번 다 실패했다 — 모델이 「색이 다른 두 요소를
그대로 두고 합치는 것」을 못 하고, 문양을 판 색으로 물들여 버린다. 지시를 뒤집어 써 봐도 같았다.

### 16-1. 규격

| 항목 | 값 |
|---|---|
| 재료 | 판 4장(`tile_low/mid/high/uniq`) + 문양 11장(`sym_<id>`) · `docs/mock/rune/` |
| 뽑는 크기 | 1024×1024 · 배경 순수 검정 |
| 나오는 것 | 25장 · 128×128 WebP(알파) · `assets/icons/rune/<id>_<등급>.webp` |
| 용량 | 합계 112KB · 평균 4.5KB |
| 조합 | 일반 7종 × 3등급 = 21 · 유니크 4종 × 1 = 4 |

### 16-2. 판(타일) — 등급을 나타낸다

꼭짓점이 위로 오는 **정육각형**(`_runeHexPts` 의 `60·i − 90` 과 같은 방향).
색은 `RUNE_GD` 가 단일 소스다 — 하급 `#9aa4b0` · 중급 `#5cd6ff` · 상급 `#ffcf6b` · 유니크 `#c98bff`.

⚠ **면이 아니라 띠가 색을 맡는다.** 안쪽 면까지 물들이면 그 위에 얹을 문양이 죽는다.
세 번 오가며 잡은 균형이다(안 보인다 → 너무 진하다 → 테두리가 밝다 → 지금).

### 16-3. 문양 — 종류를 나타낸다 · **색은 세 군뿐**

⛔ 룬마다 색을 다르게 하지 말 것. 11색을 다 쓰면 한 화면이 시끄럽고, 판 4색과 겹치는 조합이
반드시 생긴다(금색 판 + 초록 문양이 서로 뭉갰다). **효과군 3색**으로 좁힌다.

| 군 | 색 | 룬 |
|---|---|---|
| 경제 | `#7effc9` 민트 | 손끝 · 정제 · 전리품 · 열기 |
| 전투 | `#ffa3b8` 분홍 | 힘 · 연타 · 수호 · 치유 |
| 구조·속도 | `#e6eef8` 은백 | 증원 · 가속 · 신속 |

⚠ 분홍은 처음에 살구(`#ffb894`)였는데 **상급 판(금)과 계열이 가까워** 분홍 쪽으로 옮겼다.

⛔ **한 모티프는 한 룬에만**(§15-3 과 같은 규칙). 첫 안에서 연타·가속·신속이 전부 화살표라
색만 빼면 같은 그림이었다 — 리듬 막대 / 모래시계 / 잔상 화살로 갈랐다.

### 16-4. 스크립트가 하는 일

1. **잉크 경계로 가운데 맞추기** — 판도 문양도 프레임 안에서 조금씩 치우쳐 나온다.
   밝은 픽셀의 경계 상자를 재서 그 중심을 맞춘다. ⛔ 이미지 중심을 그대로 믿지 말 것.
2. **screen 으로 얹기** — 문양은 검정 배경 위 발광 그림이라 검정이 저절로 사라진다.
   ⛔ `over` 로 얹으면 검은 사각이 판을 가린다.
3. **판의 윤곽을 딴 마스크로 알파** — 안 하면 성좌 판에서 **육각형 뒤에 검은 사각**이 남는다.
   ⛔ 「어두운 픽셀을 투명하게」로 하지 말 것 — 판 안쪽 면도 어두워서 구멍이 뚫린다.
   🚨 **정육각 마스크는 판보다 컸다**(2026-09-13 사용자: 「판 바깥의 검은 테두리가 남아 있다」) — 윤곽 픽셀의 96% 가
   검정이었다(157장 전부). 판은 정육각이 아니다: 가로 비율이 넷이 다르고 **아랫 빗변이 1~2px 안으로** 들어가 있다.
   ⭐ 그래서 판 가운데에서 광선 720개를 쏘아 **바깥에서 들어오며 처음 밝은 픽셀**(문턱 40)을 잇고, 1.4px 들여 씌운다.
   처음 만난 테에서 멈추므로 안쪽 면에는 안 닿는다(구멍이 안 뚫린다).
   ⛔ 어떤 육각(정육각 · 가로세로 따로 맞춘 육각)으로도 되돌리지 말 것 — 아랫 빗변에 검정이 남는다(96% → 45%).
   ⛔ 줄인 판의 가운데를 64 로 치지 말 것 — contain 이 정수로 붙여 반 픽셀 치우친다(오른쪽 옆변에만 검은 줄).
   ⚠ 들이는 거리 0.6 은 가장 바깥 한 줄이 검정과 섞여 남는다 — 0.6·1.0·1.4 를 견줘 1.4 로 정했다.
   ⚠ 하급(회색) 판의 아랫면은 **그림자로 원래 어둡다**(판의 일부) — 검은 테가 아니다.
4. **룬별 보정**(`TWEAK`) — 눈에 보이는 가운데는 기하학적 가운데와 다르다.
   기본은 3.4px 올림이고, 수호·열기는 오히려 1.8px 내리고 가속은 크기를 줄인다.

### 16-5. 프롬프트

판·문양 모두 **§15-2 공통 블록을 그대로** 쓰되, 문양은 `TREATMENT` 를 굵고 밝게 바꾼 판을 쓴다
...
| 항목 | 값 | 왜 |
|---|---|---|
| 덮을 세로 | 격자 **−0.58 ~ 1.0** | 그림 위 6.5%(광맥 홈) ↔ 적 광맥 gy −0.48(`CAMP_FOE_ROW.mine`) · 아래 100% ↔ gy 1.0 → 높이 1.58 gy |
| CSS | `.bmapFloor::before{top:-58%}` + `background-size:auto 100%`(css/30-home.css) | 바닥 요소 자체는 격자(0~1)라 **::before** 를 위로 58% 늘려 깐다 — ⛔ 바닥 요소에 바로 배경을 주면 격자 위가 검게 남는다(실측) |
| 비율 | **9:16 으로 뽑고 양옆을 잘라 1:2** — `node scripts/map-crop.mjs <입력> <출력.webp>` | 생성기에 1:2 가 없다(2026-09-10 사용자). 세로는 그대로(세로 비율이 곧 게임판) · 1:2 로 깔면 폭이 요소의 1.55배 → 좌우 27% 씩 넘친다(가장자리는 지형지물 띠라 잘려도 된다) |
| 모델 | `gpt_image_2` · `9:16` · `2k`~`4k` · `high` | §11 과 같다(구조를 지키는 모델) · ⚠ `_min` 미리보기(752×1344)가 아니라 **원본 크기**로 받을 것 |
...
**그림 안 위치(위 0% → 아래 100%)** — gy → (gy + 0.58) / 1.58. 적 줄은 `CAMP_FOE_ROW`(23-camp-dungeon)에서 환산.
⭐ 아래 비율은 **첫 그림(2026-09-10)이 실제로 낸 것**을 기준으로 삼았다 — 모델이 요청(13~42%)보다 짧은 고원(8~27%)을 냈고, 그림을 다시 뽑는 대신 **적 기지 줄을 조여서**(`CAMP_FOE_ROW` 간격 0.08 · 사용자: 「미네랄을 더 위로 · 건물은 판 안에 더 작은 비율로」) 고원 안에 넣었다. 그래서 공통 블록의 LAYOUT 도 이 비율이다.

| 구역 | 세로 | 내용 |
|---|---|---|
| 상단 여백 | 0 ~ 6% | 벽·경계 — 재화 바·칩 아래(`campViewTop` 여유 0.18 안) |
| **적 광맥 홈** | 6 ~ 8% | 위로 볼록한 얕은 호 하나(가로 30~70%) — 광맥(gy −0.48) 이 그 위에 선다 |
| **적 기지 고원** | **8 ~ 27%** | 평평한 한 덩어리 · **대좌 없음**(자리는 랜덤) · gy −0.45 ~ −0.15: 본진 −0.42(10%) · 테크 −0.38/−0.30 · 탑 −0.34/−0.26/−0.18 · 생산 −0.22 · 보급고는 탑 줄의 좌우 0.19/0.81 |
| 통로 | 27 ~ 66% | 적이 내려오는 길 · 장애물 없음 · 격자 위끝 0.18 = 48% |
| **내 기지 석판** | **66 ~ 100%** | 석판 위끝 gy 0.46 · 본부 gy 0.64 → 77% · 광맥 줄 그 아래 · §11-2 ① |

⚠ **캠프 배경(0번)의 석판은 이 표보다 높다** — 눈금을 그려 재니(2026-09-10) **난간·문설주 gy 0.33~0.36 ·
노란 빗금 0.36~0.39** 이었다. 회복 구역 경계(`CAMP_HEAL_GY` 0.36 · 19-camp.js)가 그 문간이다.
⛔ 이 표의 0.46 을 그대로 회복 경계에 쓰지 말 것 — 석판 한참 안쪽에서 끊긴다.
그림을 새로 뽑아 석판 위끝이 옮겨지면 **눈금을 다시 찍어** `CAMP_HEAL_GY` 를 맞출 것.
| 지형지물 | 가로 0 ~ 22% · 78 ~ 100% | 화면 밖으로 넘치는 좌우 띠 + 위 모서리 |

⚠ 모델은 좌표를 못 지킨다 — 뽑은 뒤 던전 1 에 들어가 표식(`campFoeMarks`)이 **고원 안에 앉는지** 눈으로 본다(가장 빠른 길: 그림을
프로젝트 루트에 두고 `--campBg` 를 그 URL 로 바꿔 스크린샷 — 이 절의 수치가 그렇게 나왔다). 고원이 어긋나면 그림을 다시 뽑지 말고
`::before` 의 `top` 을 손본다(고원 위끝이 gy −0.48 에 오도록).

### 17-2. 방법 — 0번(캠프)을 먼저, 던전은 그것을 첨부해 위쪽만 바꾼다 (§11-1 그대로)

내 기지 석판(66~100%)은 **4장이 같아야** 한다(원정에서 돌아와도 같은 집). 캠프 0번을 마음에 들 때까지 뽑고,
...
above (orthographic, no perspective tilt, no horizon). Portrait 9:16 (the sides get cropped later,
so keep everything important inside the middle 56% of the width).
...
LAYOUT (top to bottom, of the full image height):
  0–6%    a dark bounding wall or ridge — this strip is hidden under the top bar.
  6–8%    one shallow ARC-SHAPED GROOVE bowing upward, spanning the middle 40% of the width —
          a resting mark where mineral crystals will be placed. Nothing else on it.
  8–27%   ENEMY BASE PLATEAU: one broad, flat, completely EMPTY platform of the enemy's material,
  ...
  27–66%  OPEN CORRIDOR: a wide empty passage down the centre, plain ground, nothing blocking it.
  66–100% PLAYER BASE SLAB: one broad stone-and-metal platform spanning the FULL width and running
          off both sides and off the bottom edge, completely empty. Same material in every image
          of this set.
...
OUTPUT: 9:16 portrait, no text, no UI, no frame.
--- NEGATIVE ---
...
```
Home camp. The enemy plateau (8–27%) is here just quiet unused ground of the same grey concrete,
its rim barely visible, as if nothing has settled there yet; the arc groove above it is faint.
The player slab (66–100%) is the hero of this image: large interlocking blue-grey deck plates with
...
3. `node scripts/map-crop.mjs <원본.png> assets/backgrounds/camp/camp.webp`(던전은 `dungeons/dgN.webp`) — 양옆 크롭 + WebP q82 · 세로 2688.
...
### 19-8. 🔁 **일관되게 뽑는 방법** — 프롬프트가 아니라 「어떻게 묶어 부르느냐」다

2026-09-13 사용자 질문: **「낱개로 뽑으면 서로 안 맞는다. 어떻게 그렇게 일치시켰나?」**
답은 프롬프트 문장이 아니다. **부르는 방식 다섯**이고, 순서대로 중요하다.

#### ① ⭐⭐ 같이 보여야 할 것은 **한 장에 함께 그린다** (제일 큰 요인)
건물 여섯을 **3×2 격자 한 장**으로 뽑는다. ⛔ 여섯 번 따로 부르지 말 것.
한 장 안의 것들은 **같은 붓질·같은 조명·같은 각도**로 저절로 맞는다 — 낱개로는 아무리 같은
문장을 써도 안 맞는다. 유닛 넷도, 팀 색 여덟도, 8방향도 전부 **한 장**이다.
⚠ 대신 한 장에 너무 많이 넣으면 개별 해상도가 떨어진다 — **4~8칸**이 한계다.

#### ② 🔗 그다음 장부터는 **앞 장을 레퍼런스로 물린다**
말로 각도를 설명하지 말고 **승인된 그림 자체**를 첨부한다(이 저장소에서는 `image_references`
에 앞 작업의 job_id). 그러면 각도·색·선 굵기·톤이 통째로 따라온다.
📏 §19-4 가 이걸로 고쳐진 자리다 — **같은 문장인데도** 회전 시트만 각도가 샜고, 그림을 물리니 맞았다.

#### ③ ✂ **「그대로 두고 한 가지만 바꿔라」**로 말한다
새로 묘사하지 않는다. 「첨부한 시트에서 **세로만 2.3배**로」 「나머지는 동일하고 **요만 45°**로」.
⛔ 전체를 다시 describe 하면 모델이 전부 새로 그린다.

#### ④ 📋 **고정 블록을 글자 그대로 복사**하고 장면 한 칸만 바꾼다
§19-2 의 다섯 덩어리(PROJECTION / CAMERA / PROPORTIONS / COLOR / DESIGN LANGUAGE / STYLE)를
**요약하거나 다듬지 말 것.** 한 낱말만 달라도 톤이 미묘하게 갈리고, 한 화면에 세우면 바로 티가 난다.
(§15-2·§18-2 도 같은 규칙이다 — 이 저장소가 여러 번 겪은 일이다.)

#### ⑤ 🧪 **한 번에 한 축만** 바꿔서 나란히 뽑는다
각도 사다리(30/40/50/60)처럼 **나머지를 전부 고정**하고 하나만 바꿔 **같은 배치로 4장**을 낸다.
그래야 무엇이 달라졌는지가 보인다. ⛔ 두 가지를 같이 바꾸면 무엇 때문인지 영영 모른다.

#### ⚠ 그래도 안 맞는 것 — 이건 방법의 한계다
- **8방향 회전**은 ①②를 다 써도 여덟이 미묘하게 다른 유닛이 되기 쉽다.
  → **2×2 4방향**이 칸당 크기가 커서 훨씬 안정적이다. 각도 확인은 4방향으로 한다.
- 낱개 생성이 꼭 필요하면(큰 그림 한 장) **②를 반드시** 쓴다. ①이 없으면 ②가 유일한 끈이다.
