...
| **유닛 지정 중 내 건물 탭** | `techPtrUp`(17-build-cards.js) — **건물이 지정되고 유닛 지정은 풀린다** | 2026-09-10 사용자 확정 · ⛔ 옛 규칙(「그 자리로 이동」)으로 되돌리지 말 것 — 건물을 고르려면 ⊘ 로 유닛을 먼저 풀어야 해서 두 번 눌러야 했다 · ⚠ 그 대신 **건물 위를 탭해 이동시키는 길이 사라졌다**(건물 옆 바닥을 탭한다) · ⚠ 롱프레스(탑승·수리)는 다른 갈래라 그대로 산다 · ⚠ 예외 하나 — 일시정지 건물 + 유휴 일꾼(`_keepWk`)은 재개에 쓸 일꾼 지정을 붙들어 둔다 |
