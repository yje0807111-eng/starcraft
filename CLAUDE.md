...
- 🚨 **2026-09-08 전면 개편이 확정됐다 — `GAME_DIRECTION.md` §0-A 를 먼저 읽을 것.** 게임의 뼈대가 바뀐다:
  **던전 10 → 3**(적 종족과 짝: 유니온 기지 → 스웜 기지 → 에테리얼 기지) · **종족 선택 없앰**(유니온 → 스웜 → 에테리얼 **순차** · 「종족 변이」로 넘어간다) ·
  ...
  🔁 **환생은 하나뿐**(옛 2차 환생 · 무한층에서 막히면 · 스킵·자동화·기본 배수·상한 해제를 판다) ·
  ...
  **재화는 지키되 죽은 유닛은 다시 산다**(공짜 복구는 무한 재시도라 판단이 사라진다).
  ...
  🔁🔁 **2차 환생 뼈대가 들어왔다**(2026-09-11 · `campRebirth2`/`campCanRebirth2`/`CAMP_RT2_LINES`).
  조건은 **던전 3 완주**(`C.dgDone[3]`) · 포인트는 **도달 관문 수**(⛔ 번 돈 아님 · 한 바퀴 18) ·
  배수는 **합**(`campRebMul` = 1 + 2차 기본 + 1차 누적 · ⛔ 곱 금지) · 실행하면 **1차를 통째로 비운다**
  (`rebMul`·`rbTree`·`rbPts`·`reb`·`best`·`dgDone` · ⛔ best·dgDone 을 남기면 무한 적립이 된다) ·
  💠 룬과 ⏱ 최고기록은 남는다. **갈래 넷이 다 열렸다**(2026-09-11): 기본 배수 · 스킵 · 자동화 · 상한 해제.
  ...
  2차 트리 전부 사는 값 **39점**, 한 바퀴가 18점이라 **두세 바퀴에 완성**된다(⛔ `cs` 손값을 되살리지 말 것).
  🔓 상한은 **`campCap('worker'|'supply'|'unitR')` 하나**가 단일 소스다(⛔ 상수를 직접 읽지 말 것 —
  트리를 사도 안 열린다 · 사다리 0차 = 지금 상수라 안 사면 똑같다).
  🔒 **트리 스위치가 새면 안 된다** — 2차 트리를 보는 동안에도 캠프 경제·전투는 계속 도는데,
  효과 함수(`campRtMul`·`campRtCut`·`campRtFoeMul`·`campRtNodeAdd`)가 2차 자루를 읽으면
  **1차 배수가 통째로 1 이 된다.** 그래서 그 넷은 `campWithTree1` 로 **못 박혀 있다** —
...
| **환생 트리(마인드맵)** | `#campTree` + `campTreeOpen(two)` · 노드는 `campTreeSvg()` | `#phone` 직속 전체 화면 · 밀고 확대 · 입구는 **하단 네비 「환생 › 트리 / 2차 트리」**(⛔ 맵 띠에 칩을 되돌리지 말 것) · 🌳 **화면 하나가 두 트리를 다 그린다**(2026-09-11) — 표만 갈린다(`campRtLines()`/`campRtBRs()` · 갈래 색은 `CAMP_TREE_BR`/`CAMP_TREE2_BR`) · ⛔ **두 번째 트리 렌더러를 만들지 말 것** · ⚠ 2차 갈래·묶음 마디는 **그림이 아직 없다** — 그 갈래 첫 계열 그림을 빌린다(⛔ 없는 파일 이름을 돌려주지 말 것: 깨진 그림이 뜨는데 화면은 멀쩡해 보인다) |
