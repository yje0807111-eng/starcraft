...
| 세그먼트 이동 바(한 구역 안에서 나뉘는 네비) | **`segNavHTML(items, i, act)`** → `.pdSeg`/`.pdSegInd`/`.pdSegBtn` | 장비창 섹션 바(장비·장신구) = 사냥터 업그레이드 탭(공격·방어·유틸) — 한 함수 · 미끄러지는 판이 현재 구역을 가리킨다 · 선택 밑줄 색만 화면이 덮는다(HOME 계열은 시안 금지 → 빨강) |
