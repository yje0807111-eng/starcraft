...
- 🚨 **2026-09-08 전면 개편이 확정됐다 — `GAME_DIRECTION.md` §0-A 를 먼저 읽을 것.** 게임의 뼈대가 바뀐다:
  **던전 10 → 3**(적 종족과 짝: 유니온 기지 → 스웜 기지 → 에테리얼 기지) · **종족 선택 없앰**(유니온 → 스웜 → 에테리얼 **순차** · 「종족 변이」로 넘어간다) ·
  ...
  포인트 = **도달 깊이**(통산 관문 수) · 배수 = **1 + 환생 횟수** · 되감김에 **레벨·성장 트리**가 포함된다.
  ⛔ 「번 돈」(`campWealth`)을 관문·포인트의 자로 되돌리지 말 것 — 돈→포인트→배수→돈 되먹임이 그거였다.
  ⛔ 배수를 난이도 로그(`CAMP_REB_K`)로 되돌리지 말 것(없앴다). 🚧 **환생 포인트는 쓰는 곳이 아직 없다**
  (스킵·자동화·기본 배수·상한 해제 넷이 다음 단계 · 화면이 「준비 중」이라고 말한다) ·
  ...
  ⚠ 레벨 값 셋(`CAMP_XP_KILL`·`CAMP_XP_A`·`CAMP_XP_R`)은 **안 쟀다** — 근거는 「요구량 배수 > 관문 평균(1.36)」 하나(BALANCE §0).
  ⛔ **배수 항은 전부 「더한다」 — 곱하지 말 것**(`campRebMul` = 1 + 환생 횟수 + 환생 트리 몫 ·
  실측 5회 만에 ×1,900만 · BALANCE §0).
  ...
  **재화는 지키되 죽은 유닛은 다시 산다**(공짜 복구는 무한 재시도라 판단이 사라진다).
  ...
  🌳🔁 **환생 트리가 들어왔다**(2026-09-11 · `CAMP_RBT_LINES`/`CAMP_RBT_BR`/`campRbtVal`).
  ⚠ 이건 **환생 포인트(`C.rbPts`)로 사는 트리**이고, 성장 트리(`CAMP_RT_LINES`·`C.rbTree`)는
  **레벨 포인트**로 산다 — 화면(`#campTree`)과 렌더러는 하나이고 표만 갈린다(`campRtLines()`).
  ⛔ **자루 두 이름을 헷갈리지 말 것**: 환생 트리 `C.rebTree`(환생을 넘어 **남는다**) ↔
  성장 트리 `C.rbTree`(환생이 **비운다**). 한 글자 차이인데 되감기 규칙이 정반대다.
  ⛔ **옛 「2차 환생」 함수(`campRebirth2`·`campCanRebirth2`·`campReb2*`)를 되살리지 말 것** —
  메인이 환생을 한 층으로 합치면서 같은 것이 두 벌이 되어 지웠다(스모크가 이름을 잰다).
  **갈래 넷**(2026-09-11): 기본 배수 · 스킵 · 자동화 · 상한 해제.
  📐 **갈래는 90° 씩 벌린다** — 성장 트리(갈래 셋)의 각을 베끼고 넷째만 더했더니 `-0.48π` 와
  `1.52π` 가 **같은 방향**이라 기본 배수와 자동화가 겹쳐 그려졌다(실측). 넷을 기준으로 다시 나눈다.
  🎫 **스킵** — `dgStart`(관문 n채를 부순 채로 시작) · `startRes`(회차 시작 미네랄).
  ⛔ `dgStart` 를 `C.broken` 숫자만 올려 구현하지 말 것 — 릴레이(`campFoeActive`)는 **실제로 죽은
  건물**을 보므로 난이도는 관문 n 인데 웨이브는 관문 1 짜리가 된다(실측으로 겪었다). `C.foeDead`
  에 찍는다(eid 는 `'fb<던전>_<차례>'` 로 고정) · ⚠ **문지기 탑은 안 건너뛴다**.
  🤖 **자동화** — `autoWk`(일꾼) · `autoRes`(무장 연구) 둘뿐이다(`campAutoTick` · 캠프 시계가 부른다).
  ⭐ **켜고 끌 이유가 없는 것만** 골랐다: 일꾼은 상한까지가 늘 이득이고, 연구는 **가스 전용**이라
  미네랄과 안 다툰다. ⛔ **자동 유닛 구매를 스위치 없이 넣지 말 것** — 업그레이드를 사려고
  모으는 미네랄을 자동화가 먼저 써 버린다. 자동 건설은 자리 찾기가 붙어 아직 없다.
  ⚠ 자동화는 **주기(2초)마다 하나씩**만 한다(⛔ 한 틱에 몰아 사지 말 것 — 화면이 튀고 끼어들 틈이 없다) ·
  구매는 사람이 누르는 **같은 함수**(`techDoProduce`/`techDoResearch`)를 부르되 그동안 **입을 막는다**
  (`campAutoQuiet` — 2초마다 확인음·토스트가 나면 자동화가 아니라 고장으로 보인다).
  📐 값은 **등급이 정한다**(메인이 2026-09-11 에 차수를 1로 없앴다 · `CAMP_RT_MAX_DEF`) —
  환생 트리 전부 사는 값 **37점**, 한 바퀴가 18점이라 **두세 바퀴에 완성**된다(⛔ `cs` 손값을 되살리지 말 것).
  🔓 상한은 **`campCap('worker'|'supply'|'unitR')` 하나**가 단일 소스다(⛔ 상수를 직접 읽지 말 것 —
  트리를 사도 안 열린다 · 사다리 0차 = 지금 상수라 안 사면 똑같다).
  🔒 **트리 스위치가 새면 안 된다** — 환생 트리를 보는 동안에도 캠프 경제·전투는 계속 도는데,
  효과 함수(`campRtMul`·`campRtCut`·`campRtFoeMul`·`campRtNodeAdd`)가 환생 자루를 읽으면
  **성장 배수가 통째로 1 이 된다.** 그래서 그 넷은 `campWithGrowTree` 로 **못 박혀 있다** —
  ⛔ 새 효과 함수를 만들면 반드시 거기 태울 것(스모크가 누수를 잰다).
  📊 **값을 쟀다**(2026-09-11 · `node scripts/rbtree-bench.mjs` · BALANCE §5-12) — 여덟 중 **다섯은 맞고 셋이 어긋난다**.
  ⚠ `dgStart` 는 관문 2채를 건너뛰는데 클리어가 **14%만** 준다(릴레이가 관문 3 부터 돌아 적이 더 세다) —
  ⛔ 릴레이가 `C.broken` 을 보게 만들어 고치지 말 것(스킵이 난이도를 낮추면 「사면 쉬워진다」가 된다).
  ⚠ `capWk` 는 **상한이 벽이 아니라 값이 벽**이다(일꾼 40→80: 수입 1.7배 · 사는 값 **67배**) —
  고칠 길 셋은 §5-12 ② 이고 ⚠ **아직 안 골랐다**.
  ❌ `capSup`(보급소 상한)은 **죽은 줄**이다 — 엔진이 인구를 `TECH_SUP_MAX`(200)에서 자르므로
  24채면 이미 상한이고 25채부터는 지어도 인구가 안 는다(§5-12 ③). ⛔ `TECH_SUP_MAX` 를 올려
...
| **환생 트리(마인드맵)** | `#campTree` + `campTreeOpen(two)` · 노드는 `campTreeSvg()` | `#phone` 직속 전체 화면 · 밀고 확대 · 입구는 **하단 네비 「환생 › 성장 트리 / 환생 트리」**(⛔ 맵 띠에 칩을 되돌리지 말 것) · 🌳 **화면 하나가 두 트리를 다 그린다**(2026-09-11) — 표만 갈린다(`campRtLines()`/`campRtBRs()` · 갈래 색은 `CAMP_TREE_BR`/`CAMP_RBT_BR`) · ⛔ **두 번째 트리 렌더러를 만들지 말 것** · ⚠ 환생 트리의 갈래·묶음 마디는 **그림이 아직 없다** — 그 갈래 첫 계열 그림을 빌린다(⛔ 없는 파일 이름을 돌려주지 말 것: 깨진 그림이 뜨는데 화면은 멀쩡해 보인다) |
