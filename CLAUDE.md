...
- 🗺 **지형(벽·언덕·램프)은 `js/24-terrain.js` 가 소유한다**(2026-09-10 · 구조는 `ARCHITECTURE.md` §「지형층」). 층이 셋이다: **바닥**(통 텍스처 `.bmapFloor`) · **지형**(투명 배경 `.bmapTerr` · z 1) · **건물·유닛**(3D). ⛔ 바닥이 통째로 들어간 「완성 벽 타일」로 되돌리지 말 것 — 바닥 종류마다 벽 세트를 다시 뽑아야 하고 언덕이 들어오면 조합이 곱해진다.
  ...
  - 🚪 **램프를 빼지 말 것** · 🚧 **길이 막히면 안 된다**(`campTerrConnected`) · 🧱 벽은 **0.05~0.16 띠에만**(내 격자·적 기지 줄과 겹치면 건물 위에 선다) · ⛰ 고원 밑변은 **계단**(열마다 따로 굴리면 빗살이 된다).
  - 🎬 **손댔으면 `node scripts/camp-terr-shot.mjs` 로 눈으로 볼 것.** 👁 시야 차단·🚶 길막기는 **아직 배선 안 했다**(2·3단계) — 3단계는 `_techNavRects` 를 감싸고 **벽 칸을 큰 덩어리로 합쳐야** A\* 가 안 터진다.
