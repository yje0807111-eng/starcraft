...
- 🚨 **2026-09-08 전면 개편이 확정됐다 — `GAME_DIRECTION.md` §0-A 를 먼저 읽을 것.** 게임의 뼈대가 바뀐다:
  ...
  **1차 환생 하루** · 🆕 **2차 환생**(무한층 진입으로 열림 · 스킵·자동화·기본 배수·상한 해제를 판다) ·
  🆕 **무한층**(끝이 없다 · 최고기록 등반) · 환생 포인트·관문이 **「번 돈」이 아니라 「도달 던전·라운드」** 기준.
  ⛔ **2차 배수는 1차와 「더한다」 — 곱하지 말 것**(실측 5회 만에 ×1,900만 · BALANCE §0).
  ...
  ⚔🏰 **진군 중에는 적을 쫓지 않는다**(2026-09-09 · `js/21-camp-battle.js` `_march`) — 칠 적 건물이
  있으면 사거리 밖의 적 유닛은 **쫓아가지 않는다**(이동 목표는 건물). ⛔ 빼면 던전이 통째로 멈춘다:
  쫓는 목표는 자리에서 1200 안으로 잘리고 건물 목표는 안 잘려서, 적이 하나만 보여도 되돌아갔다
  다시 나아가기를 반복한다(실측 3분에 진행 2/6 · 건물 실효 화력 10.8% → 고친 뒤 5/6 · 31.2%).
  ⛔ 「적을 다 잡고 나서 나아간다」로 되돌리지 말 것 — 릴레이는 적이 끊이지 않아 그 조건이 영영 안 온다.
  ⚠ **아군(me)에만** 건다(적에 걸면 방어의 뜻이 사라진다).
  ...
  **재화는 지키되 죽은 유닛은 다시 산다**(공짜 복구는 무한 재시도라 판단이 사라진다).
  🔁 **2차 환생 한 바퀴 = 한 종족**(종족은 2차 환생 때 고른다) · 무한모드 진입은 **「던전 3 클리어」**.
  ⚠ **아직 구현은 시작 안 했다** — 문서만 확정됐다. 값(유닛 반복 구매 배수·테크 속도)은 설계가 굳은 뒤 실측한다.
...
| **캠프 종족** | **첫 바퀴는 유니온 고정** — `campEnter`/`campOpen` 이 `C.race='terran'` 을 박는다(2026-09-09 · REDESIGN_PLAN 1-E) | 종족 선택 화면(`#campRaceOv` · `campRaceSheet`/`campPickRace`)은 **다락**으로 갔다(ATTIC.md §5-D) — ⛔ 되살리지 말 것(스모크 「캠프: 종족 선택 · 진입」이 잰다) · 종족 변이는 **2차 환생(단계 4)** 의 몫이고 그때는 `segNavHTML(CAMP_RACE_ORDER)` 띠로 고른다 · ⚠ 첫 진입 연출(검은 판 → 캠프 → 튜토리얼 `tutoKick`)은 `campRaceToCamp()` 가 그대로 맡는다 — ⛔ `campEnter` 를 바로 부르지 말 것(튜토리얼이 안 뜬다) |
| **🏰 적 기지 화면**(던전) | **모델은 `js/23-camp-dungeon.js`** — `campFoeBld3D()`(3D 엔트리) · `campFoeMarks()`(표식) · `campFoeTgtName()`(띠) · `campFoeTapAt()`(탭) · `campFoeSheetModel()`(프로필) — **DOM 은 19-camp**(`campFoeOverlayHTML` → `#cstLabels .fbLayer`) | ③안 「밑변 광원」(2026-09-09 사용자 확정 · 목업 `docs/mock/camp-foebase-4.html`): **진행 건물 6채만 바닥이 붉다** · 다음 표적 = **붉은 모서리 + 「다음 표적」** · 못 본 건물 = **속 빈 실루엣**(이름 없음) · 잔해 = ✕ · 체력 선은 **맞은 건물에만** · 잠긴 구간의 진행 건물은 🔒 · ⛔ 번호 배지·깃발·후광 금지 · ⛔ **부수 건물을 물리지 말 것**(방어탑이 흐려지면 위험이 안 보인다) · 🏗 3D 는 **기지 건물과 같은 규약**으로 `M3D.syncBuild` 목록에 얹는다(`campWithBattleDraw` 가 전투 유닛 뒤에 덧붙인다 · id `cb_`+모델키 · fitW · z) — ⛔ 두 번째 3D 동기화 함수를 만들지 말 것(`techMap3DSync` 는 **아무도 안 부르는** 관리자 맵용이다 — 거기 확장했다가 되돌렸다) · 🗺 **적 기지는 격자 위 한 화면에 있다**(2026-09-09 사용자: 「지금은 너무 가깝다 — 화면 완전 위」) — `CAMP_LANE_TOP` 을 0.18 → **−0.26** 으로 내려 격자로 옮기는 기울기만 두 배로 했다(전장 좌표는 그대로 · ⛔ 전장 y 를 음수로 늘리지 말 것 — 18-strike 길찾기 셀이 음수를 못 받는다) · 표(`CAMP_DG` gy)는 그 격자 값이라 TOP 을 바꾸면 표도 옮긴다(gy′ = −0.26 + (gy − 0.18) × 2) · 👁 **던전에 들어가면 뷰가 위 끝(적 기지 전체)** 이고 아래로 끌면 내 기지가 나온다(`campFoeLookAt` · `campFoeBase` 끝에서 — ⚠ `campEnterDungeon` 때는 `CAMPB` 가 아직 없다 · **목표 뷰 t 만** 옮긴다: v 를 직접 쓰면 바닥·광맥·그림자가 옛 자리에 남는다 — 실측) · 위 한계는 `campViewTop()`(표의 맨 위 건물 − 0.18 · 상단바 몫 포함)이고 캠프(0)에서는 격자 위끝 그대로 — ⚠ 캠프는 `_techClampView` 를 **덮어쓴다**(19-camp 6858) — 거기의 yLo 가 진짜 경로다(17-build-cards 를 고쳐 봐야 캠프엔 안 걸린다 · 실측) · 🏕 원정이 끝나 캠프로 오면 `campZoom()` 으로 시점도 집으로(안 하면 보간이 남아 프레임 제한이 안 걸린다) · 🏗 3D 는 격자 위라 행이 음수 → z 가 −1000 아래로 내려가 카메라 범위(−1200)를 벗어난다 — `zOf` 를 −1190 에서 받는다(실측: 3D 가 통째로 안 보였다) · ⚠ 격자 위 한 화면의 **바닥 그림은 없다**(`.bmap` 배경색만) — 적 진영 바닥 그림은 ART.md §17(고원 한 덩어리 · 대좌 없음) · 🎲 **배치는 생성기가 뽑는다**(2026-09-09 사용자: 「항상 정해져 있지 않고 … 랜덤」 · `campFoeLayout(d, seed)`) — 규칙은 고정이고 자리만 씨앗이다: **맨 위 광맥 호 → 본진 → 테크 → 앞줄 생산**(내 기지의 거울 · 줄은 `CAMP_FOE_ROW`) · 탑은 구간 앞 가운데 · 보급고류는 좌우 바깥 · 기지 전체 좌우 반전 · 씨앗은 원정마다 새로(`C.foeSeed` · `campEnterDungeon`) — ⛔ Math.random 금지(저장·복원하면 자리가 바뀐다) · ⛔ 표(`CAMP_DG`)에 gx·gy 를 되살리지 말 것 · 📏 같은 열에 서는 쌍은 **줄 차이 − 흔들림 합 ≥ 0.07**(발판) — 탑은 세로 흔들림이 없다(씨앗 12,000개로 잡은 규칙 · 스모크가 900판을 잰다) · ⛏ **적 광맥·가스·일꾼은 연출**(사용자 확정 「연출만」 · `CAMPB._fmine/_fgas` · `campFoeWorkers3D` — 게임 값에 안 들어간다 · ⛔ `G.tech.minerals` 에 넣지 말 것: 내 일꾼이 캐러 간다) · 🔍 던전 안 축소 하한은 1.0(`campMinZoom` · 캠프는 1.45 그대로) · 🩹 시트는 서명이 아니라 **그려진 모델**(`_cgModel.foeEid`)을 본다 — 요약판이 서명을 안 지우고 덮은 적이 있다(실측 3회 중 1회) · 🗺 맵 띠 오른쪽 `.cbTgt` 「다음 **이름**」 · 👆 **적 건물을 누르면 「들여다보기」**(`CAMPB._foeSel` · 세션 값)이고 시트에 프로필(공용 `renderCmdGrid` · 「공격 대상」 카드 하나)이 뜬다 — **표적 지정은 그 카드**(`campFoeTgtSet` → `C.foeTgt`)가 한다 · ⛔ 탭에서 바로 foeTgt 를 박지 말 것: 잠긴 건물을 누르면 아무 일도 안 일어나 「안 눌린다」로 느껴진다(실측) · 안개에 가린 것은 안 눌린다 · ⊘ 해제(`techDeselU`)가 들여다보기도 닫는다 · 표식은 **한 층(`.fbLayer`)으로 갈아 끼운다** — 덧붙이기만 하면 프레임마다 쌓인다(실측 12채 → 144개) |
